	...
	try {
		...
		const completion = await openai.chat.completions.create({
			...
			messages: [
				...
				{ role: 'user', content: `Suggest a helpful reply to this customer message: ${message}` }
			],
			max_tokens: 256,
			temperature: 0.7
		});
...
router.get('/docs', (req, res) => {
	res.json({ ok: true, docs: 'Inbox Assistant API: CRUD, AI, analytics, import/export, Shopify sync, notifications, RBAC, i18n.' });
});
...
router.get('/i18n', (req, res) => {
	res.json({ ok: true, translations: { en: 'Inbox Assistant', fr: 'Assistant de boîte de réception' } });
});

// Docs endpoint (placeholder)
router.get('/docs', (req, res) => {
	res.json({ ok: true, docs: 'Inbox Assistant API. Endpoints: /tickets, /ai/suggest-reply, /analytics, /import, /export, /shopify/sync, /notify, /rbac/check, /i18n, /docs' });
});
