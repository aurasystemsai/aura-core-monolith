...
router.get('/strategies', (req, res) => {
  res.json({ ok: true, strategies: db.list() });
});
router.get('/strategies/:id', (req, res) => {
  const strategy = db.get(req.params.id);
  if (!strategy) return res.status(404).json({ ok: false, error: 'Not found' });
  res.json({ ok: true, strategy });
});
router.post('/strategies', (req, res) => {
  const strategy = db.create(req.body || {});
  res.json({ ok: true, strategy });
});
router.put('/strategies/:id', (req, res) => {
  const strategy = db.update(req.params.id, req.body || {});
  if (!strategy) return res.status(404).json({ ok: false, error: 'Not found' });
  res.json({ ok: true, strategy });
});
...
router.post('/ai/suggest', async (req, res) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-4',
      messages: [
        { role: 'system', content: 'You are an upsell/cross-sell strategy expert.' },
...
router.get('/onboarding', (req, res) => {
  res.json({ ok: true, steps: [
    'Connect your store',
    'Describe your upsell/cross-sell strategy',
    'Test and deploy',
    'Export or share strategies',
