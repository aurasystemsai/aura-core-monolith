...
router.post('/ai/blog-outline', async (req, res) => {
  try {
    const { keyword, topic, audience = 'general', tone = 'professional', sections = 'medium', model = 'gpt-4o-mini' } = req.body || {};
    if (!keyword && !topic) return res.status(400).json({ ok: false, error: 'keyword or topic required' });
    const sectCount = sections === 'small' ? '4–6' : sections === 'large' ? '9–12' : '6–9';
    const prompt = `Create a comprehensive, SEO-optimised blog post outline for target keyword: "${keyword || topic}". Target audience: ${audience}. Tone: ${tone}. Target ${sectCount} main H2 sections. Return JSON: {"title":"SEO optimised title","metaDescription":"under 160 chars","sections":[{"heading":"H2 heading","type":"intro|body|faq|conclusion","suggestedWordCount":200,"keyPoints":["3 key points to cover"],"seoTip":"tip for this section"}],"totalWordCount":0,"estimatedReadTime":"X min read","contentAngle":"unique angle","primaryKeyword":"...","secondaryKeywords":["3-5 LSI keywords"]}`;
    const completion = await getOpenAI().chat.completions.create({ model, messages: [{ role: 'user', content: prompt }], response_format: { type: 'json_object' } });
    const data = JSON.parse(completion.choices[0].message.content);
    if (req.deductCredits) req.deductCredits({ model });
...
router.post('/ai/intro-generator', async (req, res) => {
  try {
    const { keyword, topic, style = 'PAS', audience = 'general', model = 'gpt-4o-mini', outline } = req.body || {};
    if (!keyword && !topic) return res.status(400).json({ ok: false, error: 'keyword or topic required' });
    const outlineHint = outline?.title ? ` The article is titled "${outline.title}".` : '';
    const sectionHint = Array.isArray(outline?.sections) && outline.sections.length ? ` It covers these sections: ${outline.sections.slice(0, 4).map(s => s.heading).join(', ')}.` : '';
    const prompt = `Write 3 compelling blog intro paragraphs (each 80-120 words) for a post about "${keyword || topic}".${outlineHint}${sectionHint} Use the ${style} formula (Problem-Agitate-Solve for PAS, AIDA for attention-interest-desire-action). Audience: ${audience}. Return JSON: {"intros":[{"style":"...","text":"...","hookType":"stat|question|story|bold claim","hookStrength":0-10}],"recommended":0}`;
    const completion = await getOpenAI().chat.completions.create({ model, messages: [{ role: 'user', content: prompt }], response_format: { type: 'json_object' } });
    const data = JSON.parse(completion.choices[0].message.content);
    if (req.deductCredits) req.deductCredits({ model });
