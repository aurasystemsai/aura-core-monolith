...
router.get('/keywords', (req, res) => {
  res.json({ ok: true, keywords: db.list() });
});
router.get('/keywords/:id', (req, res) => {
  const kw = db.get(req.params.id);
  if (!kw) return res.status(404).json({ ok: false, error: 'Not found' });
  res.json({ ok: true, keyword: kw });
});
router.post('/keywords', (req, res) => {
  const kw = db.create(req.body || {});
  res.json({ ok: true, keyword: kw });
});
router.put('/keywords/:id', (req, res) => {
  const kw = db.update(req.params.id, req.body || {});
  if (!kw) return res.status(404).json({ ok: false, error: 'Not found' });
  res.json({ ok: true, keyword: kw });
});
...
router.post('/ai/analyze', async (req, res) => {
  try {
    ...
    const completion = await openai.chat.completions.create({
      model: 'gpt-4',
      messages: [
        { role: 'system', content: 'You are a keyword research expert.' },
