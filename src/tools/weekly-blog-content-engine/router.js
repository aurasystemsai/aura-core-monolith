...
router.post('/ai/draft', async (req, res) => {
	try {
		...
		const completion = await openai.chat.completions.create({
			model: 'gpt-4',
			messages: [
				{ role: 'system', content: 'You are a blog content generator.' },
...
router.get('/i18n', (req, res) => {
	res.json({ ok: true, translations: { en: 'Blog Post', es: 'Entrada de blog' } });
});

// Docs
router.get('/docs', (req, res) => {
	res.json({ ok: true, docs: 'Weekly Blog Content Engine API: CRUD, AI, analytics, import/export, Shopify sync, notifications, RBAC, i18n.' });
});
