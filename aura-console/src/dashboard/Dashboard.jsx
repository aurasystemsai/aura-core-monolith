...
// Quick action cards
const QuickActionCard = ({ icon, title, description, onClick, color = "#818cf8" }) => (
	<div
		onClick={onClick}
		style={{
			background: "#18181b",
...
const Dashboard = ({ setActiveSection }) => {
	...
	if (loading) {
		return (
			<div className="aura-dashboard-shell" style={{ padding: "24px", background: "#18181b", minHeight: "100vh" }}>
				<style>{`
					@keyframes shimmer {
		...
		<div className="aura-dashboard-shell" style={{ padding: "24px", background: "#18181b", minHeight: "100vh" }}>
					...
					<div style={{ background:'#18181b', border:'1px solid #27272a', borderRadius:20, width:'100%', maxWidth:720, maxHeight:'88vh', display:'flex', flexDirection:'column', animation:'modalIn 0.25s ease' }}>
						...
						<div style={{ padding:'24px 28px 20px', borderBottom:'1px solid #27272a', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
							<div style={{ display:'flex', alignItems:'center', gap:14 }}>
								{scanningInProgress ? (
									<div style={{ width:36, height:36, border:'3px solid #818cf8', borderTop:'3px solid transparent', borderRadius:'50%', animation:'spin 0.9s linear infinite', flexShrink:0 }} />
								) : (
						...
						{scanningInProgress && (
							<div style={{ padding:'20px 28px', flex:1, overflowY:'auto' }}>
								<div style={{ background:'#18181b', borderRadius:12, padding:'14px 18px', marginBottom:16, display:'flex', alignItems:'center', gap:10 }}>
									<span style={{ width:8, height:8, background:'#818cf8', borderRadius:'50%', animation:'pulse-dot 1s ease infinite', flexShrink:0 }} />
									<span style={{ color:'#818cf8', fontSize:13, fontFamily:'monospace', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>
						...
						{!scanningInProgress && crawlResults && (
							<div style={{ flex:1, overflowY:'auto', padding:'0 28px 24px' }}>
								...
								{crawlResults.totalIssues === 0 ? (
									<div style={{ textAlign:'center', padding:'48px 0', color:'#818cf8', fontSize:16 }}>No SEO issues found — your site looks great!</div>
								) : (
											...
											<div key={i}
												...
												<div style={{ flex:1, minWidth:0 }}>
													<div style={{ color:'#fafafa', fontWeight:600, fontSize:14, display:'flex', alignItems:'center', gap:8 }}>
														{issue.type}
														{issue.fix && <span style={{ fontSize:11, color:'#818cf8', background:'rgba(127,255,212,0.1)', border:'1px solid rgba(127,255,212,0.3)', borderRadius:4, padding:'1px 7px', fontWeight:500 }}>Click to fix →</span>}
													</div>
			...
			<div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999, display: "flex", flexDirection: "column", gap: 10 }}>
				{toasts.map(toast => (
					<div key={toast.id} className="aura-toast" style={{
						...
						border: `1px solid ${toast.type === 'error' ? '#e53e3e' : '#818cf8'}`,
						borderRadius: 12,
						padding: "14px 20px",
						color: toast.type === 'error' ? '#fc8181' : '#818cf8',
						fontSize: 14,
						fontWeight: 500,
						maxWidth: 360,
				...
				<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 12 }}>
					...
					<div style={{ display: "flex", gap: 12, alignItems: "center", flexWrap: "wrap" }}>
						...
						<button
							...
							style={{
								background: autoRefresh ? "#818cf8" : "#18181b",
								border: "1px solid #27272a",
								borderRadius: 8,
								padding: "10px 16px",
								color: autoRefresh ? "#18181b" : "#818cf8",
								fontSize: 14,
								fontWeight: 600,
								cursor: "pointer",
								display: "flex",
			...
			>
				...
				<div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
					...
					<button
						...
						style={{
							background: copilotLoading ? "#3f3f46" : "#818cf8",
							color: "#18181b",
							border: "none",
							borderRadius: 10,
							fontWeight: 800,
				...
				{copilotReply && (
					...
					>
						<div style={{ fontSize: 12, color: "#818cf8", fontWeight: 700, marginBottom: 8, textTransform: "uppercase" }}>
							AI Response
						</div>
			...
			{crawlResults && (
				<div style={{ background: "#18181b", border: "1px solid #27272a", borderRadius: 16, padding: 24, marginBottom: 32 }}>
					...
					{crawlResults.totalIssues === 0 ? (
						<div style={{ textAlign: "center", padding: "32px 0", color: "#818cf8", fontSize: 16 }}>
								...
								>
									...
									<div style={{ flex: 1, minWidth: 0 }}>
										<div style={{ color: "#fafafa", fontWeight: 600, fontSize: 14, display: "flex", alignItems: "center", gap: 8 }}>
											{issue.type}
											{issue.fix && (
												<span style={{ fontSize: 11, color: "#818cf8", background: "rgba(127,255,212,0.1)", border: "1px solid rgba(127,255,212,0.3)", borderRadius: 4, padding: "1px 7px", fontWeight: 500 }}>
													Click to fix →
												</span>
											)}
										</div>
			...
			{healthScore && (
				<div
					style={{
						background: "#18181b",
						border: `2px solid ${healthScore.grade === 'A' ? '#818cf8' : healthScore.grade === 'B' ? '#4ade80' : healthScore.grade === 'C' ? '#fbbf24' : healthScore.grade === 'D' ? '#fb923c' : '#ef4444'}`,
						borderRadius: 16,
						padding: 24,
						marginBottom: 32,
						display: "flex",
				...
				>
					...
					<div style={{ textAlign: "center" }}>
						<div style={{ 
							fontSize: 56, 
							fontWeight: 900, 
							color: healthScore.grade === 'A' ? '#818cf8' : healthScore.grade === 'B' ? '#4ade80' : healthScore.grade === 'C' ? '#fbbf24' : healthScore.grade === 'D' ? '#fb923c' : '#ef4444',
							lineHeight: 1,
						}}>
							{healthScore.grade}
						</div>
