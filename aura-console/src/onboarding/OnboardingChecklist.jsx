...
const ONBOARDING_STEPS = [
  ...
  { key: "explore-docs", label: "Explore the documentation",
    help: <a href="https://aurasystemsai.com/docs" target="_blank" rel="noopener" style={{color:'#4f46e5',textDecoration:'underline'}}>Open docs ↗</a> },
...
export default function OnboardingChecklist({ onComplete, forceShow, onClose }) {
  ...
  return (
    <div style={{
        ...
        {ONBOARDING_STEPS.map(step => (
          <li key={step.key} style={{marginBottom:10, display:'flex', flexDirection:'column', alignItems:'flex-start', gap:2}}>
            <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer'}}>
              <input type="checkbox" checked={!!completed[step.key]} onChange={() => handleCheck(step.key)} style={{accentColor:'#4f46e5', width:18, height:18}} aria-label={step.label} />
      ...
      <button onClick={handleReset} style={{background:'none', color:'#4f46e5', border:'none', fontSize:14, cursor:'pointer', marginTop:2}}>Reset</button>
      <button onClick={() => { handleDismiss(); if (onClose) onClose(); }} style={{background:'none', color:'#4f46e5', border:'none', fontSize:14, cursor:'pointer', marginLeft:12}}>Dismiss</button>
      {forceShow && <button onClick={onClose} style={{background:'none', color:'#4f46e5', border:'none', fontSize:14, cursor:'pointer', marginLeft:12}}>Close</button>}
    </div>
  );
}
