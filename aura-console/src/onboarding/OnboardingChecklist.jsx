...
const ONBOARDING_STEPS = [
  ...
  { key: "explore-docs", label: "Explore the documentation",
    help: <a href="https://aurasystemsai.com/docs" target="_blank" rel="noopener" style={{color:'#6366f1',textDecoration:'underline'}}>Open docs ↗</a> },
...
export default function OnboardingChecklist({ onComplete, forceShow, onClose }) {
  ...
  return (
    <div style={{
      position: 'fixed', bottom: 32, right: 32, zIndex: 9999, background: '#0d0d11', color: '#fff', borderRadius: 14, boxShadow: '0 4px 24px #0008', padding: '24px 32px', minWidth: 260, maxWidth: 360, fontSize: 16, fontWeight: 500, letterSpacing: '0.01em', animation: 'fadeIn 0.4s', border: '1.5px solid #6366f1',
    }}>
      <div style={{fontWeight:700, fontSize:18, color:'#6366f1', marginBottom:10}}>Getting Started</div>
        ...
        {ONBOARDING_STEPS.map(step => (
          <li key={step.key} style={{marginBottom:10, display:'flex', flexDirection:'column', alignItems:'flex-start', gap:2}}>
            <label style={{display:'flex',alignItems:'center',gap:8,cursor:'pointer'}}>
              <input type="checkbox" checked={!!completed[step.key]} onChange={() => handleCheck(step.key)} style={{accentColor:'#6366f1', width:18, height:18}} aria-label={step.label} />
      ...
      <button onClick={handleReset} style={{background:'none', color:'#6366f1', border:'none', fontSize:14, cursor:'pointer', marginTop:2}}>Reset</button>
      <button onClick={() => { handleDismiss(); if (onClose) onClose(); }} style={{background:'none', color:'#6366f1', border:'none', fontSize:14, cursor:'pointer', marginLeft:12}}>Dismiss</button>
      {forceShow && <button onClick={onClose} style={{background:'none', color:'#6366f1', border:'none', fontSize:14, cursor:'pointer', marginLeft:12}}>Close</button>}
    </div>
  );
}
