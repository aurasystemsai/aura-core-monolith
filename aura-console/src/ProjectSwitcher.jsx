...
function ProjectSwitcher({ coreUrl, currentProject, onSelectProject, onDisconnect }) {
 ...
 <div style={{ fontSize: 12, color: "#e4e4e7", marginTop: 8 }}>{t('project_switcher_no_projects')}</div>
 )}
 </div>
 );
 ...
 >
 <button
 type="button"className="button button--ghost"onClick={() => setOpen((prev) => !prev)}
 >
 {open ? "Close project list": "Switch project"}
 </button>

 <button
 type="button"className="button button--ghost"style={{ fontSize: 11, padding: "4px 10px"}}
 ...
 <li key={p.id || p.name}>
 <button
 type="button"onClick={() => !isActive && handleSelect(p)}
 style={{
 width: "100%",
 textAlign: "left",
 borderRadius: 8,
 border: isActive
 ? "1px solid rgba(34,211,238,0.5)": "1px solid rgba(55,65,81,0.9)",
 background: isActive
 ? "rgba(34,211,238,0.12)": "rgba(15,23,42,0.98)",
