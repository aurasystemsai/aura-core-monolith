...
function AbandonedCheckoutTool() {
 ...
 return (
 <ProtectedTool 
 toolId="abandoned-checkout"toolName="Abandoned Checkout Winback">
 ...
 <UsageLimitWarning 
 limitType="ai_runs"currentUsage={usage.ai_runs}
 ...
 <button 
 className="btn btn-primary"disabled={isLimitReached('ai_runs')}
...
function ToolsDashboard() {
 ...
 const allTools = [
 { id: 'product-seo', name: 'Product SEO', category: 'free', icon: ''},
 ...
 {accessible ? (
 <button 
 className="btn btn-sm btn-primary"onClick={() => window.location.href = `/tools/${tool.id}`}
 >
 Open Tool
 </button>
 ) : (
 <button 
 className="btn btn-sm btn-secondary"onClick={() => window.location.href = '/billing'}
 >
 Upgrade
 </button>
 )}
