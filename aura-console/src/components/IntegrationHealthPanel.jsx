    ...
    <section className="integration-health-card" style={{ marginTop: 18, background: '#18181b', borderRadius: 18, boxShadow: '0 4px 24px #0004', padding: '32px 36px', maxWidth: 540, marginLeft: 'auto', marginRight: 'auto' }}>
      <div className="card-header" style={{ marginBottom: 18 }}>
        ...
        <p className="card-subtitle" style={{ color: '#b3c2e0', fontWeight: 500, fontSize: 15, marginTop: 6 }}>
          Shopify connection status for your app. If this is <span style={{color:'#22c55e',fontWeight:700}}>green</span>, your automations and analytics will work. If <span style={{color:'#ff4d4f',fontWeight:700}}>red</span>, check your app setup.
        </p>
      </div>
      <div className="integration-health-grid" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginTop: 18 }}>
        {INTEGRATIONS.map((integration) => {
          ...
          if (status === "ok") {
            ...
            icon = <span style={{ color: '#22c55e', fontSize: 28, marginRight: 12 }}>✓</span>;
          ...
          return (
            <div key={integration.id} className={statusClass} title={integration.name} style={{ minWidth: 260, display: 'flex', alignItems: 'center', background: '#18181b', borderRadius: 16, padding: '18px 28px', fontWeight: 700, fontSize: 18, color: status === 'ok' ? '#22c55e' : status === 'checking' ? '#ffe066' : '#ff4d4f', boxShadow: '0 2px 12px #0002', margin: 0 }}>
              {icon}
              <span className="integration-health-text" style={{ fontSize: 18, fontWeight: 800, letterSpacing: '0.01em' }}>{integration.name}</span>
              <span style={{ marginLeft: 18, fontWeight: 700, fontSize: 16, color: status === 'ok' ? '#22c55e' : status === 'checking' ? '#ffe066' : '#ff4d4f' }}>
