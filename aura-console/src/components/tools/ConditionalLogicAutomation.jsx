...
export default function ConditionalLogicAutomation() {
  ...
  return (
    <div style={{ background: "#0d0d11", borderRadius: 18, boxShadow: "0 15px 60px #0007", padding: 32, fontFamily: "Inter, sans-serif", color: "#f9fafb", border: "1px solid #222535" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12, gap: 12, flexWrap: "wrap" }}>
        <BackButton label="← Back to Suite" onClick={goBackToSuite} />
        <div style={{ color: "#a8adc4", fontSize: 13 }}>Workflows Suite · Conditional Logic & Branching</div>
      </div>
      ...
      {isViewer && (
        <div style={{ background: "#222535", border: "1px solid #454860", borderRadius: 12, padding: 12, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ color: "#a8adc4", fontSize: 13 }}>You can inspect conditional flows but need elevated access to edit or run simulations.</div>
          </div>
      ...
      {devSandbox && !isViewer && (
        <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12, marginBottom: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ color: "#a8adc4", fontSize: 13 }}>Publishing is disabled in dev. Switch to Stage/Prod to run full simulations and attach preflight.</div>
          </div>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {history.slice(0, 3).map((h, idx) => (
              <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: "8px 10px" }}>
                <div>
                  <div style={{ fontWeight: 700, color: "#f9fafb" }}>{h.summary || "Run"} · {h.env}</div>
                  <div style={{ color: "#a8adc4", fontSize: 12 }}>Saved {h.at ? new Date(h.at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "recent"}</div>
                </div>
      ...
      <div style={{ position: "sticky", top: 0, zIndex: 4, display: "flex", gap: 12, flexWrap: "wrap", background: "#0d0d11", paddingBottom: 10 }}>
        <div style={{ display: "flex", gap: 8, alignItems: "center", background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: "8px 12px" }}>
          <span style={{ color: "#a8adc4", fontWeight: 700 }}>Env</span>
          {["dev", "stage", "prod"].map(opt => (
            <button key={opt} onClick={() => setEnv(opt)} style={{ background: env === opt ? "#0ea5e9" : "#1a1c25", color: env === opt ? "#1a1c25" : "#f9fafb", border: "1px solid #222535", borderRadius: 999, padding: "6px 12px", fontWeight: 800, cursor: "pointer" }}>
              {opt.toUpperCase()}
            </button>
          ))}
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <button onClick={runPreflight} style={{ background: "#222535", color: "#fcd34d", border: "1px solid #454860", borderRadius: 12, padding: "10px 12px", fontWeight: 800, cursor: "pointer" }}>Preflight (Ctrl+S)</button>
          <button onClick={runDryRun} style={{ background: "#22c55e", color: "#1a1c25", border: "none", borderRadius: 12, padding: "10px 12px", fontWeight: 900, cursor: "pointer" }}>Dry-run (dev)</button>
          {preflightStatus && (
            <span style={{ position: "relative", display: "inline-flex", alignItems: "center", gap: 6, padding: "6px 10px", borderRadius: 999, border: "1px solid #222535", background: preflightStatus.ok ? "#1a1c25" : "#222535", color: preflightStatus.ok ? "#22c55e" : preflightStatus.issues ? "#fcd34d" : "#f87171", fontWeight: 800, fontSize: 12 }}>
              ...
              {showPreflightPopover && (
                <div style={{ position: "absolute", top: "110%", right: 0, minWidth: 220, background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: 10, boxShadow: "0 10px 30px rgba(0,0,0,0.4)", zIndex: 10 }}>
                  ...
                  <div style={{ color: "#a8adc4", fontSize: 12, marginBottom: 6 }}>Why this matters: stops broken branches from impacting Shopify customers.</div>
                  ...
                  <div style={{ marginTop: 8, display: "flex", gap: 6, flexWrap: "wrap" }}>
                    <button onClick={() => applyQuickFix("approver")} style={{ background: "#0ea5e9", color: "#1a1c25", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: "pointer" }}>Add approver note</button>
          ...
          <button onClick={rollbackToLastSimulation} disabled={!lastSimulatedSnapshot} style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 12, padding: "10px 12px", fontWeight: 800, cursor: lastSimulatedSnapshot ? "pointer" : "not-allowed", opacity: lastSimulatedSnapshot ? 1 : 0.5 }}>Rollback to last sim</button>
          <button onClick={() => setSelectedPayloadPreset(p => p)} style={{ background: "#0ea5e91a", color: "#67e8f9", border: "1px solid #222535", borderRadius: 12, padding: "10px 12px", fontWeight: 800 }}>Dev Payload Presets</button>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", marginLeft: "auto" }}>
          <span style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 999, padding: "6px 10px", color: (flowNodes.filter(n => n.type === "action").length >= 4 || (branchGroup.branches || []).length >= 3) ? "#f97316" : "#22c55e", fontWeight: 700 }}>Perf guardrail: {(flowNodes.filter(n => n.type === "action").length >= 4 || (branchGroup.branches || []).length >= 3) ? "tighten" : "OK"}</span>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 12 }}>
        <input value={reviewerEmail} onChange={e => setReviewerEmail(e.target.value)} placeholder="Reviewer email" style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 10, padding: "6px 10px", minWidth: 220 }} />
        <button onClick={attachPreflightForReviewer} style={{ background: "#8b5cf6", color: "#1a1c25", border: "none", borderRadius: 10, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>Attach preflight</button>
        <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: "6px 10px", color: preflightIssues.length ? "#f97316" : "#22c55e", fontWeight: 700 }}>Guardrails: {preflightIssues.length ? `${preflightIssues.length} issues` : "clear"}</div>
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", marginBottom: 18 }}>
        <div>
          <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
            <h2 style={{ fontWeight: 800, fontSize: 30, margin: 0, color: "#a5f3fc" }}>Conditional Logic & Branching</h2>
            <span style={{ background: "#222535", color: "#93c5fd", padding: "6px 10px", borderRadius: 999, fontSize: 13, fontWeight: 700 }}>Pro</span>
            {activeTemplate && (
          ...
          <div style={{ color: "#a8adc4", marginTop: 6 }}>
            Build multi-branch workflows, simulate outcomes, and export/import configs in one place.
          </div>
        </div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          <button onClick={validate} style={{ background: "#222535", color: "#e0f2fe", border: "1px solid #454860", borderRadius: 10, padding: "10px 14px", fontWeight: 700, cursor: "pointer" }}>
             Validate
          </button>
          <button onClick={simulate} style={{ background: "#22c55e", color: "#1a1c25", border: "none", borderRadius: 10, padding: "10px 14px", fontWeight: 800, cursor: "pointer" }}>
            ️ Simulate
          </button>
          <button onClick={() => fileInputRef.current?.click()} style={{ background: "#fbbf24", color: "#222535", border: "none", borderRadius: 10, padding: "10px 14px", fontWeight: 800, cursor: "pointer" }}>
            ️ Import
          </button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: 10, marginBottom: 12 }}>
        <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: "#a8adc4", fontSize: 13 }}>{healthSignals.summary}</div>
        </div>
        <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: "#a8adc4", fontSize: 12, marginBottom: preflightIssues.length ? 6 : 0 }}>Trigger ready: {healthSignals.triggerOk ? "Yes" : "No"}</div>
      ...
      <div style={{ marginBottom: 12, background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12 }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 6 }}>
          ...
          <div style={{ color: "#a8adc4", fontSize: 12 }}>Keeps you honest before shipping</div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 16 }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(160px, 1fr))", gap: 12 }}>
            {[{ label: "Triggers", value: stats.triggers, color: "#93c5fd" }, { label: "Conditions", value: stats.conditions, color: "#a5f3fc" }, { label: "Actions", value: stats.actions, color: "#c084fc" }, { label: "Branches", value: stats.branches, color: "#f59e0b" }].map(card => (
              <div key={card.label} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12 }}>
                <div style={{ color: card.color, fontSize: 24, fontWeight: 800 }}>{card.value}</div>
                <div style={{ color: "#a8adc4", fontWeight: 600 }}>{card.label}</div>
              </div>
            ))}
          </div>

          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
              <div style={{ fontWeight: 700, color: "#f9fafb" }}>Palette & Templates</div>
              <input
                value={paletteFilter}
                onChange={e => setPaletteFilter(e.target.value)}
                placeholder="Search triggers, conditions, actions"
                style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 10, padding: "8px 10px", width: 240 }}
              />
            </div>
            <div style={{ marginTop: 10, display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10 }}>
              {[{ title: "Triggers", items: filteredPalette.triggers, accent: "#38bdf8" }, { title: "Conditions", items: filteredPalette.conditions, accent: "#a78bfa" }, { title: "Actions", items: filteredPalette.actions, accent: "#22c55e" }].map(group => (
                <div key={group.title} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 10 }}>
                  <div style={{ color: group.accent, fontWeight: 700, marginBottom: 6 }}>{group.title}</div>
                  {group.items.map(item => (
                    <div key={item.title} style={{ marginBottom: 8, padding: 8, background: "#1a1c25", borderRadius: 10, border: "1px solid #222535" }}>
                      <div style={{ fontWeight: 700, color: "#f9fafb" }}>{item.title}</div>
                      <div style={{ color: "#a8adc4", fontSize: 13 }}>{item.description}</div>
                      <button onClick={() => addNode(item)} style={{ marginTop: 6, background: group.accent, color: "#1a1c25", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Add</button>
                    </div>
                  ))}
                </div>
              ))}
            ...
            <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap" }}>
              {TEMPLATE_PRESETS.map(t => (
                <button key={t.id} onClick={() => applyTemplate(t)} style={{ background: "#1a1c25", color: "#a5f3fc", border: "1px solid #222535", borderRadius: 10, padding: "10px 12px", fontWeight: 700, cursor: "pointer" }}>
                   {t.name}
                </button>
              ))}
            </div>
          ...
          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontWeight: 800, color: "#f9fafb" }}>Flow Canvas</div>
              <span style={{ color: "#a8adc4" }}>Drag-less inline editing</span>
            </div>
            {flowNodes.length === 0 && (
              <div style={{ color: "#a8adc4", background: "#1a1c25", border: "1px dashed #222535", borderRadius: 10, padding: 16 }}>
                Add triggers/conditions/actions from the palette or load a template.
              </div>
            )}
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {flowNodes.map(node => (
                <div key={node.id} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                      <span style={{ background: "#1a1c25", padding: "4px 10px", borderRadius: 999, color: "#e9ebf5", fontWeight: 700 }}>{node.type?.toUpperCase()}</span>
                      <input
                        value={node.title || ""}
                        onChange={e => updateNode(node.id, { title: e.target.value })}
                        style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", minWidth: 140 }}
                      />
                    </div>
                  ...
                  <textarea
                    ...
                    rows={2}
                    style={{ width: "100%", background: "#1a1c25", color: "#e9ebf5", border: "1px solid #222535", borderRadius: 10, padding: 8, marginTop: 8 }}
                  ...
                  {node.type === "condition" && (
                    <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                      <input
                        ...
                        placeholder="field"
                        style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", flex: 1, minWidth: 120 }}
                      />
                      <select
                        value={node.config?.operator || "equals"}
                        onChange={e => updateNode(node.id, { config: { ...node.config, operator: e.target.value } })}
                        style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", minWidth: 130 }}
                      >
                        {OPERATORS.map(op => (
                  ...
                  {node.type === "trigger" && (
                    <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                      <input
                        ...
                        style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", minWidth: 160 }}
                      />
                      <input
                  ...
                  {node.type === "action" && (
                    <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                      <input
                        ...
                        style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", minWidth: 180 }}
                      />
                      <input
          ...
          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontWeight: 800, color: "#f9fafb" }}>Branching Builder</div>
              <button onClick={addBranch} style={{ background: "#22c55e", color: "#1a1c25", border: "none", borderRadius: 10, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>+ Add Branch</button>
            </div>
            {(branchGroup.branches || []).length === 0 && (
              <div style={{ color: "#a8adc4", background: "#1a1c25", border: "1px dashed #222535", borderRadius: 10, padding: 14 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
              {(branchGroup.branches || []).map((branch, idx) => (
                <div key={branch.id} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                    <span style={{ background: "#1a1c25", color: "#c084fc", padding: "4px 10px", borderRadius: 999, fontWeight: 800 }}>IF #{idx + 1}</span>
                    <input
                      value={branch.label || ""}
                  ...
                  <div style={{ display: "flex", gap: 8, marginTop: 8, flexWrap: "wrap" }}>
                    ...
                    <select
                      ...
                      style={{ background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", minWidth: 150 }}
                    >
                      {OPERATORS.map(op => (
                        <option key={op} value={op}>{op}</option>
                      ))}
                  ...
                  <div style={{ marginTop: 10 }}>
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                      <div style={{ color: "#a8adc4", fontWeight: 700 }}>Actions</div>
                      <div style={{ display: "flex", gap: 6 }}>
                        {ACTION_LIBRARY.map(a => (
                          <button key={a.title + branch.id} onClick={() => addBranchAction(branch.id, a)} style={{ background: "#1a1c25", color: "#22c55e", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", cursor: "pointer", fontWeight: 700 }}>
                            + {a.title}
                          </button>
                        ))}
                      </div>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
                      {(branch.actions || []).length === 0 && (
                        <div style={{ color: "#8b8fa8", background: "#1a1c25", borderRadius: 8, padding: 10, border: "1px dashed #222535" }}>No actions yet.</div>
                      )}
                      {(branch.actions || []).map(action => (
                        <div key={action.id} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: 8, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8 }}>
                          <div>
                            <div style={{ color: "#22c55e", fontWeight: 700 }}>{action.title}</div>
                            <div style={{ color: "#a8adc4", fontSize: 13 }}>{action.description}</div>
                          </div>
            ...
            <div style={{ marginTop: 12, background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 10 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                <div style={{ color: "#f97316", fontWeight: 800 }}>Else</div>
                <div style={{ display: "flex", gap: 6 }}>
                  {ACTION_LIBRARY.map(a => (
                    <button key={a.title + "else"} onClick={() => addElseAction(a)} style={{ background: "#1a1c25", color: "#f59e0b", border: "1px solid #222535", borderRadius: 8, padding: "6px 8px", cursor: "pointer", fontWeight: 700 }}>
                      + {a.title}
                    </button>
                  ))}
                </div>
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: 8, marginTop: 8 }}>
                {(branchGroup.elseActions || []).length === 0 && (
                  <div style={{ color: "#8b8fa8", background: "#1a1c25", borderRadius: 8, padding: 10, border: "1px dashed #222535" }}>No else actions yet.</div>
                )}
          ...
          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
              <div style={{ fontWeight: 800, color: "#f9fafb" }}>Simulation Preview</div>
              <small style={{ color: "#a8adc4" }}>Paste sample payload to test pathing</small>
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 8 }}>
              {PAYLOAD_PRESETS.map(p => (
                <button key={p.id} onClick={() => { setSelectedPayloadPreset(p.id); setSimulationInput(JSON.stringify(p.payload, null, 2)); }} style={{ background: selectedPayloadPreset === p.id ? "#0ea5e9" : "#1a1c25", color: selectedPayloadPreset === p.id ? "#1a1c25" : "#f9fafb", border: "1px solid #222535", borderRadius: 10, padding: "8px 10px", fontWeight: 700, cursor: "pointer" }}>
            ...
            <textarea
              ...
              rows={7}
              style={{ width: "100%", background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 12, padding: 10 }}
            />
            <button onClick={simulate} style={{ marginTop: 10, background: "#22c55e", color: "#1a1c25", border: "none", borderRadius: 10, padding: "10px 14px", fontWeight: 800, cursor: "pointer" }}>Run Simulation</button>
            {simulationResult && (
              <div style={{ marginTop: 10, background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 12 }}>
                <div style={{ color: "#a5f3fc", fontWeight: 800 }}>Matched: {simulationResult.matchedBranch}</div>
                <div style={{ color: "#a8adc4", marginTop: 4 }}>Actions to fire:</div>
                <ul style={{ margin: 0, paddingLeft: 18, color: "#f9fafb" }}>
                  ...
                  {simulationResult.actions.length === 0 && <li style={{ color: "#8b8fa8" }}>No actions</li>}
                </ul>
                {diffSummary && (
                  <div style={{ marginTop: 10, color: "#a8adc4", fontSize: 13 }}>
                    <div style={{ color: "#f9fafb", fontWeight: 700 }}>Changes since last simulation</div>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ fontWeight: 800, color: "#f9fafb", marginBottom: 8 }}>Data Explorer</div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 10 }}>
              <button onClick={fetchLogicBlocks} style={{ background: "#1a1c25", color: "#a5f3fc", border: "1px solid #222535", borderRadius: 8, padding: "8px 10px", fontWeight: 700, cursor: "pointer" }}>Load Logic Blocks</button>
              <button onClick={fetchWorkflows} style={{ background: "#1a1c25", color: "#22c55e", border: "1px solid #222535", borderRadius: 8, padding: "8px 10px", fontWeight: 700, cursor: "pointer" }}>Load Workflows</button>
              <button onClick={fetchTriggers} style={{ background: "#1a1c25", color: "#38bdf8", border: "1px solid #222535", borderRadius: 8, padding: "8px 10px", fontWeight: 700, cursor: "pointer" }}>Load Triggers</button>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 8, maxHeight: 300, overflowY: "auto" }}>
              {logicBlocks.map(b => (
                <div key={b.id} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: 8 }}>
                  <div style={{ color: "#a5f3fc", fontWeight: 700 }}>{b.name}</div>
                  <div style={{ color: "#a8adc4", fontSize: 13 }}>{b.description}</div>
              ...
              {workflows.map(w => (
                <div key={w.id} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: 8 }}>
                  <div style={{ color: "#22c55e", fontWeight: 700 }}>{w.name}</div>
                  <div style={{ color: "#a8adc4", fontSize: 13 }}>Steps: {(w.steps || []).join(", ")}</div>
                </div>
              ))}
              {triggers.map(t => (
                <div key={t.id} style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: 8 }}>
          ...
          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ fontWeight: 800, color: "#f9fafb", marginBottom: 8 }}>Assistant</div>
            <textarea
              value={aiPrompt}
            ...
            {aiResponse && (
              <div style={{ marginTop: 8, background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 10, color: "#f9fafb", whiteSpace: "pre-wrap" }}>{aiResponse}</div>
            )}
          </div>

          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ fontWeight: 800, color: "#f9fafb", marginBottom: 8 }}>Validation</div>
            ...
            {preflightTrace.length > 0 && (
              <div style={{ marginTop: 10, background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: 10 }}>
                ...
                <ul style={{ margin: 6, paddingLeft: 18, color: "#f9fafb", display: "flex", flexDirection: "column", gap: 6 }}>
            ...
            {env === "prod" && (
              <div style={{ marginTop: 10, background: "#0ea5e91a", border: "1px solid #0ea5e9", borderRadius: 10, padding: 10 }}>
                ...
                <input
                  ...
                  style={{ marginTop: 6, width: "100%", background: "#1a1c25", color: "#f9fafb", border: "1px solid #222535", borderRadius: 8, padding: "8px 10px" }}
                />
              </div>
            )}
          </div>
          ...
          <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 14, padding: 14 }}>
            <div style={{ fontWeight: 800, color: "#f9fafb", marginBottom: 8 }}>Feedback</div>
            <form onSubmit={e => { e.preventDefault(); handleFeedback(); }} aria-label="Send feedback">
              <textarea
                value={feedback}
                onChange={e => setFeedback(e.target.value)}
                rows={2}
                style={{ width: "100%", fontSize: 15, padding: 10, borderRadius: 10, border: "1px solid #222535", marginBottom: 10, background: "#1a1c25", color: "#f9fafb" }}
                placeholder="What should we improve next?"
                aria-label="Feedback input"
              />
              <button type="submit" style={{ background: "#6366f1", color: "#1a1c25", border: "none", borderRadius: 10, padding: "10px 14px", fontWeight: 800, cursor: "pointer" }}>Send Feedback</button>
            </form>
          </div>

          {(imported || error) && (
            <div style={{ background: "#1a1c25", border: "1px solid #222535", borderRadius: 12, padding: 10 }}>
