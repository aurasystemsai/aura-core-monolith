...
export default function ProductSEOEngine() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await apiFetch("/api/product-seo-engine/history");
  ...
  const fetchAnalytics = async () => {
    try {
      const res = await apiFetch("/api/product-seo-engine/analytics");
      const data = await res.json();
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      // Save to history
      await apiFetch("/api/product-seo-engine/history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ product: input, report: data.seoReport })
      });
      fetchHistory();
    } catch (err) {
  ...
  const handleImport = e => {
    ...
    reader.onload = async evt => {
      try {
        const res = await apiFetch("/api/product-seo-engine/import", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", background: "#23263a", color: "#f3f4f6", borderRadius: 18, boxShadow: "0 2px 24px #0002", padding: 36, fontFamily: 'Inter, sans-serif' }}>
      <h2 style={{ fontWeight: 800, fontSize: 32, marginBottom: 8 }}>Product SEO Engine</h2>
      <div style={{ marginBottom: 10, color: "#0ea5e9", fontWeight: 600 }}>
        <span role="img" aria-label="sparkles">✨</span> Generate, manage, and analyze product SEO with AI and analytics.
      </div>
      <textarea
        value={input}
        ...
        placeholder="Type your product or prompt here..."
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        {exported && <a href={exported} download="product-seo-history.json" style={{ marginLeft: 8, color: "#0ea5e9", fontWeight: 600 }}>Download</a>}
      </div>
