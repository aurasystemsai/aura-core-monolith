...
function SectionCard({ title, description, children, accent }) {
  return (
    <div style={{ background: "#0d0d11", border: `1px solid ${accent || "#222535"}` , borderRadius: 14, padding: 18, marginBottom: 14 }}>
      ...
      {description && <div style={{ color: "#a8adc4", fontSize: 13, marginBottom: 10 }}>{description}</div>}
      {children}
    </div>
  );
}
...
export default function ProductSEOEngine() {
  ...
  const renderTab = () => {
    switch (activeTab) {
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10 }}>
              {products.map(p => (
                <div key={p.id} style={{ border: `1px solid ${selectedProduct?.id === p.id ? "#6366f1" : "#222535"}`, background: "#1a1c25", borderRadius: 12, padding: 12 }}>
                  ...
                  <Divider />
                  <div style={{ color: "#a8adc4", fontSize: 12 }}>Price: ${p.price || "-"}</div>
      ...
      case "product-editor": {
        ...
        return (
          <SectionCard title="Product Editor" description="Edit product fields · per-field AI generate · focus keywords · SERP preview">
            {!selectedProduct ? (
              <div style={{ color: "#a8adc4" }}>Select a product from Product List.</div>
            ) : (
                ...
                <div>
                  ...
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, background: "#1a1c25", border: "1px solid #222535", borderRadius: 10, padding: "8px 10px", minHeight: 44, alignItems: "center" }}>
                    ...
                    <input
                      ...
                      onBlur={addKw}
                      style={{ flex: 1, minWidth: 140, background: "none", border: "none", color: "#f9fafb", fontSize: 13, outline: "none" }}
      ...
      case "templates":
        return (
          <SectionCard title="Templates" description="Prompt templates powered by /ai/prompts">
            ...
            <Divider />
            <div style={{ color: "#a8adc4", fontSize: 12 }}>Use prompt templates to accelerate optimization workflows.</div>
          </SectionCard>
        );
      case "categories":
        return (
      ...
      case "trash-recovery":
        return (
          <SectionCard title="Trash & Recovery" description="Placeholder for soft-delete flows.">
            <div style={{ color: "#a8adc4" }}>Soft-delete and restore actions can be wired to bulk-delete and rollback endpoints.</div>
          </SectionCard>
        );
      case "title-optimization":
        return (
          <SectionCard title="Title Optimization" description="Call /title-suggestions and apply best title." accent="#6366f1">
            ...
            {selectedProduct?.title && <div style={{ color: "#a8adc4", fontSize: 13 }}>Preview: {selectedProduct.title}</div>}
          </SectionCard>
        );
      ...
      case "notifications":
        return (
          <SectionCard title="Notifications" description="Notification preferences." accent="#eab308">
            <div style={{ color: "#a8adc4" }}>Configure weekly digests, anomaly alerts, and webhook events.</div>
          </SectionCard>
        );
      case "integrations":
        return (
      ...
      default:
        return <div style={{ color: "#a8adc4" }}>Tab not implemented yet.</div>;
    }
  };

  return (
    <div style={{ background: "#05080f", minHeight: "100%", padding: 20, color: "#f9fafb", fontFamily: "Inter, system-ui, sans-serif" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 800 }}>Product SEO Engine</div>
          <div style={{ color: "#a8adc4" }}>42-tab enterprise console · Backed by 200 endpoints</div>
        </div>
