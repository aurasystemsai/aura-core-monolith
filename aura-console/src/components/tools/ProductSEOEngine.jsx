...
function SectionCard({ title, description, children, accent }) {
  return (
    <div style={{ background: "#0a0b0f", border: `1px solid ${accent || "#283044"}` , borderRadius: 14, padding: 18, marginBottom: 14 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        <div style={{ color: "#f6f7fb", fontWeight: 700 }}>{title}</div>
        {accent && <span style={{ width: 10, height: 10, borderRadius: "50%", background: accent }} />}
      </div>
      {description && <div style={{ color: "#b8bed2", fontSize: 13, marginBottom: 10 }}>{description}</div>}
      {children}
    </div>
  );
}
...
function StatPill({ label, value }) {
  return (
    <div style={{ padding: "10px 14px", background: "#1f2433", border: "1px solid #283044", borderRadius: 12, minWidth: 140 }}>
      <div style={{ color: "#b8bed2", fontSize: 12 }}>{label}</div>
      <div style={{ color: "#f6f7fb", fontWeight: 700 }}>{value}</div>
    </div>
  );
}
...
function InlineInput({ value, onChange, placeholder, type = "text", width = "100%" }) {
  return (
    <input
      ...
      style={{ width, background: "#1f2433", border: "1px solid #283044", color: "#f6f7fb", padding: "10px 12px", borderRadius: 10 }}
    />
  );
}

function Divider() {
  return <div style={{ height: 1, background: "#283044", margin: "12px 0" }} />;
}
...
export default function ProductSEOEngine() {
  ...
  const renderList = (items, key) => (
    <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10 }}>
      {items.map((item, idx) => (
        <div key={idx} style={{ padding: 12, borderRadius: 10, background: "#1f2433", border: "1px solid #283044" }}>
          <pre style={{ margin: 0, color: "#f6f7fb", fontSize: 12, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{JSON.stringify(item, null, 2)}</pre>
        </div>
      ))}
    </div>
  );
  ...
  const renderTab = () => {
    switch (activeTab) {
      case "product-list":
        return (
          <SectionCard title="Product List" description="Browse and select products. Uses /api/product-seo/products.">
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(240px, 1fr))", gap: 10 }}>
              {products.map(p => (
                <div key={p.id} style={{ border: `1px solid ${selectedProduct?.id === p.id ? "#4f46e5" : "#283044"}`, background: "#1f2433", borderRadius: 12, padding: 12 }}>
                  <div style={{ color: "#f6f7fb", fontWeight: 700 }}>{p.title}</div>
                  <div style={{ color: "#b8bed2", fontSize: 12 }}>{p.slug}</div>
                  <Divider />
                  <div style={{ color: "#b8bed2", fontSize: 12 }}>Price: ${p.price || "-"}</div>
      ...
      case "product-editor": {
        ...
        return (
          <SectionCard title="Product Editor" description="Edit product fields · per-field AI generate · focus keywords · SERP preview">
            {!selectedProduct ? (
              <div style={{ color: "#b8bed2" }}>Select a product from Product List.</div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>

                {/* Title */}
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#b8bed2", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>Title</div>
                  <div style={{ display: "flex", gap: 8 }}>
                    <input value={selectedProduct.title || ""} onChange={e => setSelectedProduct({ ...selectedProduct, title: e.target.value })} placeholder="Product title" style={{ flex: 1, background: "#1f2433", border: "1px solid #283044", color: "#f6f7fb", padding: "10px 12px", borderRadius: 10 }} />
                ...
                {/* Description */}
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#b8bed2", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>Description</div>
                  <div style={{ display: "flex", gap: 8, alignItems: "flex-start" }}>
                    <textarea value={selectedProduct.description || ""} onChange={e => setSelectedProduct({ ...selectedProduct, description: e.target.value })} rows={4} placeholder="Product description" style={{ flex: 1, background: "#1f2433", border: "1px solid #283044", color: "#f6f7fb", padding: "10px 12px", borderRadius: 10, resize: "vertical" }} />
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#b8bed2", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>URL Handle / Slug</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input value={selectedProduct.slug || ""} onChange={e => setSelectedProduct({ ...selectedProduct, slug: e.target.value })} placeholder="url-handle" style={{ flex: 1, background: "#1f2433", border: "1px solid #283044", color: "#f6f7fb", padding: "10px 12px", borderRadius: 10 }} />
                  ...
                  <div>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#b8bed2", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>Image Alt Text</div>
                    <div style={{ display: "flex", gap: 8 }}>
                      <input value={selectedProduct.altText || ""} onChange={e => setSelectedProduct({ ...selectedProduct, altText: e.target.value })} placeholder="Alt text…" style={{ flex: 1, background: "#1f2433", border: "1px solid #283044", color: "#f6f7fb", padding: "10px 12px", borderRadius: 10 }} />
                ...
                {/* Focus Keywords */}
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#b8bed2", marginBottom: 5, textTransform: "uppercase", letterSpacing: 0.5 }}>Focus Keywords <span style={{ fontWeight: 400, textTransform: "none" }}>(Enter or comma to add)</span></div>
                  <div style={{ display: "flex", flexWrap: "wrap", gap: 6, background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: "8px 10px", minHeight: 44, alignItems: "center" }}>
                    {focusKeywords.map(kw => (
                      <span key={kw} style={{ background: "#283044", color: "#60a5fa", borderRadius: 20, padding: "3px 11px 3px 12px", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
                        {kw}
                    ...
                    <input
                      ...
                      onBlur={addKw}
                      style={{ flex: 1, minWidth: 140, background: "none", border: "none", color: "#f6f7fb", fontSize: 13, outline: "none" }}
                ...
                {focusKeywords.length > 0 && (
                  <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: "12px 14px" }}>
                    <div style={{ fontSize: 12, fontWeight: 700, color: "#b8bed2", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>Keyword Presence Check</div>
                    <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
                      {focusKeywords.map(kw => (
                        <div key={kw} style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 8, padding: "6px 12px", fontSize: 13 }}>
                          <span style={{ fontWeight: 600, color: "#f6f7fb" }}>{kw}</span>
                ...
                {/* Google SERP Preview */}
                <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: "14px 16px" }}>
                  <div style={{ fontSize: 12, fontWeight: 700, color: "#b8bed2", textTransform: "uppercase", letterSpacing: 0.5, marginBottom: 10 }}>Google SERP Preview</div>
                  <div style={{ background: "#4f46e5", borderRadius: 8, padding: "14px 18px", maxWidth: 600 }}>
                    <div style={{ fontSize: 12, color: "#283044", marginBottom: 2 }}>{storeBase}/products/{serpSlug}</div>
                    <div style={{ fontSize: 20, color: "#4b5780", fontWeight: 500, marginBottom: 3, lineHeight: 1.3, textDecoration: "underline", cursor: "pointer" }}>{serpTitle.slice(0, 60)}{serpTitle.length > 60 ? "…" : ""}</div>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
                  <input value={selectedProduct.price || ""} onChange={e => setSelectedProduct({ ...selectedProduct, price: e.target.value })} placeholder="Price" style={{ width: 120, background: "#1f2433", border: "1px solid #283044", color: "#f6f7fb", padding: "10px 12px", borderRadius: 10 }} />
                  <button
      ...
      case "templates":
        return (
          <SectionCard title="Templates" description="Prompt templates powered by /ai/prompts">
            ...
            <Divider />
            <div style={{ color: "#b8bed2", fontSize: 12 }}>Use prompt templates to accelerate optimization workflows.</div>
          </SectionCard>
        );
      case "categories":
        return (
      ...
      case "trash-recovery":
        return (
          <SectionCard title="Trash & Recovery" description="Placeholder for soft-delete flows.">
            <div style={{ color: "#b8bed2" }}>Soft-delete and restore actions can be wired to bulk-delete and rollback endpoints.</div>
          </SectionCard>
        );
      case "title-optimization":
        return (
          <SectionCard title="Title Optimization" description="Call /title-suggestions and apply best title." accent="#4f46e5">
            ...
            {selectedProduct?.title && <div style={{ color: "#b8bed2", fontSize: 13 }}>Preview: {selectedProduct.title}</div>}
          </SectionCard>
        );
      ...
      case "notifications":
        return (
          <SectionCard title="Notifications" description="Notification preferences." accent="#eab308">
            <div style={{ color: "#b8bed2" }}>Configure weekly digests, anomaly alerts, and webhook events.</div>
          </SectionCard>
        );
      case "integrations":
        return (
      ...
      default:
        return <div style={{ color: "#b8bed2" }}>Tab not implemented yet.</div>;
    }
  };

  return (
    <div style={{ background: "#05080f", minHeight: "100%", padding: 20, color: "#f6f7fb", fontFamily: "Inter, system-ui, sans-serif" }}>
      <header style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div>
          <div style={{ fontSize: 26, fontWeight: 800 }}>Product SEO Engine</div>
          <div style={{ color: "#b8bed2" }}>42-tab enterprise console · Backed by 200 endpoints</div>
        </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "240px 1fr", gap: 14 }}>
        <div style={{ background: "#1f2433", border: "1px solid #1f2433", borderRadius: 14, padding: 12, maxHeight: "82vh", overflow: "auto" }}>
          {categories.map(cat => (
                ...
                {cat.tabs.map(tab => (
                  <button
                    ...
                    style={{
                      ...
                      background: activeTab === tab.id ? cat.accent + "22" : "#1f2433",
                      color: "#f6f7fb",
                      cursor: "pointer"
                    }}
                  >
        ...
        <div style={{ background: "#1f2433", border: "1px solid #1f2433", borderRadius: 14, padding: 16, minHeight: "80vh" }}>
          {renderTab()}
        </div>
      </div>
      ...
      {toast && (
        <div style={{ position: "fixed", bottom: 20, left: 20, background: "#1f2433", color: "#f6f7fb", padding: "10px 14px", borderRadius: 10, border: "1px solid #283044" }}>
          {toast}
        </div>
      )}

      <style>{`
        .btn { background: #2563eb; border: 1px solid #4b5780; color: #fff; padding: 10px 14px; border-radius: 10px; cursor: pointer; font-weight: 600; }
        .btn-secondary { background: #1f2433; border: 1px solid #283044; color: #f6f7fb; padding: 10px 14px; border-radius: 10px; cursor: pointer; font-weight: 600; }
        .btn-tertiary { background: #1f2433; border: 1px dashed #4b5780; color: #f6f7fb; padding: 10px 14px; border-radius: 10px; cursor: pointer; font-weight: 600; }
        .text-area { width: 100%; background: #1f2433; border: 1px solid #283044; color: #f6f7fb; padding: 10px; border-radius: 10px; }
        .code-block { background: #05080f; border: 1px solid #283044; color: #f6f7fb; padding: 12px; border-radius: 10px; margin-top: 10px; white-space: pre-wrap; word-break: break-word; }
