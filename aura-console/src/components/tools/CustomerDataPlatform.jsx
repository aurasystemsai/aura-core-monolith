...
function FeedbackModal({ open, onClose, onSubmit }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#0008', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#0d0d11', borderRadius: 14, padding: 28, minWidth: 340, boxShadow: '0 8px 32px #000a' }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 12, color: '#f3f4f6' }}>Send Feedback</div>
        <textarea value={text} onChange={e => setText(e.target.value)} rows={4} style={{ width: '100%', background: '#2e3045', color: '#f3f4f6', border: '1px solid #525578', borderRadius: 8, padding: 10, fontSize: 14, boxSizing: 'border-box' }} placeholder='Share your feedback...' />
        <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
          <button onClick={() => { onSubmit(text); setText(''); }} disabled={!text} style={{ background: '#6366f1', color: '#0d0d11', border: 'none', borderRadius: 8, padding: '8px 20px', fontWeight: 700, cursor: 'pointer' }}>Submit</button>
          <button onClick={onClose} style={{ background: '#525578', color: '#f3f4f6', border: 'none', borderRadius: 8, padding: '8px 20px', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 16 }}>
        ...
        <button onClick={fetchAnalytics} style={{ background: "#eab308", color: "#0d0d11", border: "none", borderRadius: 8, padding: "7px 18px", fontWeight: 600, fontSize: 15, cursor: "pointer" }}>Analytics</button>
        ...
        <button onClick={handleExport} style={{ background: "#a3e635", color: "#0d0d11", border: "none", borderRadius: 8, padding: "7px 18px", fontWeight: 600, fontSize: 15, cursor: "pointer" }} disabled={exporting}>Export</button>
        ...
        <button onClick={runPlugin} style={{ background: "#e0e7ff", color: "#0d0d11", border: "none", borderRadius: 8, padding: "7px 18px", fontWeight: 600, fontSize: 15, cursor: "pointer" }}>Plugin</button>
      </div>
