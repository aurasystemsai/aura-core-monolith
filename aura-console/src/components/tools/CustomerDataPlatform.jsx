  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: '#0008', zIndex: 1000, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
      <div style={{ background: '#09090b', borderRadius: 14, padding: 28, minWidth: 340, boxShadow: '0 8px 32px #000a' }}>
        ...
        <textarea value={text} onChange={e => setText(e.target.value)} rows={4} style={{ width: '100%', background: '#27272a', color: '#f3f4f6', border: '1px solid #52525b', borderRadius: 8, padding: 10, fontSize: 14, boxSizing: 'border-box' }} placeholder='Share your feedback...' />
        <div style={{ display: 'flex', gap: 10, marginTop: 14 }}>
          <button onClick={() => { onSubmit(text); setText(''); }} disabled={!text} style={{ background: '#818cf8', color: '#09090b', border: 'none', borderRadius: 8, padding: '8px 20px', fontWeight: 700, cursor: 'pointer' }}>Submit</button>
          <button onClick={onClose} style={{ background: '#52525b', color: '#f3f4f6', border: 'none', borderRadius: 8, padding: '8px 20px', fontWeight: 600, cursor: 'pointer' }}>Cancel</button>
        </div>
      </div>
    </div>
  );
