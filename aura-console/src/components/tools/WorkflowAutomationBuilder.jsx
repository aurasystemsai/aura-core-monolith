...
export default function WorkflowAutomationBuilder() {
  ...
  return (
    <div style={{ background: "#0d0d11", color: "#f9fafb", padding: 24, borderRadius: 16, border: "1px solid #2e3045", boxShadow: "0 12px 48px #0007" }}>
      ...
      {issueHelp && (
        <div style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 10, padding: 10, display: "grid", gap: 8, marginBottom: 12 }}>
            ...
            {history.slice(0, 3).map((h, idx) => (
              <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", background: "#252638", border: "1px solid #2e3045", borderRadius: 10, padding: "8px 10px" }}>
                ...
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  <button aria-label={`Load snapshot ${idx + 1}`} onClick={() => restoreSnapshot(h)} style={{ background: "#2e3045", color: "#f9fafb", border: "1px solid #525578", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Load</button>
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, gap: 12, flexWrap: "wrap" }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <select value={env} onChange={e => setEnv(e.target.value)} style={{ background: "#252638", color: "#f9fafb", border: "1px solid #2e3045", borderRadius: 10, padding: "8px 12px", fontWeight: 700 }}>
            <option value="dev">Dev</option><option value="stage">Stage</option><option value="prod">Prod</option>
          </select>
          <input value={versionTag} onChange={e => setVersionTag(e.target.value)} placeholder="Version tag" style={{ background: "#252638", color: "#f9fafb", border: "1px solid #2e3045", borderRadius: 10, padding: "8px 12px", minWidth: 120 }} />
          <label style={{ display: "flex", alignItems: "center", gap: 6, background: "#252638", border: "1px solid #2e3045", borderRadius: 10, padding: "6px 10px", fontWeight: 700 }}>
          ...
          <input value={approverEmail} onChange={e => setApproverEmail(e.target.value)} placeholder="Approver email" style={{ background: "#252638", color: "#f9fafb", border: "1px solid #2e3045", borderRadius: 10, padding: "8px 12px", minWidth: 180 }} />
        </div>
      </div>
      ...
      {(env === "prod" || riskLevel === "high") && (
        <div style={{ marginBottom: 10, background: "#2e3045", border: "1px solid #525578", borderRadius: 10, padding: 10, display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
          <div style={{ color: "#fbbf24", fontWeight: 700 }}>Prod / high-risk confirmation</div>
          <input value={confirmationNote} onChange={e => setConfirmationNote(e.target.value)} placeholder="Add a confirmation note" style={{ flex: 1, minWidth: 220, background: "#252638", color: "#f9fafb", border: "1px solid #2e3045", borderRadius: 10, padding: "8px 12px" }} />
        </div>
      )}

      <div style={{ position: "sticky", top: 0, zIndex: 2, background: "#0d0d11", paddingBottom: 6, marginBottom: 12 }}>
        <div style={{ display: "flex", gap: 10, alignItems: "center", marginBottom: 6, flexWrap: "wrap" }}>
          <div style={{ background: "#252638", border: "1px solid #2e3045", padding: "8px 12px", borderRadius: 10, fontWeight: 700, display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <button onClick={handleManualSave} style={{ background: "#0ea5e9", color: "#252638", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 800, cursor: "pointer" }}>Save draft</button>
          <input value={newVersionLabel} onChange={e => setNewVersionLabel(e.target.value)} placeholder="Version name" style={{ background: "#252638", color: "#f9fafb", border: "1px solid #2e3045", borderRadius: 8, padding: "8px 12px", minWidth: 140 }} />
          <button onClick={() => { if (newVersionLabel) setVersionTag(newVersionLabel); handleCreateVersion(); }} style={{ background: "#a855f7", color: "#252638", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 800, cursor: "pointer" }}>Snapshot version</button>
          <button onClick={() => runPreflight({ steps, env, versionTag, approvalRequired, approverEmail, confirmationNote })} style={{ background: "#f59e0b", color: "#252638", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 800, cursor: "pointer" }}>Preflight</button>
          <button onClick={runDryRun} style={{ background: "#22c55e", color: "#252638", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 800, cursor: "pointer" }}>Dry-run (dev)</button>
          <button onClick={() => handleApplyPreset("checkout-winback")} style={{ background: "#0ea5e9", color: "#252638", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>Shopify winback preset</button>
          <button onClick={rollbackToLastBuild} disabled={!lastBuiltSnapshot || isViewer} style={{ background: "#ef4444", color: "#252638", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: (!lastBuiltSnapshot || isViewer) ? "not-allowed" : "pointer", opacity: (!lastBuiltSnapshot || isViewer) ? 0.6 : 1 }}>Rollback to last build</button>
          {preflightStatus && (
          ...
          <button onClick={handleExportJson} style={{ background: "#14b8a6", color: "#252638", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 800, cursor: "pointer" }}>Export JSON</button>
          <button onClick={handleCopySummary} style={{ background: "#8b5cf6", color: "#252638", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 800, cursor: "pointer" }}>Copy summary</button>
          <button onClick={handleSaveTemplate} style={{ background: "#22d3ee", color: "#252638", border: "none", borderRadius: 8, padding: "8px 14px", fontWeight: 800, cursor: "pointer" }}>Save as template</button>
          <button aria-label="Build workflow" onClick={() => handleBuild()} disabled={loading || devSandbox || isViewer} style={{ background: devSandbox ? "#2e3045" : "#22c55e", color: devSandbox ? "#a8adc4" : "#252638", border: "none", borderRadius: 10, padding: "10px 18px", fontWeight: 800, fontSize: 15, cursor: (loading || devSandbox || isViewer) ? "not-allowed" : "pointer", opacity: (loading || devSandbox || isViewer) ? 0.65 : 1 }}>{devSandbox ? "Sandbox (set Stage)" : loading ? "Building..." : "Build"}</button>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <span style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 999, padding: "6px 10px", color: preflightIssues.length ? "#f97316" : "#22c55e", fontWeight: 700 }}>Trigger: {steps.some(s => s.type === "trigger") ? "OK" : "Missing"}</span>
          <span style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 999, padding: "6px 10px", color: approvalRequired && !approverEmail ? "#f97316" : "#22c55e", fontWeight: 700 }}>Approvals: {approvalRequired ? (approverEmail ? "Ready" : "Email needed") : "Off"}</span>
          ...
          <span style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 999, padding: "6px 10px", color: stepSummary.actions >= 4 ? "#f97316" : "#22c55e", fontWeight: 700 }}>Perf guardrail: {stepSummary.actions >= 4 ? "tighten" : "OK"}</span>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 6 }}>
          ...
          <button onClick={attachPreflightForReviewer} style={{ background: "#8b5cf6", color: "#252638", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>Attach preflight</button>
      ...
      <div style={{ display: "flex", gap: 12, flexWrap: "wrap", marginBottom: 12 }}>
        <div style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 10, padding: 10, minWidth: 220 }}>
          ...
          <select value={presetTag} onChange={e => setPresetTag(e.target.value)} style={{ width: "100%", background: "#252638", color: "#f9fafb", border: "1px solid #0d0d11", borderRadius: 8, padding: 8, marginBottom: 6 }}>
            <option value="">All tags</option>
            {Array.from(new Set(PRESETS.flatMap(p => p.tags))).map(tag => <option key={tag} value={tag}>{tag}</option>)}
          </select>
          <select value={selectedPreset} onChange={e => handleApplyPreset(e.target.value)} style={{ width: "100%", background: "#252638", color: "#f9fafb", border: "1px solid #0d0d11", borderRadius: 8, padding: 8, marginBottom: 8 }}>
          ...
          <div style={{ marginTop: 6, padding: 10, borderRadius: 8, background: "#252638", border: "1px solid #2e3045", color: "#f9fafb", fontSize: 12 }}>
          ...
          <button onClick={handleClearDraft} style={{ marginTop: 8, background: "#ef4444", color: "#252638", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: "pointer", width: "100%" }}>Reset draft</button>
        </div>
        <div style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 10, padding: 10, flex: 1, minWidth: 260 }}>
          ...
          <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap" }}>
            <select value={selectedPayloadPreset} onChange={e => setSelectedPayloadPreset(e.target.value)} style={{ background: "#252638", color: "#f9fafb", border: "1px solid #0d0d11", borderRadius: 8, padding: 8 }}>
              {PAYLOAD_PRESETS.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}
            </select>
            <button onClick={handleTestInDev} style={{ background: "#22c55e", color: "#252638", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>{testStatus === "running" ? "Testing..." : "Test in dev"}</button>
          </div>
        </div>
        <div style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 10, padding: 10, minWidth: 220 }}>
          ...
          {versions.slice(0, 4).map(v => (
            <div key={v.id} style={{ padding: 8, borderRadius: 8, border: "1px solid #0d0d11", marginBottom: 6, background: "#252638" }}>
              <div style={{ fontWeight: 700 }}>{v.label}</div>
              <div style={{ color: "#a8adc4", fontSize: 12 }}>Saved {formatTime(v.createdAt)}</div>
              <button onClick={() => handleRestoreVersion(v)} style={{ marginTop: 6, background: "#22c55e", color: "#252638", border: "none", borderRadius: 6, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Restore</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 18, marginBottom: 18, flexWrap: "wrap" }}>
        <div style={{ minWidth: 260, maxWidth: 320 }}>
          ...
          <ul style={{ listStyle: "none", padding: 0 }}>
            {steps.map(step => (
              <li key={step.id} style={{ marginBottom: 8, background: selectedStep === step.id ? "#0d0d11" : "#252638", borderRadius: 8, padding: 8, cursor: "pointer", border: selectedStep === step.id ? "2px solid #6366f1" : "1px solid #0d0d11" }} onClick={() => setSelectedStep(step.id)}>
          ...
          <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 8 }}>
            <button onClick={handleAddStep} style={{ background: "#22c55e", color: "#252638", border: "none", borderRadius: 8, padding: "6px 14px", fontWeight: 800, cursor: "pointer" }}>+ Add Action</button>
            <button onClick={() => { const nextId = Math.max(...steps.map(s => s.id)) + 1; setSteps([...steps, { id: nextId, name: `Trigger ${nextId}`, type: "trigger", config: "" }]); setSelectedStep(nextId); }} style={{ background: "#38bdf8", color: "#252638", border: "none", borderRadius: 8, padding: "6px 14px", fontWeight: 800, cursor: "pointer" }}>+ Add Trigger</button>
            <button onClick={handleCloneStep} style={{ background: "#a855f7", color: "#252638", border: "none", borderRadius: 8, padding: "6px 14px", fontWeight: 800, cursor: "pointer" }}>Clone step</button>
          </div>
          <div style={{ marginTop: 12, background: "#252638", border: "1px solid #2e3045", borderRadius: 10, padding: 10 }}>
            <div style={{ fontWeight: 700, marginBottom: 6 }}>Action library</div>
            <input value={actionSearch} onChange={e => setActionSearch(e.target.value)} placeholder="Search actions/triggers" style={{ width: "100%", background: "#252638", color: "#f9fafb", border: "1px solid #0d0d11", borderRadius: 8, padding: 8, marginBottom: 6 }} />
            <select value={actionTag} onChange={e => setActionTag(e.target.value)} style={{ width: "100%", background: "#252638", color: "#f9fafb", border: "1px solid #0d0d11", borderRadius: 8, padding: 8, marginBottom: 8 }}>
              <option value="">All tags</option>
            ...
            <div style={{ maxHeight: 180, overflowY: "auto", display: "grid", gap: 6 }}>
              {ACTION_LIBRARY.filter(a => (!actionTag || a.tags.includes(actionTag)) && (!actionSearch || a.name.toLowerCase().includes(actionSearch.toLowerCase()) || a.tags.some(t => t.includes(actionSearch.toLowerCase())))).map(item => (
                <div key={item.id} style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 8, padding: 8 }}>
                  ...
                  <div style={{ display: "flex", gap: 4, flexWrap: "wrap", marginTop: 4 }}>
                    {item.tags.map(t => <span key={t} style={{ background: "#252638", color: "#e9ebf5", borderRadius: 999, padding: "2px 8px", fontSize: 11 }}>{t}</span>)}
                  </div>
                  <button onClick={() => { const nextId = Math.max(...steps.map(s => s.id)) + 1; setSteps([...steps, { id: nextId, name: item.name, type: item.type, config: item.config }]); setSelectedStep(nextId); }} style={{ marginTop: 6, background: "#22c55e", color: "#252638", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: "pointer", width: "100%" }}>Add</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ flex: 1, minWidth: 260 }}>
          ...
          {steps.map(step => step.id === selectedStep && (
            <div key={step.id}>
              <input value={step.name} onChange={e => setSteps(steps.map(s => s.id === step.id ? { ...s, name: e.target.value } : s))} style={{ fontWeight: 700, fontSize: 16, marginBottom: 8, width: "100%", borderRadius: 6, border: "1px solid #0d0d11", padding: 8, background: "#252638", color: "#f9fafb" }} />
              <select value={step.type} onChange={e => setSteps(steps.map(s => s.id === step.id ? { ...s, type: e.target.value } : s))} style={{ marginBottom: 8, width: "100%", borderRadius: 6, border: "1px solid #0d0d11", padding: 8, background: "#252638", color: "#f9fafb" }}>
              ...
              <div style={{ display: "flex", gap: 8, marginTop: 6, flexWrap: "wrap" }}>
                <button onClick={() => handleStepChange(step.id, step.type === "trigger" ? "Auto-detected event with debounce" : "Send Slack + email with retry") } style={{ background: "#0ea5e9", color: "#252638", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Suggest config</button>
                <button onClick={() => setStepStatuses(prev => ({ ...prev, [step.id]: "pending" }))} style={{ background: "#f59e0b", color: "#252638", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Mark pending</button>
              </div>
            </div>
          ))}
        </div>
      ...
      {result && (
        <div style={{ marginTop: 16, background: "#252638", border: "1px solid #2e3045", borderRadius: 12, padding: 12 }}>
      ...
      {diffSummary && (
        <div style={{ marginTop: 18, background: "#252638", border: "1px solid #2e3045", borderRadius: 12, padding: 12 }}>
      ...
      <div style={{ marginTop: 18, background: "#252638", border: "1px solid #2e3045", borderRadius: 12, padding: 12, display: "flex", gap: 12, flexWrap: "wrap" }}>
