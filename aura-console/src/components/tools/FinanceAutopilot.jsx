  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        <button onClick={handleAddTask} disabled={!result} style={{ background: "#4f46e5", color: "#0a0b0f", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>Save Task</button>
      ...
      <div style={{ marginTop: 24, background: darkMode ? "#1f2433" : "#fff", borderRadius: 12, padding: 18, border: "1px solid #283044" }}>
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 10, color: darkMode ? "#e2e8f0" : "#0a0b0f" }}>Tasks</div>
        {tasks.map(t => (
          <div key={t.id} style={{ background: darkMode ? "#0a0b0f" : "#f8fafc", borderRadius: 8, padding: "10px 14px", marginBottom: 8, border: "1px solid #283044" }}>
            <span style={{ fontWeight: 600, color: darkMode ? "#e2e8f0" : "#0a0b0f" }}>{t.content ? t.content.slice(0, 80) + (t.content.length > 80 ? "…" : "") : `Task #${t.id}`}</span>
          </div>
        ))}
      </div>
