...
export default function ContentHealthAuditor() {
  ...
  const handleRun = async () => {
    ...
    try {
      ...
      if (bulkUpload) {
        ...
        if (input) body.append("content", input);
        headers = {};
      } else {
        body = JSON.stringify({ content: input });
        headers = { "Content-Type": "application/json" };
      }
      const res = await fetch("/api/content-health-auditor/ai/audit", {
        method: "POST",
        headers,
        body
      });
      ...
      setNotification("Audit complete.");
      setHistory(prev => [{
        content: input,
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "content-health-audit.txt";
    a.click();
  ...
  const handleFeedback = async () => {
    ...
    try {
      await fetch("/api/content-health-auditor/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  const onboardingContent = (
    <div style={{ padding: 24, background: darkMode ? "#23263a" : "#f1f5f9", borderRadius: 12, marginBottom: 18 }}>
      <h3 style={{ fontWeight: 700, fontSize: 22 }}>Welcome to Content Health Auditor</h3>
      <ul style={{ margin: "16px 0 0 18px", color: darkMode ? "#a3e635" : "#334155", fontSize: 16 }}>
        <li>Paste content or upload a CSV/Excel file for bulk audit</li>
        <li>Get AI-powered quality, compliance, and health analytics</li>
        <li>Export results and review audit history</li>
        <li>Collaborate and share with your team</li>
        <li>Integrate with CMS, Shopify, and compliance APIs</li>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <h2 style={{ fontWeight: 800, fontSize: 32, margin: 0 }}>Content Health Auditor</h2>
      ...
      <div style={{ marginBottom: 10, color: darkMode ? "#a3e635" : "#0ea5e9", fontWeight: 600 }}>
        <span role="img" aria-label="audit">🩺</span> AI-powered content audit, bulk upload, analytics, and collaboration.
      </div>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        <button onClick={handleRun} disabled={loading || (!input && !bulkUpload)} style={{ background: "#7fffd4", color: "#23263a", border: "none", borderRadius: 8, padding: "12px 32px", fontWeight: 700, fontSize: 17, cursor: loading ? "not-allowed" : "pointer", boxShadow: "0 2px 12px #22d3ee55" }}>{loading ? "Auditing..." : "Run Audit"}</button>
      ...
      <div style={{ marginTop: 32, fontSize: 13, color: darkMode ? "#a3e635" : "#64748b", textAlign: "center" }}>
        <span>Integrations: <b>CMS</b>, <b>Shopify</b>, <b>Compliance APIs</b> | Accessible, secure, and best-in-class SaaS features.</span>
      </div>
    </div>
  );
}
