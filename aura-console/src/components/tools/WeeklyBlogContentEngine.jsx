...
export default function WeeklyBlogContentEngine() {
  ...
  const handleRun = async () => {
    ...
    setResponse("");
    try {
      const res = await apiFetch("/api/weekly-blog-content-engine/ai/generate", {
        method: "POST",
      ...
      // Save to history
      await apiFetch("/api/weekly-blog-content-engine/history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ topic: input, content: data.blogContent })
      });
      fetchHistory();
    } catch (err) {
  ...
  const handleImport = e => {
    ...
    reader.onload = async evt => {
      try {
        const res = await apiFetch("/api/weekly-blog-content-engine/import", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleFeedback = async () => {
    ...
    setError("");
    try {
      await apiFetch("/api/weekly-blog-content-engine/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", background: "#23263a", color: "#f3f4f6", borderRadius: 18, boxShadow: "0 2px 24px #0002", padding: 36, fontFamily: 'Inter, sans-serif' }}>
      <h2 style={{ fontWeight: 800, fontSize: 32, marginBottom: 8 }}>Weekly Blog Content Engine</h2>
      <div style={{ marginBottom: 10, color: "#0ea5e9", fontWeight: 600 }}>
        <span role="img" aria-label="sparkles">✨</span> Generate, manage, and analyze weekly blog content with AI and analytics.
      </div>
      <textarea
        value={input}
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        {exported && <a href={exported} download="weekly-blog-history.json" style={{ marginLeft: 8, color: "#0ea5e9", fontWeight: 600 }}>Download</a>}
      </div>
        ...
        <ul style={{ paddingLeft: 18 }}>
          {history.map(h => (
            <li key={h.id} style={{ marginBottom: 10 }}>
              <div><b>ID:</b> {h.id}</div>
              <div><b>Topic:</b> {h.topic}</div>
              <div><b>Content:</b> {h.content || JSON.stringify(h)}</div>
            </li>
          ))}
        </ul>
      </div>
