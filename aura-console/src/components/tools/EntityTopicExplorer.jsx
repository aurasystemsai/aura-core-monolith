  ...
  return (
    <div style={{ background: "#18181b", borderRadius: 18, boxShadow: "0 2px 24px #0008", padding: 36, fontFamily: 'Inter, sans-serif', color: '#fafafa' }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        <button onClick={fetchEntities} style={{ background: "#4f46e5", color: "#fff", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>Extract Entities</button>
      ...
      <div style={{ display: "flex", gap: 18, marginBottom: 18 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <ul style={{ paddingLeft: 18 }}>
            {topics.map((t, idx) => (
              <li key={t.id || idx} style={{ marginBottom: 8, background: "#18181b", borderRadius: 8, padding: 8, color: '#4f46e5' }}>{t.name}</li>
            ))}
          </ul>
        </div>
        <div style={{ flex: 1 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginTop: 20, marginBottom: 16, flexWrap: "wrap" }}>
        <div style={{ background: "#27272a", borderRadius: 10, padding: "12px 20px", border: "1px solid #27272a" }}>
          <div style={{ fontSize: 11, color: "#71717a", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>Entities</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: "#4f46e5", marginTop: 2 }}>{entities.length}</div>
        </div>
        <div style={{ background: "#27272a", borderRadius: 10, padding: "12px 20px", border: "1px solid #27272a" }}>
          <div style={{ fontSize: 11, color: "#71717a", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>Topics</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: "#0ea5e9", marginTop: 2 }}>{topics.length}</div>
        </div>
        <div style={{ background: "#27272a", borderRadius: 10, padding: "12px 20px", border: "1px solid #27272a" }}>
          <div style={{ fontSize: 11, color: "#71717a", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>Analysis</div>
          <div style={{ fontSize: 26, fontWeight: 800, color: "#4f46e5", marginTop: 2 }}>{analysis.length}</div>
        </div>
      </div>
      <div style={{ fontSize: 13, color: "#71717a", textAlign: "center", paddingBottom: 8 }}>Questions? <a href="mailto:support@aura-core.ai" style={{ color: "#0ea5e9", textDecoration: "underline" }}>Contact Support</a></div>
    </div>
  );
}
