  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        <button onClick={handleAddCreative} disabled={!result} style={{ background: "#818cf8", color: "#09090b", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>Save Creative</button>
      ...
      <div style={{ marginTop: 24, background: darkMode ? "#18181b" : "#fff", borderRadius: 12, padding: 18, border: "1px solid #27272a" }}>
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 10, color: darkMode ? "#e2e8f0" : "#09090b" }}>Creatives</div>
        {creatives.map(c => (
          <div key={c.id} style={{ background: darkMode ? "#09090b" : "#f8fafc", borderRadius: 8, padding: "10px 14px", marginBottom: 8, border: "1px solid #27272a" }}>
            <span style={{ fontWeight: 600, color: darkMode ? "#e2e8f0" : "#09090b" }}>{c.content ? c.content.slice(0, 80) + (c.content.length > 80 ? "…" : "") : `Creative #${c.id}`}</span>
          </div>
        ))}
      </div>
