  ...
  return (
    ...
    >
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        <button onClick={handleGenerate} disabled={loading || !brief} style={{ background: "#a3e635", color: "#0d0d11", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>{loading ? "Generating..." : "AI Generate"}</button>
        <button onClick={handleAddCreative} disabled={!result} style={{ background: "#6366f1", color: "#0d0d11", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 16, cursor: "pointer" }}>Save Creative</button>
      ...
      <div style={{ marginTop: 24, background: darkMode ? "#252638" : "#fff", borderRadius: 12, padding: 18, border: "1px solid #2e3045" }}>
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 10, color: darkMode ? "#e2e8f0" : "#0d0d11" }}>Creatives</div>
        {creatives.map(c => (
          <div key={c.id} style={{ background: darkMode ? "#0d0d11" : "#f8fafc", borderRadius: 8, padding: "10px 14px", marginBottom: 8, border: "1px solid #2e3045" }}>
            <span style={{ fontWeight: 600, color: darkMode ? "#e2e8f0" : "#0d0d11" }}>{c.content ? c.content.slice(0, 80) + (c.content.length > 80 ? "…" : "") : `Creative #${c.id}`}</span>
          </div>
        ))}
      </div>
