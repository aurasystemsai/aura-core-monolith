...
export default function InboxReplyAssistant() {
  ...
  const handleRun = async () => {
    ...
    setNotification("");
    try {
      const res = await fetch("/api/inbox-reply-assistant/ai/reply", {
        method: "POST",
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "inbox-reply-assistant-reply.txt";
    a.click();
  ...
  const handleFeedback = async () => {
    ...
    try {
      await fetch("/api/inbox-reply-assistant/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2 style={{ fontWeight: 700, fontSize: 28, marginBottom: 12 }}>Inbox Reply Assistant</h2>
        <button
      ...
      <p style={{ color: darkMode ? "#e0e7ff" : "#444", marginBottom: 18 }}>
        Enter an inbox reply message below. The AI will generate a suggested reply. <span style={{ fontWeight: 600 }}>All features are fully accessible.</span>
      </p>
      <textarea
        value={input}
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <input type="text" value={education} onChange={e => setEducation(e.target.value)} placeholder="Reply topic or question" style={{ fontSize: 15, padding: 8, borderRadius: 8, border: darkMode ? "1px solid #555" : "1px solid #ccc" }} />
      </div>
      <button
        onClick={handleRun}
