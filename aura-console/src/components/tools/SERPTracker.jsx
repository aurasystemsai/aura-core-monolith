...
export default function SERPTracker() {
  ...
  const handleTrack = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/serp-tracker/track", {
        method: "POST",
  ...
  const onboardingContent = (
    <div style={{ padding: 24, background: darkMode ? "#23263a" : "#f1f5f9", borderRadius: 12, marginBottom: 18 }}>
      <h3 style={{ fontWeight: 700, fontSize: 22 }}>Welcome to SERP Tracker</h3>
      <ul style={{ margin: "16px 0 0 18px", color: darkMode ? "#a3e635" : "#334155", fontSize: 16 }}>
        <li>Track keyword rankings, volatility, and featured snippets</li>
        <li>Local/geo tracking and competitor monitoring</li>
        <li>Export, share, and review tracking history</li>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <h2 style={{ fontWeight: 800, fontSize: 32, margin: 0 }}>SERP Tracker</h2>
