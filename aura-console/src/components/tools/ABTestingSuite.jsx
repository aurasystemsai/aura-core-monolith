  ...
  return (
    <div style={{ maxWidth: 1100, margin: '0 auto', padding: 24 }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <input value={testName} onChange={e => setTestName(e.target.value)} placeholder="Test name" style={{ fontSize: 16, padding: 8, borderRadius: 8, border: '1px solid #ccc', width: 220, marginRight: 12 }} aria-label="Test name" />
        <div style={{ display: 'flex', gap: 18, marginBottom: 18 }}>
          {variants.map((v, i) => (
            <div key={i} style={{ flex: 1 }}>
              <input value={v.name} onChange={e => setVariants(variants.map((vv, idx) => idx === i ? { ...vv, name: e.target.value } : vv))} placeholder={`Variant ${String.fromCharCode(65 + i)}`} style={{ fontSize: 16, padding: 8, borderRadius: 8, border: '1px solid #ccc', width: 120, marginBottom: 8 }} aria-label={`Variant ${String.fromCharCode(65 + i)}`} />
              <textarea value={v.content} onChange={e => setVariants(variants.map((vv, idx) => idx === i ? { ...vv, content: e.target.value } : vv))} rows={2} style={{ width: '100%', fontSize: 16, padding: 8, borderRadius: 8, border: '1px solid #ccc', marginBottom: 8 }} placeholder="Variant content" aria-label="Variant content" />
            </div>
          ))}
          <button onClick={() => setVariants([...variants, { name: `Variant ${String.fromCharCode(65 + variants.length)}`, content: "" }])} style={{ background: '#0ea5e9', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 18px', fontWeight: 600, fontSize: 15, cursor: 'pointer', height: 40 }}>Add Variant</button>
        </div>
      </div>
      {/* Import/Export */}
      <div style={{ marginBottom: 24 }}>
        <input type="file" accept="application/json" ref={fileInputRef} style={{ display: 'none' }} onChange={handleImport} />
        <button onClick={() => fileInputRef.current.click()} style={{ background: '#0ea5e9', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 18px', fontWeight: 600, fontSize: 15, cursor: 'pointer', marginRight: 12 }}>Import Test</button>
        <button onClick={handleExport} style={{ background: '#22c55e', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 18px', fontWeight: 600, fontSize: 15, cursor: 'pointer' }}>Export Test</button>
