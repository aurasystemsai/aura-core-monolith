...
export default function MultiChannelOptimizer() {
  ...
  const [channels, setChannels] = useState({ web: true, mobile: false, email: false, sms: false, social: false, api: false });
  ...
  const handleRun = async () => {
    ...
    setNotification("");
    try {
      const res = await fetch("/api/multi-channel-optimizer/ai/optimize", {
        method: "POST",
      ...
      setResponse(data.optimization || "No optimization generated");
      setAnalytics(data.analytics || null);
      setNotification("Optimization complete and saved.");
      setHistory(prev => [{
        campaign: input,
        bulkUpload,
        channels,
        ...
        compliance,
        education,
        optimization: data.optimization || "No optimization generated",
        analytics: data.analytics || null
      }, ...prev].slice(0, 10));
      setReportUrl(window.location.origin + window.location.pathname + "?optimization=" + encodeURIComponent(data.optimization || ""));
    } catch (err) {
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "multi-channel-optimization.txt";
    a.click();
  ...
  const handleShare = () => {
    ...
    setTimeout(() => setNotification("Optimization complete and saved."), 2000);
  };
  ...
  const handleFeedback = async () => {
    ...
    try {
      await fetch("/api/multi-channel-optimizer/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2 style={{ fontWeight: 700, fontSize: 28, marginBottom: 12 }}>Multi-Channel Optimizer</h2>
        <button
          aria-label="Toggle dark mode"
          onClick={() => setDarkMode(d => !d)}
          style={{ background: darkMode ? "#f3f4f6" : "#09090b", color: darkMode ? "#09090b" : "#f3f4f6", border: "none", borderRadius: 8, padding: "8px 16px", fontWeight: 600, fontSize: 15, cursor: "pointer" }}
        >
          {darkMode ? "Light" : "Dark"}
        </button>
      </div>
      <p style={{ color: darkMode ? "#e0e7ff" : "#444", marginBottom: 18 }}>
        Enter a marketing campaign or channel mix below. The AI will optimize for best results. <span style={{ fontWeight: 600 }}>All features are fully accessible.</span>
      </p>
      <textarea
        value={input}
        onChange={e => setInput(e.target.value)}
        rows={5}
        style={{ width: "100%", fontSize: 16, padding: 12, borderRadius: 8, border: darkMode ? "1px solid #555" : "1px solid #ccc", marginBottom: 18, background: darkMode ? "#09090b" : "#fff", color: darkMode ? "#f3f4f6" : "#09090b" }}
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <input type="text" value={education} onChange={e => setEducation(e.target.value)} placeholder="Campaign topic or question" style={{ fontSize: 15, padding: 8, borderRadius: 8, border: darkMode ? "1px solid #555" : "1px solid #ccc" }} />
      </div>
      <button
        onClick={handleRun}
        disabled={loading || !input}
        style={{ background: "#818cf8", color: "#09090b", border: "none", borderRadius: 8, padding: "12px 32px", fontWeight: 700, fontSize: 17, cursor: "pointer", boxShadow: "0 2px 12px #22d3ee55", marginRight: 12 }}
          ...
          <ul style={{ paddingLeft: 18 }}>
            {history.map((h, i) => (
              <li key={i} style={{ marginBottom: 10 }}>
                <div><b>Campaign:</b> {h.campaign.slice(0, 60)}{h.campaign.length > 60 ? "..." : ""}</div>
                ...
                <div><b>Optimization:</b> {h.optimization.slice(0, 60)}{h.optimization.length > 60 ? "..." : ""}</div>
              </li>
            ))}
          </ul>
        </div>
