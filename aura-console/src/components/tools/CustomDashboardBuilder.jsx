  ...
  return (
    <div style={{ background: "#18181b", borderRadius: 18, boxShadow: "0 2px 24px #0008", padding: 36, fontFamily: 'Inter, sans-serif', color: '#e5e7eb' }}>
      ...
      <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
        <div style={{ color: "#9ca3af", fontSize: 13 }}>Unsaved changes: {unsaved ? "Yes" : "No"}</div>
        {draftSavedAt && <div style={{ color: "#9ca3af", fontSize: 12 }}>Draft saved: {new Date(draftSavedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>}
        <div style={{ color: "#6ee7b7", fontSize: 12 }}>Autosave every 30s.</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => setRole(role === "editor" ? "viewer" : "editor")} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>Role: {role}</button>
          <button onClick={() => setEditLocked(!editLocked)} style={{ background: editLocked ? "#7f1d1d" : "#1f2937", color: editLocked ? "#fecdd3" : "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>{editLocked ? "Unlock" : "Lock edits"}</button>
          <button onClick={() => {
            const raw = localStorage.getItem("cdb-draft");
            if (raw) {
              const parsed = JSON.parse(raw);
          ...
          }} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>Restore draft</button>
        </div>
        {editLocked && <div style={{ background: "#2f1114", border: "1px solid #4b5563", borderRadius: 10, padding: 10, color: "#fecdd3" }}>View-only: edits and publish are disabled.</div>}
      </div>
      <div style={{ display: "grid", gap: 12, marginBottom: 18 }}>
        <div style={{ background: "#0b1221", border: "1px solid #1f2937", borderRadius: 12, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div>
            <div style={{ fontWeight: 800, color: "#7fffd4" }}>Quality & Guardrails</div>
            <div style={{ color: "#9ca3af", fontSize: 13 }}>Complexity score {complexityScore} · Risk: {riskLevel} · {freshnessLabel} · Perf: {perfSummary()}</div>
            {lintFindings.length > 0 && (
              <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
                <span style={{ color: "#9ca3af", fontSize: 12 }}>Lint findings:</span>
                ...
                <button onClick={() => setLintAcknowledged(true)} disabled={lintFindings.length === 0} style={{ background: lintAcknowledged ? "#14532d" : "#1f2937", color: lintAcknowledged ? "#bbf7d0" : "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "4px 8px", fontWeight: 700, cursor: lintFindings.length === 0 ? "not-allowed" : "pointer", opacity: lintFindings.length === 0 ? 0.6 : 1, fontSize: 12 }}>{lintAcknowledged ? "Acknowledged" : "Acknowledge"}</button>
              </div>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            <button onClick={() => setMaintenanceFreeze(m => !m)} style={{ background: maintenanceFreeze ? "#7f1d1d" : "#1f2937", color: maintenanceFreeze ? "#fecdd3" : "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>{maintenanceFreeze ? "Freeze: ON" : "Freeze: OFF"}</button>
        ...
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            <div style={{ color: "#cbd5e1", fontWeight: 700, fontSize: 13 }}>Guardrail recap</div>
            <div style={{ color: "#9ca3af", fontSize: 12 }}>Freeze, review, lint, PII, owners</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ background: maintenanceFreeze ? "#2f1114" : "#0b1221", color: maintenanceFreeze ? "#fecdd3" : "#bbf7d0", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Freeze: {maintenanceFreeze ? "Active" : "Off"}</span>
            <span style={{ background: reviewStatus !== "approved" ? "#2f2311" : "#0b1221", color: reviewStatus !== "approved" ? "#fcd34d" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Review: {reviewStatus}</span>
            <span style={{ background: highLintCount > 0 ? "#2f1114" : "#0b1221", color: highLintCount > 0 ? "#fecdd3" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>High lint: {highLintCount}</span>
            <span style={{ background: missingPiiCount > 0 ? "#2f2311" : "#0b1221", color: missingPiiCount > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>PII unchecked: {missingPiiCount}</span>
            <span style={{ background: missingOwnersCount > 0 ? "#2f2311" : "#0b1221", color: missingOwnersCount > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Owners missing: {missingOwnersCount}</span>
            <span style={{ background: targetCoverage === 100 ? "#0f172a" : "#2f2311", color: targetCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Target coverage: {targetCoverage}%</span>
            <span style={{ background: ownerCoverage === 100 ? "#0f172a" : "#2f2311", color: ownerCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Owner coverage: {ownerCoverage}%</span>
            <span style={{ background: piiCoverage === 100 ? "#0f172a" : "#2f2311", color: piiCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>PII coverage: {piiCoverage}%</span>
            <span style={{ background: watchlistMissingTargets.length > 0 ? "#2f2311" : "#0f172a", color: watchlistMissingTargets.length > 0 ? "#fcd34d" : "#bbf7d0", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Watchlist gaps: {watchlistMissingTargets.length}</span>
            <button onClick={copyGuardrailSummary} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy summary</button>
            <button onClick={() => setShowGuardrailText(s => !s)} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>{showGuardrailText ? "Hide text" : "Show text"}</button>
          </div>
          {showGuardrailText && (
            <div style={{ display: "grid", gap: 8 }}>
              <textarea readOnly value={buildGuardrailSummary()} style={{ width: "100%", background: "#0b1221", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 10, padding: 10, fontSize: 12 }} rows={2} aria-label="Guardrail summary text" />
              <textarea readOnly value={buildSlaSnapshot()} style={{ width: "100%", background: "#0b1221", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 10, padding: 10, fontSize: 12 }} rows={2} aria-label="SLA snapshot text" />
              {watchlistMissingTargets.length > 0 && (
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  <div style={{ color: "#9ca3af", fontSize: 12 }}>Watchlist gaps: {watchlistMissingTargets.join(", ")}</div>
                  <button onClick={copyWatchlistGaps} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy watchlist gaps</button>
        ...
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ color: "#9ca3af", fontSize: 12 }}>Targets and last lint</div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ background: missingTargetsCount > 0 ? "#2f2311" : "#0b1221", color: missingTargetsCount > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Missing targets: {missingTargetsCount}</span>
            <span style={{ color: "#9ca3af", fontSize: 12 }}>Watchlist missing: {dashboard.filter(w => watchlist.includes(w.id) && !targets[w.id]).length}</span>
            ...
            <div style={{ color: "#9ca3af", fontSize: 12 }}>{lintRunAt ? `Lint at ${new Date(lintRunAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Lint not run yet"}</div>
            <button onClick={runLint} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Run lint now</button>
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            <div style={{ color: "#9ca3af", fontSize: 12 }}>Target coverage: {targetCoverage}%</div>
            <div style={{ height: 8, background: "#0b1221", borderRadius: 6, overflow: "hidden", border: "1px solid #1f2937" }}>
        ...
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ color: "#9ca3af", fontSize: 12 }}>Stale & unbound check</div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ background: staleSources.length > 0 ? "#2f1114" : "#0b1221", color: staleSources.length > 0 ? "#fecdd3" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Stale: {staleSources.length}</span>
            <span style={{ background: unboundSources.length > 0 ? "#2f2311" : "#0b1221", color: unboundSources.length > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Unbound: {unboundSources.length}</span>
            <button onClick={autoRecheckSources} disabled={dataSources.length === 0} style={{ background: "#0b1221", color: dataSources.length === 0 ? "#9ca3af" : "#7dd3fc", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: dataSources.length === 0 ? "not-allowed" : "pointer", opacity: dataSources.length === 0 ? 0.6 : 1, fontSize: 12 }}>Auto re-check</button>
            <button onClick={healStaleSources} disabled={dataSources.length === 0} style={{ background: "#14532d", color: dataSources.length === 0 ? "#9ca3af" : "#bbf7d0", border: "1px solid #166534", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: dataSources.length === 0 ? "not-allowed" : "pointer", opacity: dataSources.length === 0 ? 0.6 : 1, fontSize: 12 }}>Heal stale</button>
            <button onClick={assignOwnersForMissing} disabled={missingOwnersCount === 0} style={{ background: "#1f2937", color: missingOwnersCount === 0 ? "#9ca3af" : "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: missingOwnersCount === 0 ? "not-allowed" : "pointer", opacity: missingOwnersCount === 0 ? 0.6 : 1, fontSize: 12 }}>Assign owners</button>
            <button onClick={markPiiForMissing} disabled={missingPiiCount === 0} style={{ background: "#0b1221", color: missingPiiCount === 0 ? "#9ca3af" : "#a5f3fc", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: missingPiiCount === 0 ? "not-allowed" : "pointer", opacity: missingPiiCount === 0 ? 0.6 : 1, fontSize: 12 }}>Mark PII missing</button>
          </div>
          {(staleSources.length > 0 || unboundSources.length > 0) && (
            <div style={{ display: "grid", gap: 4, color: "#9ca3af", fontSize: 12 }}>
          ...
          {dataSources.length > 0 && (
            <div style={{ display: "grid", gap: 6 }}>
              <div style={{ color: "#9ca3af", fontSize: 12 }}>Owner coverage: {ownerCoverage}%</div>
              <div style={{ height: 8, background: "#0b1221", borderRadius: 6, overflow: "hidden", border: "1px solid #1f2937" }}>
              ...
              <div style={{ color: "#9ca3af", fontSize: 12 }}>PII coverage: {piiCoverage}%</div>
              <div style={{ height: 8, background: "#0b1221", borderRadius: 6, overflow: "hidden", border: "1px solid #1f2937" }}>
        ...
        <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <label style={{ color: "#cbd5e1", fontSize: 13, display: "flex", gap: 6, alignItems: "center" }}>Perf budget (ms)
              <input type="number" min="100" max="2000" value={perfBudget} onChange={e => setPerfBudget(Math.max(50, Number(e.target.value) || 0))} style={{ width: 90, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "6px 8px" }} />
            </label>
            <div style={{ color: "#9ca3af", fontSize: 13 }}>Avg latency: {averageLatency()}ms · {perfSummary()}</div>
          </div>
          <div style={{ height: 8, background: "#0b1221", borderRadius: 6, overflow: "hidden", border: "1px solid #1f2937" }}>
        ...
        {lintFindings.length > 0 && (
          <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <div style={{ color: "#cbd5e1", fontWeight: 700, fontSize: 13 }}>Open lint findings</div>
              <button onClick={() => setLintFindings([])} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Clear findings</button>
            </div>
            {lintFindings.map((f, idx) => (
              <div key={idx} style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", color: f.level === "high" ? "#f87171" : "#fbbf24" }}>
                <span style={{ fontWeight: 800, fontSize: 12, padding: "2px 8px", borderRadius: 6, background: f.level === "high" ? "#2f1114" : "#2f2311", border: "1px solid #4b5563", color: f.level === "high" ? "#fecdd3" : "#fef3c7" }}>{f.level.toUpperCase()}</span>
                <span>{f.msg}</span>
              </div>
            ))}
          </div>
        ...
        <div style={{ background: "#0b1221", border: "1px solid #1f2937", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            <div style={{ color: "#cbd5e1", fontWeight: 700, fontSize: 13 }}>SLA & owners</div>
            <div style={{ color: "#9ca3af", fontSize: 12 }}>Coverage and alerts snapshot</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ background: ownerCoverage === 100 ? "#0f172a" : "#2f2311", color: ownerCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Owner coverage: {ownerCoverage}%</span>
            <span style={{ background: piiCoverage === 100 ? "#0f172a" : "#2f2311", color: piiCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>PII coverage: {piiCoverage}%</span>
            <span style={{ background: watchlist.length > 0 ? "#1e3a8a" : "#0f172a", color: "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Watchlist: {watchlist.length}</span>
            <span style={{ background: slaBreach ? "#2f1114" : "#0b1221", color: slaBreach ? "#fecdd3" : "#cbd5e1", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>SLA breach: {slaBreach ? "Simulated" : "None"}</span>
            {watchlistMissingTargets.length > 0 && <span style={{ background: "#2f2311", color: "#fcd34d", border: "1px solid #334155", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Watchlist missing targets: {watchlistMissingTargets.length}</span>}
            <button onClick={copySlaSnapshot} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy SLA snapshot</button>
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            <div style={{ color: "#9ca3af", fontSize: 12 }}>Owner coverage</div>
            <div style={{ height: 8, background: "#0b1221", borderRadius: 6, overflow: "hidden", border: "1px solid #1f2937" }}>
              <div style={{ width: `${ownerCoverage}%`, height: "100%", background: ownerCoverage === 100 ? "#22c55e" : ownerCoverage >= 70 ? "#fbbf24" : "#ef4444", transition: "width 0.2s ease" }} />
            </div>
            <div style={{ color: "#9ca3af", fontSize: 12 }}>PII coverage</div>
            <div style={{ height: 8, background: "#0b1221", borderRadius: 6, overflow: "hidden", border: "1px solid #1f2937" }}>
              <div style={{ width: `${piiCoverage}%`, height: "100%", background: piiCoverage === 100 ? "#22c55e" : piiCoverage >= 70 ? "#fbbf24" : "#ef4444", transition: "width 0.2s ease" }} />
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              ...
              <button onClick={() => { setSlaBreach(false); setAuditLog(prev => [{ at: Date.now(), message: "Cleared SLA breach from banner" }, ...prev].slice(0, 8)); runLint(); }} disabled={!slaBreach} style={{ background: "#0b1221", color: slaBreach ? "#e5e7eb" : "#9ca3af", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: slaBreach ? "pointer" : "not-allowed", opacity: slaBreach ? 1 : 0.6, fontSize: 12 }}>Clear SLA breach</button>
            </div>
          </div>
        </div>
        {reminderBanner && (
          <div style={{ background: "#1f2937", border: "1px solid #334155", borderRadius: 12, padding: 12, color: "#fde68a" }}>Reminder: dashboard hasn’t been published in 7+ days.</div>
        )}
        {slaBreach && watchlist.length > 0 && (
          <div style={{ background: "#2f1114", border: "1px solid #4b5563", borderRadius: 12, padding: 12, color: "#fecdd3", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <div>SLA breach simulated for watchlist widgets — route alerts to {escalationTarget || escalationChannel}.</div>
            <button onClick={() => { setSlaBreach(false); setAuditLog(prev => [{ at: Date.now(), message: "SLA breach cleared" }, ...prev].slice(0, 8)); runLint(); }} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Clear breach</button>
          </div>
        )}
        {templateApplied && (
          <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 10, color: "#cbd5e1" }}>Template applied: {templateApplied}</div>
        )}
        {publishPreview.added.length + publishPreview.removed.length > 0 && (
          <div style={{ background: "#0b1221", border: "1px solid #1f2937", borderRadius: 12, padding: 12, color: "#e5e7eb", display: "grid", gap: 6 }}>
        ...
        {(() => {
          ...
          return (
            <div style={{ background: "#111827", border: "1px solid #1f2937", borderRadius: 12, padding: 10, color: "#fcd34d", fontSize: 13, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ fontWeight: 700, color: "#fde68a" }}>Publish checklist:</span>
              {blockers.map((b, idx) => (
                <span key={idx} style={{ background: "#1f2937", border: "1px solid #334155", borderRadius: 10, padding: "4px 8px", color: "#cbd5e1", fontSize: 12 }}>{b}</span>
              ))}
            </div>
          );
        })()}
        {versions.length > 0 && (
          <div style={{ background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
        ...
        {webhookLog && (
          <div style={{ background: "#0b1221", border: "1px solid #1f2937", borderRadius: 10, padding: 10, color: "#bae6fd" }}>Webhook: {webhookLog}</div>
        )}
      </div>
      <div style={{ marginBottom: 18, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        ...
        <button onClick={refreshSources} disabled={Object.keys(freshness).length === 0} style={{ background: Object.keys(freshness).length === 0 ? "#1f2937" : "#22c55e", color: Object.keys(freshness).length === 0 ? "#9ca3af" : "#0b1221", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: Object.keys(freshness).length === 0 ? "not-allowed" : "pointer", opacity: Object.keys(freshness).length === 0 ? 0.6 : 1 }}>Refresh Sources</button>
          <button onClick={autoRecheckSources} disabled={Object.keys(freshness).length === 0} style={{ background: "#0b1221", color: Object.keys(freshness).length === 0 ? "#9ca3af" : "#7dd3fc", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: Object.keys(freshness).length === 0 ? "not-allowed" : "pointer", opacity: Object.keys(freshness).length === 0 ? 0.6 : 1 }}>Auto Re-check</button>
        <input value={schedule} onChange={e => { setSchedule(e.target.value); setUnsaved(true); }} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px", fontWeight: 600 }} aria-label="Publish schedule" />
      </div>
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        <div style={{ fontWeight: 700, color: "#e5e7eb" }}>Metadata & compliance</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input value={metadata.title} onChange={e => { setMetadata({ ...metadata, title: e.target.value }); setUnsaved(true); }} placeholder="Title" style={{ flex: 1, minWidth: 160, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
          <input value={metadata.owner} onChange={e => { setMetadata({ ...metadata, owner: e.target.value }); setUnsaved(true); }} placeholder="Owner" style={{ flex: 1, minWidth: 160, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
          <input value={metadata.tags} onChange={e => { setMetadata({ ...metadata, tags: e.target.value }); setUnsaved(true); }} placeholder="Tags (comma separated)" style={{ flex: 2, minWidth: 200, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
        </div>
        <textarea value={runbook} onChange={e => { setRunbook(e.target.value); setUnsaved(true); }} rows={2} placeholder="Runbook / Ops notes (e.g., how to respond to anomalies)" style={{ width: "100%", background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 10, padding: "8px 10px", fontSize: 13 }} />
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <select value={escalationChannel} onChange={e => { setEscalationChannel(e.target.value); setUnsaved(true); }} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
            <option value="slack">Slack</option>
            <option value="email">Email</option>
            <option value="pager">Pager</option>
          </select>
          <input value={escalationTarget} onChange={e => { setEscalationTarget(e.target.value); setUnsaved(true); }} placeholder="Escalation target (channel, email, rotation)" style={{ flex: 1, minWidth: 200, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
          <div style={{ color: "#9ca3af", fontSize: 12 }}>Alert routing for on-call.</div>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <input value={reviewer} onChange={e => { setReviewer(e.target.value); setUnsaved(true); }} placeholder="Reviewer" style={{ flex: 1, minWidth: 160, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
          <select value={reviewStatus} onChange={e => { setReviewStatus(e.target.value); setUnsaved(true); }} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
            <option value="pending">Pending</option>
            <option value="requested">Requested</option>
            <option value="approved">Approved</option>
            <option value="changes">Changes needed</option>
          </select>
          <div style={{ color: "#9ca3af", fontSize: 12 }}>Approval required to publish.</div>
          <button onClick={() => { setReviewStatus("requested"); setUnsaved(true); setAuditLog(prev => [{ at: Date.now(), message: "Review requested" }, ...prev].slice(0, 8)); }} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Request review</button>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <input value={versionLabel} onChange={e => { setVersionLabel(e.target.value); setUnsaved(true); }} placeholder="Version label (e.g., Q1 refresh)" style={{ flex: 1, minWidth: 200, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
          <input value={versionNotes} onChange={e => { setVersionNotes(e.target.value); setUnsaved(true); }} placeholder="Version notes" style={{ flex: 2, minWidth: 240, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
        </div>
      </div>
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        <div style={{ fontWeight: 700, color: "#e5e7eb" }}>Layout & Targets</div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#cbd5e1" }}>Columns
            <input type="number" min="1" max="6" value={layoutCols} onChange={e => setLayoutCols(Number(e.target.value) || 1)} style={{ width: 64, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "6px 8px" }} />
          </label>
          <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#cbd5e1" }}>Row height
            <input type="number" min="120" max="480" value={rowHeight} onChange={e => setRowHeight(Number(e.target.value) || 120)} style={{ width: 80, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "6px 8px" }} /> px
          </label>
          <div style={{ color: "#9ca3af", fontSize: 13 }}>Preview: {layoutCols} cols · {rowHeight}px rows</div>
          {watchlist.length > 0 && <div style={{ color: "#7dd3fc", fontSize: 13 }}>Watchlist: {watchlist.length} widget{watchlist.length > 1 ? "s" : ""}</div>}
        </div>
        <div style={{ display: "grid", gap: 8 }}>
          <div style={{ color: "#9ca3af", fontSize: 13 }}>Set target notes per widget</div>
          {dashboard.length > 0 && (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", background: "#0b1221", border: "1px solid #1f2937", borderRadius: 10, padding: 8 }}>
              <input value={defaultTarget} onChange={e => { setDefaultTarget(e.target.value); setUnsaved(true); }} placeholder="Default target" style={{ minWidth: 180, background: "#0f172a", color: "#e5e7eb", border: "1px solid #1e293b", borderRadius: 8, padding: "8px 10px" }} />
              ...
              <div style={{ color: "#9ca3af", fontSize: 12 }}>Uses saved default or prompts you.</div>
            </div>
          )}
          {dashboard.map((w, idx) => (
            <div key={w.id} style={{ display: "flex", gap: 8, alignItems: "center", background: "#0f172a", border: "1px solid #1e293b", borderRadius: 10, padding: 8 }}>
              <div style={{ color: "#7dd3fc", fontWeight: 700, minWidth: 140 }}>{w.name}</div>
              <input value={targets[w.id] || ""} onChange={e => { setTargets(prev => ({ ...prev, [w.id]: e.target.value })); setUnsaved(true); }} placeholder="Target / SLA" style={{ flex: 1, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
            </div>
          ))}
          {dashboard.length === 0 && <div style={{ color: "#9ca3af", fontSize: 13 }}>Add widgets to set targets.</div>}
        </div>
      </div>
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        <div style={{ fontWeight: 700, color: "#e5e7eb" }}>Templates</div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {Object.keys(templates).map(key => (
            <button key={key} onClick={() => applyTemplate(key)} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 10, padding: "10px 14px", fontWeight: 700, cursor: "pointer" }}>{key}</button>
          ))}
        </div>
      </div>
      <div style={{ display: "flex", gap: 18, marginBottom: 18 }}>
        <div style={{ flex: 1 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search widgets" style={{ width: "100%", marginBottom: 8, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "8px 10px" }} />
        ...
        <div style={{ flex: 2 }}>
          ...
          <ul style={{ paddingLeft: 18 }}>
            {dashboard.map((w, idx) => (
              <li key={w.id} style={{ marginBottom: 8, background: "#18181b", borderRadius: 8, padding: 8, color: '#7fffd4' }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span>{w.name}</span>
                  {targets[w.id] && <span style={{ color: "#c7d2fe", fontSize: 12 }}>Target: {targets[w.id]}</span>}
                  <button onClick={() => { setWatchlist(prev => prev.includes(w.id) ? prev.filter(id => id !== w.id) : [...prev, w.id]); setUnsaved(true); }} style={{ background: watchlist.includes(w.id) ? "#0ea5e9" : "#1f2937", color: watchlist.includes(w.id) ? "#e0f2fe" : "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>{watchlist.includes(w.id) ? "Watchlisted" : "Watch"}</button>
                </div>
                <div style={{ marginTop: 6, display: "grid", gap: 6 }}>
                  <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                    <input placeholder="Add comment" onKeyDown={e => { if (e.key === "Enter") { addComment(w.id, e.target.value); e.target.value = ""; } }} style={{ flex: 1, minWidth: 180, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "6px 8px", fontSize: 13 }} />
                    <span style={{ color: "#9ca3af", fontSize: 12 }}>Comments: {(comments[w.id] || []).length}</span>
                  </div>
                  {(comments[w.id] || []).slice(-2).map((c, cidx) => (
                    <div key={cidx} style={{ color: "#9ca3af", fontSize: 12, background: "#0b1221", border: "1px solid #1f2937", borderRadius: 8, padding: 6 }}>
          ...
          {watchlist.length > 0 && (
            <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ color: "#9ca3af", fontSize: 12 }}>Watchlist items: {watchlist.length}</div>
              <button onClick={clearWatchlist} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Clear watchlist</button>
            </div>
          )}
          <div style={{ marginTop: 8, color: "#9ca3af", fontSize: 12 }}>Keyboard: Enter to add comment · Space to toggle watchlist · Drag to reorder.</div>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600, marginBottom: 8 }}>Data Sources</div>
          <ul style={{ paddingLeft: 18 }}>
            {dataSources.map((ds, idx) => (
              <li key={ds.id || idx} style={{ marginBottom: 8, background: "#232336", borderRadius: 8, padding: 8, color: '#e5e7eb' }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    {ds.name}
                    {!piiFlags[ds.id || ds.name] && <span style={{ background: "#2f1114", color: "#fecdd3", border: "1px solid #4b5563", borderRadius: 8, padding: "2px 8px", fontSize: 11 }}>PII unchecked</span>}
                  </span>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    <span style={{ color: "#9ca3af", fontSize: 12 }}>Freshness: {formatFreshness(ds.name)}</span>
                    ...
                    <button onClick={() => { setPiiFlags(prev => ({ ...prev, [ds.id || ds.name]: true })); setUnsaved(true); }} style={{ background: "#0b1221", color: "#a5f3fc", border: "1px solid #1f2937", borderRadius: 8, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Mark PII reviewed</button>
                    {(() => {
              ...
              {dataSources.length > 0 && (
                <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  ...
                  <div style={{ color: "#9ca3af", fontSize: 12 }}>Use when legal/compliance review is complete.</div>
                </div>
              )}
                  <input value={dataSourceOwners[ds.name] || ""} onChange={e => { setDataSourceOwners(prev => ({ ...prev, [ds.name]: e.target.value })); setUnsaved(true); }} placeholder="Owner" style={{ minWidth: 140, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "6px 8px", fontSize: 12 }} />
                  <input value={dataSourceNotes[ds.name] || ""} onChange={e => { setDataSourceNotes(prev => ({ ...prev, [ds.name]: e.target.value })); setUnsaved(true); }} placeholder="Notes" style={{ flex: 1, minWidth: 180, background: "#0b1221", color: "#e5e7eb", border: "1px solid #1f2937", borderRadius: 8, padding: "6px 8px", fontSize: 12 }} />
                </div>
              </li>
            ))}
          </ul>
          {(driftAdded.length > 0 || driftRemoved.length > 0) && (
            <div style={{ marginTop: 10, background: "#1f2937", border: "1px solid #334155", borderRadius: 10, padding: 10, color: "#fde68a", fontSize: 13 }}>
          ...
          {dependencyMap.length > 0 && (
            <div style={{ marginTop: 10, background: "#0b1221", border: "1px solid #1f2937", borderRadius: 10, padding: 10, display: "grid", gap: 6 }}>
              <div style={{ fontWeight: 700, color: "#cbd5e1" }}>Dependencies</div>
              {dependencyMap.map((dep, idx) => (
                <div key={idx} style={{ color: "#9ca3af", fontSize: 12 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        <button onClick={downloadDraft} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "10px 18px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Download draft JSON</button>
        <button onClick={copyGuardrailSummary} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy guardrail summary</button>
        <button onClick={copySlaSnapshot} style={{ background: "#0b1221", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy SLA snapshot</button>
        <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#cbd5e1", fontSize: 12 }}>
        ...
        {exportChecksum && <span style={{ marginLeft: 8, color: "#9ca3af", fontSize: 12 }}>Checksum: {exportChecksum}</span>}
      </div>
      ...
      {auditLog.length > 0 && (
        <div style={{ marginBottom: 18, background: "#0f172a", border: "1px solid #1e293b", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
          <div style={{ fontWeight: 800, color: "#cbd5e1" }}>Audit trail</div>
          {auditLog.map((a, idx) => (
            <div key={idx} style={{ display: "grid", gap: 4, color: "#94a3b8", fontSize: 13, borderBottom: "1px solid #1e293b", paddingBottom: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
                <span>{a.message}</span>
                <span>{a.at ? new Date(a.at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : ""}</span>
              </div>
              {(a.guardrails || a.slaSnapshot) && (
                <div style={{ display: "grid", gap: 4, background: "#0b1221", border: "1px solid #1f2937", borderRadius: 8, padding: 8 }}>
                  {a.guardrails && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <div style={{ color: "#cbd5e1", fontSize: 12, wordBreak: "break-word" }}>Guardrails: {a.guardrails}</div>
                      <button onClick={() => { if (navigator?.clipboard?.writeText) navigator.clipboard.writeText(a.guardrails); }} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 6, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Copy</button>
                    </div>
                  )}
                  {a.slaSnapshot && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <div style={{ color: "#cbd5e1", fontSize: 12, wordBreak: "break-word" }}>SLA: {a.slaSnapshot}</div>
                      <button onClick={() => { if (navigator?.clipboard?.writeText) navigator.clipboard.writeText(a.slaSnapshot); }} style={{ background: "#1f2937", color: "#e5e7eb", border: "1px solid #334155", borderRadius: 6, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Copy</button>
                    </div>
                  )}
                </div>
              )}
      ...
      {publishPreview.added.length + publishPreview.removed.length === 0 && versions.length === 0 && (
        <div style={{ marginBottom: 18, background: "#0b1221", border: "1px solid #1f2937", borderRadius: 10, padding: 12, color: "#9ca3af", fontSize: 13 }}>
