  ...
  return (
    <div style={{ background: "#18181b", borderRadius: 18, boxShadow: "0 2px 24px #0008", padding: 36, fontFamily: 'Inter, sans-serif', color: '#f0f0f0' }}>
      ...
      <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
        <div style={{ color: "#888888", fontSize: 13 }}>Unsaved changes: {unsaved ? "Yes" : "No"}</div>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          ...
          }} style={{ background: "#1a1a1a", color: "#f0f0f0", border: "1px solid #333333", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>Restore draft</button>
        </div>
      ...
      <div style={{ display: "grid", gap: 12, marginBottom: 18 }}>
          ...
          <div style={{ background: "#111827", border: "1px solid #1a1a1a", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
            ...
            {lintFindings.map((f, idx) => (
              <div key={idx} style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", color: f.level === "high" ? "#f87171" : "#fbbf24" }}>
                <span style={{ fontWeight: 800, fontSize: 12, padding: "2px 8px", borderRadius: 6, background: f.level === "high" ? "#2f1114" : "#2f2311", border: "1px solid #444444", color: f.level === "high" ? "#fecdd3" : "#fef3c7" }}>{f.level.toUpperCase()}</span>
                <span>{f.msg}</span>
              </div>
            ))}
          </div>
        ...
        {reminderBanner && (
          <div style={{ background: "#1a1a1a", border: "1px solid #333333", borderRadius: 12, padding: 12, color: "#fde68a" }}>Reminder: dashboard hasn’t been published in 7+ days.</div>
        )}
        {slaBreach && watchlist.length > 0 && (
          <div style={{ background: "#2f1114", border: "1px solid #444444", borderRadius: 12, padding: 12, color: "#fecdd3", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <div>SLA breach simulated for watchlist widgets — route alerts to {escalationTarget || escalationChannel}.</div>
            <button onClick={() => { setSlaBreach(false); setAuditLog(prev => [{ at: Date.now(), message: "SLA breach cleared" }, ...prev].slice(0, 8)); runLint(); }} style={{ background: "#1a1a1a", color: "#f0f0f0", border: "1px solid #333333", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Clear breach</button>
          </div>
        )}
        {templateApplied && (
      ...
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        ...
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          ...
          <div style={{ color: "#888888", fontSize: 12 }}>Approval required to publish.</div>
      ...
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        ...
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {Object.keys(templates).map(key => (
            <button key={key} onClick={() => applyTemplate(key)} style={{ background: "#1a1a1a", color: "#f0f0f0", border: "1px solid #333333", borderRadius: 10, padding: "10px 14px", fontWeight: 700, cursor: "pointer" }}>{key}</button>
          ))}
        </div>
      </div>
        ...
        <div style={{ flex: 2 }}>
            ...
            {dashboard.map((w, idx) => (
              <li key={w.id} style={{ marginBottom: 8, background: "#18181b", borderRadius: 8, padding: 8, color: '#7fffd4' }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  ...
                  <button onClick={() => { setWatchlist(prev => prev.includes(w.id) ? prev.filter(id => id !== w.id) : [...prev, w.id]); setUnsaved(true); }} style={{ background: watchlist.includes(w.id) ? "#0ea5e9" : "#1a1a1a", color: watchlist.includes(w.id) ? "#e0f2fe" : "#f0f0f0", border: "1px solid #333333", borderRadius: 8, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>{watchlist.includes(w.id) ? "Watchlisted" : "Watch"}</button>
                </div>
          ...
          {watchlist.length > 0 && (
            <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ color: "#888888", fontSize: 12 }}>Watchlist items: {watchlist.length}</div>
              <button onClick={clearWatchlist} style={{ background: "#1a1a1a", color: "#f0f0f0", border: "1px solid #333333", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Clear watchlist</button>
            </div>
          )}
          <div style={{ marginTop: 8, color: "#888888", fontSize: 12 }}>Keyboard: Enter to add comment · Space to toggle watchlist · Drag to reorder.</div>
        </div>
        <div style={{ flex: 1 }}>
              ...
              <li key={ds.id || idx} style={{ marginBottom: 8, background: "#232336", borderRadius: 8, padding: 8, color: '#f0f0f0' }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    {ds.name}
                    {!piiFlags[ds.id || ds.name] && <span style={{ background: "#2f1114", color: "#fecdd3", border: "1px solid #444444", borderRadius: 8, padding: "2px 8px", fontSize: 11 }}>PII unchecked</span>}
                  </span>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    <span style={{ color: "#888888", fontSize: 12 }}>Freshness: {formatFreshness(ds.name)}</span>
          ...
          {(driftAdded.length > 0 || driftRemoved.length > 0) && (
            <div style={{ marginTop: 10, background: "#1a1a1a", border: "1px solid #333333", borderRadius: 10, padding: 10, color: "#fde68a", fontSize: 13 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        {exportChecksum && <span style={{ marginLeft: 8, color: "#888888", fontSize: 12 }}>Checksum: {exportChecksum}</span>}
      </div>
      ...
      {auditLog.length > 0 && (
        <div style={{ marginBottom: 18, background: "#111111", border: "1px solid #1a1a1a", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
          <div style={{ fontWeight: 800, color: "#cbd5e1" }}>Audit trail</div>
          {auditLog.map((a, idx) => (
            <div key={idx} style={{ display: "grid", gap: 4, color: "#94a3b8", fontSize: 13, borderBottom: "1px solid #1a1a1a", paddingBottom: 6 }}>
                ...
                <div style={{ display: "grid", gap: 4, background: "#0b1221", border: "1px solid #1a1a1a", borderRadius: 8, padding: 8 }}>
                  {a.guardrails && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      ...
                      <button onClick={() => { if (navigator?.clipboard?.writeText) navigator.clipboard.writeText(a.guardrails); }} style={{ background: "#1a1a1a", color: "#f0f0f0", border: "1px solid #333333", borderRadius: 6, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Copy</button>
                    </div>
                  )}
                  {a.slaSnapshot && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <div style={{ color: "#cbd5e1", fontSize: 12, wordBreak: "break-word" }}>SLA: {a.slaSnapshot}</div>
                      <button onClick={() => { if (navigator?.clipboard?.writeText) navigator.clipboard.writeText(a.slaSnapshot); }} style={{ background: "#1a1a1a", color: "#f0f0f0", border: "1px solid #333333", borderRadius: 6, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Copy</button>
                    </div>
                  )}
                </div>
              )}
