...
export default function CustomDashboardBuilder() {
  ...
  return (
    <div style={{ background: "#1f2433", borderRadius: 18, boxShadow: "0 2px 24px #0008", padding: 36, fontFamily: 'Inter, sans-serif', color: '#f6f7fb' }}>
      ...
      {devSandbox && (
        <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 14 }}>
          <div>
            <div style={{ fontWeight: 800, color: "#f59e0b" }}>Sandbox mode</div>
            <div style={{ color: "#b8bed2", fontSize: 13 }}>API-backed fetches are blocked in dev. Switch to Stage/Prod to sync widgets and data sources.</div>
          </div>
      ...
      <div style={{ display: "grid", gap: 8, marginBottom: 14 }}>
        <div style={{ color: "#b8bed2", fontSize: 13 }}>Unsaved changes: {unsaved ? "Yes" : "No"}</div>
        {draftSavedAt && <div style={{ color: "#b8bed2", fontSize: 12 }}>Draft saved: {new Date(draftSavedAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}</div>}
        <div style={{ color: "#6ee7b7", fontSize: 12 }}>Autosave every 30s.</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <button onClick={() => setRole(role === "editor" ? "viewer" : "editor")} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>Role: {role}</button>
          <button onClick={() => setEditLocked(!editLocked)} style={{ background: editLocked ? "#7f1d1d" : "#283044", color: editLocked ? "#fecdd3" : "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>{editLocked ? "Unlock" : "Lock edits"}</button>
          <button onClick={() => {
            const raw = localStorage.getItem("cdb-draft");
            if (raw) {
              const parsed = JSON.parse(raw);
          ...
          }} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>Restore draft</button>
        </div>
        {editLocked && <div style={{ background: "#2f1114", border: "1px solid #4b5780", borderRadius: 10, padding: 10, color: "#fecdd3" }}>View-only: edits and publish are disabled.</div>}
      </div>
      <div style={{ display: "grid", gap: 12, marginBottom: 18 }}>
        <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 10 }}>
          <div>
            <div style={{ fontWeight: 800, color: "#4f46e5" }}>Quality & Guardrails</div>
            <div style={{ color: "#b8bed2", fontSize: 13 }}>Complexity score {complexityScore} · Risk: {riskLevel} · {freshnessLabel} · Perf: {perfSummary()}</div>
            {lintFindings.length > 0 && (
              <div style={{ display: "flex", gap: 6, marginTop: 6, flexWrap: "wrap", alignItems: "center" }}>
                <span style={{ color: "#b8bed2", fontSize: 12 }}>Lint findings:</span>
                ...
                <span style={{ background: "#1f2433", color: "#cbd5e1", borderRadius: 8, padding: "2px 8px", fontSize: 12 }}>Low: {lintSummary.low || 0}</span>
                <button onClick={() => setLintAcknowledged(true)} disabled={lintFindings.length === 0} style={{ background: lintAcknowledged ? "#14532d" : "#283044", color: lintAcknowledged ? "#bbf7d0" : "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "4px 8px", fontWeight: 700, cursor: lintFindings.length === 0 ? "not-allowed" : "pointer", opacity: lintFindings.length === 0 ? 0.6 : 1, fontSize: 12 }}>{lintAcknowledged ? "Acknowledged" : "Acknowledge"}</button>
              </div>
            )}
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => { if (!ensureWritable()) return; runLint(); }} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: isReadOnly ? "not-allowed" : "pointer", opacity: isReadOnly ? 0.6 : 1 }} disabled={isReadOnly}>Run lint</button>
            <button onClick={() => { if (!ensureWritable()) return; handlePublish(); }} style={{ background: published ? "#0ea5e9" : "#22c55e", color: published ? "#e0f2fe" : "#1f2433", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: isReadOnly ? "not-allowed" : "pointer", opacity: isReadOnly ? 0.6 : 1 }} disabled={isReadOnly}>{published ? "Published" : "Publish"}</button>
            <button onClick={handleShare} disabled={devSandbox} style={{ background: devSandbox ? "#283044" : "#4f46e5", color: devSandbox ? "#b8bed2" : "#fff", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: devSandbox ? "not-allowed" : "pointer", opacity: devSandbox ? 0.6 : 1 }}>Share JSON</button>
            <button onClick={() => { if (!ensureWritable()) return; duplicateDashboard(); }} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: isReadOnly ? "not-allowed" : "pointer", opacity: isReadOnly ? 0.6 : 1 }} disabled={isReadOnly}>Duplicate</button>
            <button onClick={() => { if (!ensureWritable()) return; schedulePublish(); }} style={{ background: "#1f2433", color: "#bae6fd", border: "1px solid #283044", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: isReadOnly ? "not-allowed" : "pointer", opacity: isReadOnly ? 0.6 : 1 }} disabled={isReadOnly}>Schedule</button>
            <button onClick={() => { if (!ensureWritable()) return; setWebhookEnabled(!webhookEnabled); }} style={{ background: webhookEnabled ? "#0ea5e9" : "#283044", color: webhookEnabled ? "#e0f2fe" : "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: isReadOnly ? "not-allowed" : "pointer", opacity: isReadOnly ? 0.6 : 1 }} disabled={isReadOnly}>{webhookEnabled ? "Webhook On" : "Webhook Off"}</button>
            <button onClick={generatePreviewToken} disabled={devSandbox} style={{ background: devSandbox ? "#283044" : "#4b5780", color: devSandbox ? "#b8bed2" : "#e0f2fe", border: "1px solid #283044", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: devSandbox ? "not-allowed" : "pointer", opacity: devSandbox ? 0.6 : 1 }}>Share Preview</button>
            <button onClick={measureLatency} style={{ background: "#283044", color: "#7dd3fc", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: isReadOnly ? "not-allowed" : "pointer", opacity: isReadOnly ? 0.6 : 1 }} disabled={isReadOnly}>Measure latency</button>
            <button onClick={simulateLoadTest} style={{ background: simulatedLoad ? "#4b5780" : "#f97316", color: simulatedLoad ? "#f6f7fb" : "#1f2433", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>{simulatedLoad ? "Load active" : "Simulate load"}</button>
            <button onClick={cleanupDashboard} style={{ background: "#1f2433", color: "#a5f3fc", border: "1px solid #283044", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer" }}>Cleanup</button>
            ...
            <button onClick={() => setMaintenanceFreeze(m => !m)} style={{ background: maintenanceFreeze ? "#7f1d1d" : "#283044", color: maintenanceFreeze ? "#fecdd3" : "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>{maintenanceFreeze ? "Freeze: ON" : "Freeze: OFF"}</button>
            <button onClick={() => { if (watchlist.length === 0) { setError("Add watchlist items to simulate SLA breach."); return; } setSlaBreach(true); setAuditLog(prev => [{ at: Date.now(), message: "SLA breach simulated for watchlist" }, ...prev].slice(0, 8)); runLint(); }} style={{ background: slaBreach ? "#7f1d1d" : "#f59e0b", color: slaBreach ? "#fecdd3" : "#1f2433", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>{slaBreach ? "Breach active" : "Simulate SLA breach"}</button>
          </div>
        </div>
        <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            <div style={{ color: "#cbd5e1", fontWeight: 700, fontSize: 13 }}>Guardrail recap</div>
            <div style={{ color: "#b8bed2", fontSize: 12 }}>Freeze, review, lint, PII, owners</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ background: maintenanceFreeze ? "#2f1114" : "#1f2433", color: maintenanceFreeze ? "#fecdd3" : "#bbf7d0", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Freeze: {maintenanceFreeze ? "Active" : "Off"}</span>
            <span style={{ background: reviewStatus !== "approved" ? "#2f2311" : "#1f2433", color: reviewStatus !== "approved" ? "#fcd34d" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Review: {reviewStatus}</span>
            <span style={{ background: highLintCount > 0 ? "#2f1114" : "#1f2433", color: highLintCount > 0 ? "#fecdd3" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>High lint: {highLintCount}</span>
            <span style={{ background: missingPiiCount > 0 ? "#2f2311" : "#1f2433", color: missingPiiCount > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>PII unchecked: {missingPiiCount}</span>
            <span style={{ background: missingOwnersCount > 0 ? "#2f2311" : "#1f2433", color: missingOwnersCount > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Owners missing: {missingOwnersCount}</span>
            <span style={{ background: targetCoverage === 100 ? "#1f2433" : "#2f2311", color: targetCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Target coverage: {targetCoverage}%</span>
            <span style={{ background: ownerCoverage === 100 ? "#1f2433" : "#2f2311", color: ownerCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Owner coverage: {ownerCoverage}%</span>
            <span style={{ background: piiCoverage === 100 ? "#1f2433" : "#2f2311", color: piiCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>PII coverage: {piiCoverage}%</span>
            <span style={{ background: watchlistMissingTargets.length > 0 ? "#2f2311" : "#1f2433", color: watchlistMissingTargets.length > 0 ? "#fcd34d" : "#bbf7d0", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Watchlist gaps: {watchlistMissingTargets.length}</span>
            <button onClick={copyGuardrailSummary} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy summary</button>
            <button onClick={() => setShowGuardrailText(s => !s)} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>{showGuardrailText ? "Hide text" : "Show text"}</button>
          </div>
          {showGuardrailText && (
            <div style={{ display: "grid", gap: 8 }}>
              <textarea readOnly value={buildGuardrailSummary()} style={{ width: "100%", background: "#1f2433", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 10, padding: 10, fontSize: 12 }} rows={2} aria-label="Guardrail summary text" />
              <textarea readOnly value={buildSlaSnapshot()} style={{ width: "100%", background: "#1f2433", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 10, padding: 10, fontSize: 12 }} rows={2} aria-label="SLA snapshot text" />
              {watchlistMissingTargets.length > 0 && (
                <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  <div style={{ color: "#b8bed2", fontSize: 12 }}>Watchlist gaps: {watchlistMissingTargets.join(", ")}</div>
                  <button onClick={copyWatchlistGaps} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy watchlist gaps</button>
                  <button onClick={fillWatchlistTargets} style={{ background: "#22c55e", color: "#1f2433", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: "pointer", fontSize: 12 }}>Fill watchlist targets</button>
                </div>
              )}
            </div>
          )}
        ...
        <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ color: "#b8bed2", fontSize: 12 }}>Targets and last lint</div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ background: missingTargetsCount > 0 ? "#2f2311" : "#1f2433", color: missingTargetsCount > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Missing targets: {missingTargetsCount}</span>
            <span style={{ color: "#b8bed2", fontSize: 12 }}>Watchlist missing: {dashboard.filter(w => watchlist.includes(w.id) && !targets[w.id]).length}</span>
            <button onClick={setAllTargets} disabled={dashboard.length === 0} style={{ background: "#22c55e", color: "#1f2433", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: dashboard.length === 0 ? "not-allowed" : "pointer", opacity: dashboard.length === 0 ? 0.6 : 1, fontSize: 12 }}>Fill missing targets</button>
            <div style={{ color: "#b8bed2", fontSize: 12 }}>{lintRunAt ? `Lint at ${new Date(lintRunAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}` : "Lint not run yet"}</div>
            <button onClick={runLint} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Run lint now</button>
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            <div style={{ color: "#b8bed2", fontSize: 12 }}>Target coverage: {targetCoverage}%</div>
            <div style={{ height: 8, background: "#1f2433", borderRadius: 6, overflow: "hidden", border: "1px solid #283044" }}>
        ...
        <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ color: "#b8bed2", fontSize: 12 }}>Stale & unbound check</div>
          </div>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <span style={{ background: staleSources.length > 0 ? "#2f1114" : "#1f2433", color: staleSources.length > 0 ? "#fecdd3" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Stale: {staleSources.length}</span>
            <span style={{ background: unboundSources.length > 0 ? "#2f2311" : "#1f2433", color: unboundSources.length > 0 ? "#fcd34d" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Unbound: {unboundSources.length}</span>
            <button onClick={autoRecheckSources} disabled={dataSources.length === 0} style={{ background: "#1f2433", color: dataSources.length === 0 ? "#b8bed2" : "#7dd3fc", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: dataSources.length === 0 ? "not-allowed" : "pointer", opacity: dataSources.length === 0 ? 0.6 : 1, fontSize: 12 }}>Auto re-check</button>
            <button onClick={healStaleSources} disabled={dataSources.length === 0} style={{ background: "#14532d", color: dataSources.length === 0 ? "#b8bed2" : "#bbf7d0", border: "1px solid #166534", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: dataSources.length === 0 ? "not-allowed" : "pointer", opacity: dataSources.length === 0 ? 0.6 : 1, fontSize: 12 }}>Heal stale</button>
            <button onClick={assignOwnersForMissing} disabled={missingOwnersCount === 0} style={{ background: "#283044", color: missingOwnersCount === 0 ? "#b8bed2" : "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: missingOwnersCount === 0 ? "not-allowed" : "pointer", opacity: missingOwnersCount === 0 ? 0.6 : 1, fontSize: 12 }}>Assign owners</button>
            <button onClick={markPiiForMissing} disabled={missingPiiCount === 0} style={{ background: "#1f2433", color: missingPiiCount === 0 ? "#b8bed2" : "#a5f3fc", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: missingPiiCount === 0 ? "not-allowed" : "pointer", opacity: missingPiiCount === 0 ? 0.6 : 1, fontSize: 12 }}>Mark PII missing</button>
          </div>
          {(staleSources.length > 0 || unboundSources.length > 0) && (
            <div style={{ display: "grid", gap: 4, color: "#b8bed2", fontSize: 12 }}>
          ...
          {dataSources.length > 0 && (
            <div style={{ display: "grid", gap: 6 }}>
              <div style={{ color: "#b8bed2", fontSize: 12 }}>Owner coverage: {ownerCoverage}%</div>
              <div style={{ height: 8, background: "#1f2433", borderRadius: 6, overflow: "hidden", border: "1px solid #283044" }}>
              ...
              <div style={{ color: "#b8bed2", fontSize: 12 }}>PII coverage: {piiCoverage}%</div>
              <div style={{ height: 8, background: "#1f2433", borderRadius: 6, overflow: "hidden", border: "1px solid #283044" }}>
        ...
        <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap" }}>
            <label style={{ color: "#cbd5e1", fontSize: 13, display: "flex", gap: 6, alignItems: "center" }}>Perf budget (ms)
              <input type="number" min="100" max="2000" value={perfBudget} onChange={e => setPerfBudget(Math.max(50, Number(e.target.value) || 0))} style={{ width: 90, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "6px 8px" }} />
            </label>
            <div style={{ color: "#b8bed2", fontSize: 13 }}>Avg latency: {averageLatency()}ms · {perfSummary()}</div>
          </div>
          <div style={{ height: 8, background: "#1f2433", borderRadius: 6, overflow: "hidden", border: "1px solid #283044" }}>
        ...
        {lintFindings.length > 0 && (
          <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
              <div style={{ color: "#cbd5e1", fontWeight: 700, fontSize: 13 }}>Open lint findings</div>
              <button onClick={() => setLintFindings([])} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Clear findings</button>
            </div>
            {lintFindings.map((f, idx) => (
              <div key={idx} style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", color: f.level === "high" ? "#f87171" : "#fbbf24" }}>
                <span style={{ fontWeight: 800, fontSize: 12, padding: "2px 8px", borderRadius: 6, background: f.level === "high" ? "#2f1114" : "#2f2311", border: "1px solid #4b5780", color: f.level === "high" ? "#fecdd3" : "#fef3c7" }}>{f.level.toUpperCase()}</span>
                <span>{f.msg}</span>
              </div>
            ))}
          </div>
        ...
        <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            ...
            <div style={{ color: "#b8bed2", fontSize: 12 }}>Coverage and alerts snapshot</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
            <span style={{ background: ownerCoverage === 100 ? "#1f2433" : "#2f2311", color: ownerCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Owner coverage: {ownerCoverage}%</span>
            <span style={{ background: piiCoverage === 100 ? "#1f2433" : "#2f2311", color: piiCoverage === 100 ? "#bbf7d0" : "#fcd34d", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>PII coverage: {piiCoverage}%</span>
            <span style={{ background: watchlist.length > 0 ? "#283044" : "#1f2433", color: "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Watchlist: {watchlist.length}</span>
            <span style={{ background: slaBreach ? "#2f1114" : "#1f2433", color: slaBreach ? "#fecdd3" : "#cbd5e1", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>SLA breach: {slaBreach ? "Simulated" : "None"}</span>
            {watchlistMissingTargets.length > 0 && <span style={{ background: "#2f2311", color: "#fcd34d", border: "1px solid #4b5780", borderRadius: 10, padding: "6px 10px", fontSize: 12 }}>Watchlist missing targets: {watchlistMissingTargets.length}</span>}
            <button onClick={copySlaSnapshot} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy SLA snapshot</button>
          </div>
          <div style={{ display: "grid", gap: 6 }}>
            <div style={{ color: "#b8bed2", fontSize: 12 }}>Owner coverage</div>
            <div style={{ height: 8, background: "#1f2433", borderRadius: 6, overflow: "hidden", border: "1px solid #283044" }}>
              <div style={{ width: `${ownerCoverage}%`, height: "100%", background: ownerCoverage === 100 ? "#22c55e" : ownerCoverage >= 70 ? "#fbbf24" : "#ef4444", transition: "width 0.2s ease" }} />
            </div>
            <div style={{ color: "#b8bed2", fontSize: 12 }}>PII coverage</div>
            <div style={{ height: 8, background: "#1f2433", borderRadius: 6, overflow: "hidden", border: "1px solid #283044" }}>
              <div style={{ width: `${piiCoverage}%`, height: "100%", background: piiCoverage === 100 ? "#22c55e" : piiCoverage >= 70 ? "#fbbf24" : "#ef4444", transition: "width 0.2s ease" }} />
            </div>
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              <button onClick={assignOwnersForMissing} disabled={missingOwnersCount === 0} style={{ background: "#283044", color: missingOwnersCount === 0 ? "#b8bed2" : "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: missingOwnersCount === 0 ? "not-allowed" : "pointer", opacity: missingOwnersCount === 0 ? 0.6 : 1, fontSize: 12 }}>Assign owners</button>
              <button onClick={markPiiForMissing} disabled={missingPiiCount === 0} style={{ background: "#1f2433", color: missingPiiCount === 0 ? "#b8bed2" : "#a5f3fc", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: missingPiiCount === 0 ? "not-allowed" : "pointer", opacity: missingPiiCount === 0 ? 0.6 : 1, fontSize: 12 }}>Mark PII missing</button>
              <button onClick={setAllTargets} disabled={dashboard.length === 0} style={{ background: "#22c55e", color: "#1f2433", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: dashboard.length === 0 ? "not-allowed" : "pointer", opacity: dashboard.length === 0 ? 0.6 : 1, fontSize: 12 }}>Fill targets</button>
              <button onClick={() => { setSlaBreach(false); setAuditLog(prev => [{ at: Date.now(), message: "Cleared SLA breach from banner" }, ...prev].slice(0, 8)); runLint(); }} disabled={!slaBreach} style={{ background: "#1f2433", color: slaBreach ? "#f6f7fb" : "#b8bed2", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: slaBreach ? "pointer" : "not-allowed", opacity: slaBreach ? 1 : 0.6, fontSize: 12 }}>Clear SLA breach</button>
            </div>
          </div>
        </div>
        {reminderBanner && (
          <div style={{ background: "#283044", border: "1px solid #4b5780", borderRadius: 12, padding: 12, color: "#fde68a" }}>Reminder: dashboard hasn’t been published in 7+ days.</div>
        )}
        {slaBreach && watchlist.length > 0 && (
          <div style={{ background: "#2f1114", border: "1px solid #4b5780", borderRadius: 12, padding: 12, color: "#fecdd3", display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <div>SLA breach simulated for watchlist widgets — route alerts to {escalationTarget || escalationChannel}.</div>
            <button onClick={() => { setSlaBreach(false); setAuditLog(prev => [{ at: Date.now(), message: "SLA breach cleared" }, ...prev].slice(0, 8)); runLint(); }} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Clear breach</button>
          </div>
        )}
        {templateApplied && (
          <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 10, color: "#cbd5e1" }}>Template applied: {templateApplied}</div>
        )}
        {publishPreview.added.length + publishPreview.removed.length > 0 && (
          <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, color: "#f6f7fb", display: "grid", gap: 6 }}>
        ...
        {(() => {
          ...
          return (
            <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 10, color: "#fcd34d", fontSize: 13, display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              <span style={{ fontWeight: 700, color: "#fde68a" }}>Publish checklist:</span>
              {blockers.map((b, idx) => (
                <span key={idx} style={{ background: "#283044", border: "1px solid #4b5780", borderRadius: 10, padding: "4px 8px", color: "#cbd5e1", fontSize: 12 }}>{b}</span>
              ))}
            </div>
          );
        })()}
        ...
        {webhookLog && (
          <div style={{ background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: 10, color: "#bae6fd" }}>Webhook: {webhookLog}</div>
        )}
      </div>
      <div style={{ marginBottom: 18, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
        <select value={env} onChange={e => setEnv(e.target.value)} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "10px 12px", fontWeight: 700 }}>
          <option value="dev">Dev</option>
          <option value="stage">Stage</option>
          <option value="prod">Prod</option>
        </select>
        <button onClick={fetchWidgets} disabled={devSandbox} style={{ background: devSandbox ? "#283044" : "#4f46e5", color: devSandbox ? "#b8bed2" : "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontWeight: 600, fontSize: 15, cursor: devSandbox ? "not-allowed" : "pointer", opacity: devSandbox ? 0.7 : 1 }}>Load Widget Library</button>
        <button onClick={fetchDataSources} disabled={devSandbox} style={{ background: devSandbox ? "#283044" : "#0ea5e9", color: devSandbox ? "#b8bed2" : "#fff", border: "none", borderRadius: 8, padding: "8px 18px", fontWeight: 600, fontSize: 15, cursor: devSandbox ? "not-allowed" : "pointer", marginLeft: 0, opacity: devSandbox ? 0.7 : 1 }}>Load Data Sources</button>
        <button onClick={refreshSources} disabled={Object.keys(freshness).length === 0} style={{ background: Object.keys(freshness).length === 0 ? "#283044" : "#22c55e", color: Object.keys(freshness).length === 0 ? "#b8bed2" : "#1f2433", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: Object.keys(freshness).length === 0 ? "not-allowed" : "pointer", opacity: Object.keys(freshness).length === 0 ? 0.6 : 1 }}>Refresh Sources</button>
          <button onClick={autoRecheckSources} disabled={Object.keys(freshness).length === 0} style={{ background: "#1f2433", color: Object.keys(freshness).length === 0 ? "#b8bed2" : "#7dd3fc", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: Object.keys(freshness).length === 0 ? "not-allowed" : "pointer", opacity: Object.keys(freshness).length === 0 ? 0.6 : 1 }}>Auto Re-check</button>
        <input value={schedule} onChange={e => { setSchedule(e.target.value); setUnsaved(true); }} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px", fontWeight: 600 }} aria-label="Publish schedule" />
      </div>
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        <div style={{ fontWeight: 700, color: "#f6f7fb" }}>Metadata & compliance</div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
          <input value={metadata.title} onChange={e => { setMetadata({ ...metadata, title: e.target.value }); setUnsaved(true); }} placeholder="Title" style={{ flex: 1, minWidth: 160, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
          <input value={metadata.owner} onChange={e => { setMetadata({ ...metadata, owner: e.target.value }); setUnsaved(true); }} placeholder="Owner" style={{ flex: 1, minWidth: 160, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
          <input value={metadata.tags} onChange={e => { setMetadata({ ...metadata, tags: e.target.value }); setUnsaved(true); }} placeholder="Tags (comma separated)" style={{ flex: 2, minWidth: 200, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
        </div>
        <textarea value={runbook} onChange={e => { setRunbook(e.target.value); setUnsaved(true); }} rows={2} placeholder="Runbook / Ops notes (e.g., how to respond to anomalies)" style={{ width: "100%", background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 10, padding: "8px 10px", fontSize: 13 }} />
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <select value={escalationChannel} onChange={e => { setEscalationChannel(e.target.value); setUnsaved(true); }} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
            <option value="slack">Slack</option>
            <option value="email">Email</option>
            <option value="pager">Pager</option>
          </select>
          <input value={escalationTarget} onChange={e => { setEscalationTarget(e.target.value); setUnsaved(true); }} placeholder="Escalation target (channel, email, rotation)" style={{ flex: 1, minWidth: 200, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
          <div style={{ color: "#b8bed2", fontSize: 12 }}>Alert routing for on-call.</div>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <input value={reviewer} onChange={e => { setReviewer(e.target.value); setUnsaved(true); }} placeholder="Reviewer" style={{ flex: 1, minWidth: 160, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
          <select value={reviewStatus} onChange={e => { setReviewStatus(e.target.value); setUnsaved(true); }} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
            <option value="pending">Pending</option>
            <option value="requested">Requested</option>
            <option value="approved">Approved</option>
            <option value="changes">Changes needed</option>
          </select>
          <div style={{ color: "#b8bed2", fontSize: 12 }}>Approval required to publish.</div>
          <button onClick={() => { setReviewStatus("requested"); setUnsaved(true); setAuditLog(prev => [{ at: Date.now(), message: "Review requested" }, ...prev].slice(0, 8)); }} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Request review</button>
        </div>
        <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <input value={versionLabel} onChange={e => { setVersionLabel(e.target.value); setUnsaved(true); }} placeholder="Version label (e.g., Q1 refresh)" style={{ flex: 1, minWidth: 200, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
          <input value={versionNotes} onChange={e => { setVersionNotes(e.target.value); setUnsaved(true); }} placeholder="Version notes" style={{ flex: 2, minWidth: 240, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
        </div>
      </div>
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        <div style={{ fontWeight: 700, color: "#f6f7fb" }}>Layout & Targets</div>
        <div style={{ display: "flex", gap: 12, flexWrap: "wrap", alignItems: "center" }}>
          <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#cbd5e1" }}>Columns
            <input type="number" min="1" max="6" value={layoutCols} onChange={e => setLayoutCols(Number(e.target.value) || 1)} style={{ width: 64, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "6px 8px" }} />
          </label>
          <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#cbd5e1" }}>Row height
            <input type="number" min="120" max="480" value={rowHeight} onChange={e => setRowHeight(Number(e.target.value) || 120)} style={{ width: 80, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "6px 8px" }} /> px
          </label>
          <div style={{ color: "#b8bed2", fontSize: 13 }}>Preview: {layoutCols} cols · {rowHeight}px rows</div>
          {watchlist.length > 0 && <div style={{ color: "#7dd3fc", fontSize: 13 }}>Watchlist: {watchlist.length} widget{watchlist.length > 1 ? "s" : ""}</div>}
        </div>
        <div style={{ display: "grid", gap: 8 }}>
          <div style={{ color: "#b8bed2", fontSize: 13 }}>Set target notes per widget</div>
          {dashboard.length > 0 && (
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center", background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: 8 }}>
              <input value={defaultTarget} onChange={e => { setDefaultTarget(e.target.value); setUnsaved(true); }} placeholder="Default target" style={{ minWidth: 180, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
              <button onClick={setAllTargets} style={{ background: "#22c55e", color: "#1f2433", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>Apply to missing targets</button>
              <div style={{ color: "#b8bed2", fontSize: 12 }}>Uses saved default or prompts you.</div>
            </div>
          )}
          {dashboard.map((w, idx) => (
            <div key={w.id} style={{ display: "flex", gap: 8, alignItems: "center", background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: 8 }}>
              <div style={{ color: "#7dd3fc", fontWeight: 700, minWidth: 140 }}>{w.name}</div>
              <input value={targets[w.id] || ""} onChange={e => { setTargets(prev => ({ ...prev, [w.id]: e.target.value })); setUnsaved(true); }} placeholder="Target / SLA" style={{ flex: 1, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
            </div>
          ))}
          {dashboard.length === 0 && <div style={{ color: "#b8bed2", fontSize: 13 }}>Add widgets to set targets.</div>}
        </div>
      </div>
      <div style={{ display: "grid", gap: 10, marginBottom: 18 }}>
        <div style={{ fontWeight: 700, color: "#f6f7fb" }}>Templates</div>
        <div style={{ display: "flex", gap: 10, flexWrap: "wrap" }}>
          {Object.keys(templates).map(key => (
            <button key={key} onClick={() => applyTemplate(key)} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 10, padding: "10px 14px", fontWeight: 700, cursor: "pointer" }}>{key}</button>
          ))}
        </div>
      </div>
      <div style={{ display: "flex", gap: 18, marginBottom: 18 }}>
        <div style={{ flex: 1 }}>
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search widgets" style={{ width: "100%", marginBottom: 8, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "8px 10px" }} />
          <ul style={{ paddingLeft: 18 }}>
            {filteredWidgets.map((w, idx) => (
              <li key={w.id || idx} draggable onDragStart={() => handleDragStart(idx)} style={{ marginBottom: 8, background: "#3a4560", borderRadius: 8, padding: 8, cursor: "grab", color: '#f6f7fb' }}>{w.name}</li>
            ))}
          </ul>
        </div>
        <div style={{ flex: 2 }}>
          ...
          <ul style={{ paddingLeft: 18 }}>
            {dashboard.map((w, idx) => (
              <li key={w.id} style={{ marginBottom: 8, background: "#1f2433", borderRadius: 8, padding: 8, color: '#4f46e5' }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span>{w.name}</span>
                  {targets[w.id] && <span style={{ color: "#e9ebf5", fontSize: 12 }}>Target: {targets[w.id]}</span>}
                  <button onClick={() => { setWatchlist(prev => prev.includes(w.id) ? prev.filter(id => id !== w.id) : [...prev, w.id]); setUnsaved(true); }} style={{ background: watchlist.includes(w.id) ? "#0ea5e9" : "#283044", color: watchlist.includes(w.id) ? "#e0f2fe" : "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>{watchlist.includes(w.id) ? "Watchlisted" : "Watch"}</button>
                </div>
                <div style={{ marginTop: 6, display: "grid", gap: 6 }}>
                  <div style={{ display: "flex", gap: 6, alignItems: "center", flexWrap: "wrap" }}>
                    <input placeholder="Add comment" onKeyDown={e => { if (e.key === "Enter") { addComment(w.id, e.target.value); e.target.value = ""; } }} style={{ flex: 1, minWidth: 180, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "6px 8px", fontSize: 13 }} />
                    <span style={{ color: "#b8bed2", fontSize: 12 }}>Comments: {(comments[w.id] || []).length}</span>
                  </div>
                  {(comments[w.id] || []).slice(-2).map((c, cidx) => (
                    <div key={cidx} style={{ color: "#b8bed2", fontSize: 12, background: "#1f2433", border: "1px solid #283044", borderRadius: 8, padding: 6 }}>
          ...
          {watchlist.length > 0 && (
            <div style={{ marginTop: 10, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
              <div style={{ color: "#b8bed2", fontSize: 12 }}>Watchlist items: {watchlist.length}</div>
              <button onClick={clearWatchlist} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Clear watchlist</button>
            </div>
          )}
          <div style={{ marginTop: 8, color: "#b8bed2", fontSize: 12 }}>Keyboard: Enter to add comment · Space to toggle watchlist · Drag to reorder.</div>
        </div>
        <div style={{ flex: 1 }}>
          <div style={{ fontWeight: 600, marginBottom: 8 }}>Data Sources</div>
          <ul style={{ paddingLeft: 18 }}>
            {dataSources.map((ds, idx) => (
              <li key={ds.id || idx} style={{ marginBottom: 8, background: "#3a4560", borderRadius: 8, padding: 8, color: '#f6f7fb' }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                  <span style={{ display: "flex", gap: 6, alignItems: "center" }}>
                    {ds.name}
                    {!piiFlags[ds.id || ds.name] && <span style={{ background: "#2f1114", color: "#fecdd3", border: "1px solid #4b5780", borderRadius: 8, padding: "2px 8px", fontSize: 11 }}>PII unchecked</span>}
                  </span>
                  <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
                    <span style={{ color: "#b8bed2", fontSize: 12 }}>Freshness: {formatFreshness(ds.name)}</span>
                    ...
                    <button onClick={() => { setPiiFlags(prev => ({ ...prev, [ds.id || ds.name]: true })); setUnsaved(true); }} style={{ background: "#1f2433", color: "#a5f3fc", border: "1px solid #283044", borderRadius: 8, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Mark PII reviewed</button>
                    {(() => {
              ...
              {dataSources.length > 0 && (
                <div style={{ marginTop: 8, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
                  <button onClick={() => { const updated = { ...piiFlags }; dataSources.forEach(ds => { updated[ds.id || ds.name] = true; }); setPiiFlags(updated); setUnsaved(true); }} style={{ background: "#0ea5e9", color: "#1f2433", border: "none", borderRadius: 8, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>Mark all PII reviewed</button>
                  <div style={{ color: "#b8bed2", fontSize: 12 }}>Use when legal/compliance review is complete.</div>
                </div>
              )}
                  <input value={dataSourceOwners[ds.name] || ""} onChange={e => { setDataSourceOwners(prev => ({ ...prev, [ds.name]: e.target.value })); setUnsaved(true); }} placeholder="Owner" style={{ minWidth: 140, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "6px 8px", fontSize: 12 }} />
                  <input value={dataSourceNotes[ds.name] || ""} onChange={e => { setDataSourceNotes(prev => ({ ...prev, [ds.name]: e.target.value })); setUnsaved(true); }} placeholder="Notes" style={{ flex: 1, minWidth: 180, background: "#1f2433", color: "#f6f7fb", border: "1px solid #283044", borderRadius: 8, padding: "6px 8px", fontSize: 12 }} />
                </div>
              </li>
            ))}
          </ul>
          {(driftAdded.length > 0 || driftRemoved.length > 0) && (
            <div style={{ marginTop: 10, background: "#283044", border: "1px solid #4b5780", borderRadius: 10, padding: 10, color: "#fde68a", fontSize: 13 }}>
          ...
          {dependencyMap.length > 0 && (
            <div style={{ marginTop: 10, background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: 10, display: "grid", gap: 6 }}>
              <div style={{ fontWeight: 700, color: "#cbd5e1" }}>Dependencies</div>
              {dependencyMap.map((dep, idx) => (
                <div key={idx} style={{ color: "#b8bed2", fontSize: 12 }}>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        <button onClick={downloadDraft} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "10px 18px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Download draft JSON</button>
        <button onClick={copyGuardrailSummary} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy guardrail summary</button>
        <button onClick={copySlaSnapshot} style={{ background: "#1f2433", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "8px 12px", fontWeight: 700, cursor: "pointer", fontSize: 12 }}>Copy SLA snapshot</button>
        <label style={{ display: "flex", alignItems: "center", gap: 6, color: "#cbd5e1", fontSize: 12 }}>
        ...
        {exportChecksum && <span style={{ marginLeft: 8, color: "#b8bed2", fontSize: 12 }}>Checksum: {exportChecksum}</span>}
      </div>
      {imported && <div style={{ color: "#22c55e", marginBottom: 8 }}>Imported: {imported}</div>}
      {error && (
        <div style={{ color: "#ef4444", marginBottom: 10, display: "flex", gap: 8, flexWrap: "wrap", alignItems: "center" }}>
          <span>{error}</span>
          {quickFixForIssue(error) === "retry" && <button onClick={() => { setError(""); fetchWidgets(); }} style={{ background: "#22c55e", color: "#1f2433", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: "pointer" }}>Retry fetch</button>}
          {quickFixForIssue(error) === "reset" && <button onClick={() => setDashboard([])} style={{ background: "#22c55e", color: "#1f2433", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: "pointer" }}>Reset dashboard</button>}
        </div>
      )}
      ...
      {history.length > 0 && (
        <div style={{ marginBottom: 18, background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 8 }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
            <div style={{ fontWeight: 800, color: "#f6f7fb" }}>Recent dashboards</div>
          ...
          <div style={{ display: "grid", gap: 8 }}>
            {history.slice(0, 3).map((h, idx) => (
              <div key={idx} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: "8px 10px" }}>
                <div>
                  <div style={{ fontWeight: 700, color: "#f6f7fb" }}>{h.summary || "Snapshot"}</div>
                  <div style={{ color: "#b8bed2", fontSize: 12 }}>{h.at ? new Date(h.at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : "recent"} · {h.env}</div>
                </div>
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                  <button onClick={() => restoreSnapshot(h)} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 8, padding: "6px 10px", fontWeight: 700, cursor: "pointer" }}>Load</button>
                  <button onClick={() => { restoreSnapshot(h); setTimeout(() => handleExport(), 0); }} disabled={devSandbox} style={{ background: devSandbox ? "#283044" : "#22c55e", color: devSandbox ? "#b8bed2" : "#1f2433", border: "none", borderRadius: 8, padding: "6px 10px", fontWeight: 800, cursor: devSandbox ? "not-allowed" : "pointer", opacity: devSandbox ? 0.6 : 1 }}>{devSandbox ? "Sandbox" : "Re-export"}</button>
                </div>
              </div>
            ))}
          </div>
      ...
      {auditLog.length > 0 && (
        <div style={{ marginBottom: 18, background: "#1f2433", border: "1px solid #283044", borderRadius: 12, padding: 12, display: "grid", gap: 6 }}>
          <div style={{ fontWeight: 800, color: "#cbd5e1" }}>Audit trail</div>
          {auditLog.map((a, idx) => (
            <div key={idx} style={{ display: "grid", gap: 4, color: "#94a3b8", fontSize: 13, borderBottom: "1px solid #283044", paddingBottom: 6 }}>
              <div style={{ display: "flex", justifyContent: "space-between", gap: 8, flexWrap: "wrap" }}>
                <span>{a.message}</span>
                <span>{a.at ? new Date(a.at).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" }) : ""}</span>
              </div>
              {(a.guardrails || a.slaSnapshot) && (
                <div style={{ display: "grid", gap: 4, background: "#1f2433", border: "1px solid #283044", borderRadius: 8, padding: 8 }}>
                  {a.guardrails && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <div style={{ color: "#cbd5e1", fontSize: 12, wordBreak: "break-word" }}>Guardrails: {a.guardrails}</div>
                      <button onClick={() => { if (navigator?.clipboard?.writeText) navigator.clipboard.writeText(a.guardrails); }} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 6, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Copy</button>
                    </div>
                  )}
                  {a.slaSnapshot && (
                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 8, flexWrap: "wrap" }}>
                      <div style={{ color: "#cbd5e1", fontSize: 12, wordBreak: "break-word" }}>SLA: {a.slaSnapshot}</div>
                      <button onClick={() => { if (navigator?.clipboard?.writeText) navigator.clipboard.writeText(a.slaSnapshot); }} style={{ background: "#283044", color: "#f6f7fb", border: "1px solid #4b5780", borderRadius: 6, padding: "4px 8px", fontWeight: 700, cursor: "pointer", fontSize: 11 }}>Copy</button>
                    </div>
                  )}
                </div>
              )}
      ...
      {publishPreview.added.length + publishPreview.removed.length === 0 && versions.length === 0 && (
        <div style={{ marginBottom: 18, background: "#1f2433", border: "1px solid #283044", borderRadius: 10, padding: 12, color: "#b8bed2", fontSize: 13 }}>
      ...
      <form onSubmit={e => { e.preventDefault(); handleFeedback(); }} style={{ marginTop: 32, background: "#3a4560", borderRadius: 12, padding: 20 }} aria-label="Send feedback">
        ...
        <textarea
          ...
          rows={2}
          style={{ width: "100%", fontSize: 15, padding: 10, borderRadius: 8, border: "1px solid #4f46e5", marginBottom: 12, background: "#1f2433", color: "#f6f7fb" }}
