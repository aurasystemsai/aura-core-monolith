...
export default function WebhookApiTriggers() {
  ...
  const onboardingContent = (
    <div style={{ padding: 24, background: '#111827', borderRadius: 12, marginBottom: 18, color: '#e5e7eb', border: '1px solid #1f2937' }}>
      <h3 style={{ fontWeight: 700, fontSize: 22 }}>Welcome to Webhook & API Triggers</h3>
      <ul style={{ margin: '16px 0 0 18px', color: '#cbd5f5', fontSize: 16 }}>
    ...
    <div style={{ padding: 24, background: '#0f1115', color: '#e5e7eb', borderRadius: 16, border: '1px solid #1f2937', boxShadow: '0 12px 48px #0007' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 12, marginBottom: 16 }}>
        <div style={{ background: '#111827', borderRadius: 12, padding: 14, border: '1px solid #1f2937' }}>
          <div style={{ fontWeight: 700, marginBottom: 6, color: '#e5e7eb' }}>Webhook Config</div>
          <input value={webhookInput} onChange={e => setWebhookInput(e.target.value)} placeholder="Webhook URL" style={{ width: '100%', fontSize: 15, padding: 10, borderRadius: 10, border: '1px solid #1f2937', background: '#0f172a', color: '#e5e7eb', marginBottom: 10 }} />
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            {selectedWebhookId && <span style={{ color: '#cbd5f5', fontSize: 13 }}>Editing ID {selectedWebhookId.slice(0, 8)}… · rev {selectedWebhookRevision ?? '—'}</span>}
          </div>
        </div>
        <div style={{ background: '#111827', borderRadius: 12, padding: 14, border: '1px solid #1f2937' }}>
          <div style={{ fontWeight: 700, marginBottom: 6, color: '#e5e7eb' }}>API Trigger</div>
          <input value={apiInput} onChange={e => setApiInput(e.target.value)} placeholder="API endpoint" style={{ width: '100%', fontSize: 15, padding: 10, borderRadius: 10, border: '1px solid #1f2937', background: '#0f172a', color: '#e5e7eb', marginBottom: 10 }} />
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            ...
            <button onClick={handleUpdateApi} style={{ background: '#6366f1', color: '#e5e7eb', border: 'none', borderRadius: 10, padding: '8px 14px', fontWeight: 800, cursor: 'pointer' }}>Update Selected</button>
            {selectedApiId && <span style={{ color: '#cbd5f5', fontSize: 13 }}>Editing ID {selectedApiId.slice(0, 8)}… · rev {selectedApiRevision ?? '—'}</span>}
          </div>
        </div>
        <div style={{ background: '#111827', borderRadius: 12, padding: 14, border: '1px solid #1f2937' }}>
          <div style={{ fontWeight: 700, marginBottom: 6, color: '#e5e7eb' }}>Guardrails</div>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, color: '#cbd5f5', fontSize: 14, marginBottom: 6 }}>
            Preset
            <select value={guardrailPreset} onChange={e => applyPreset(e.target.value)} style={{ background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 8 }}>
              <option value="conservative">Conservative</option>
              <option value="standard">Standard</option>
              <option value="aggressive">Aggressive</option>
            </select>
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, color: '#cbd5f5', fontSize: 14 }}>
            Rate limit (req/min)
            <input type="number" value={rateLimit} onChange={e => setRateLimit(Number(e.target.value))} style={{ background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 8 }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, color: '#cbd5f5', fontSize: 14, marginTop: 8 }}>
            Concurrency limit
            <input type="number" value={concurrencyLimit} onChange={e => setConcurrencyLimit(Number(e.target.value))} style={{ background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 8 }} />
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 10, color: '#cbd5f5' }}>
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, color: '#cbd5f5', fontSize: 14, marginTop: 8 }}>
            ...
            <input type="number" value={canaryPercent} onChange={e => setCanaryPercent(Number(e.target.value))} min={0} max={100} style={{ background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 8 }} />
          </label>
          <label style={{ display: 'flex', alignItems: 'center', gap: 8, marginTop: 8, color: '#cbd5f5' }}>
          ...
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, color: '#cbd5f5', fontSize: 14, marginTop: 8 }}>
            ...
            <input type="number" value={performanceBudgetMs} onChange={e => setPerformanceBudgetMs(Number(e.target.value))} placeholder="e.g., 500" style={{ background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 8 }} />
          </label>
        ...
        <div style={{ background: '#111827', borderRadius: 12, padding: 14, border: '1px solid #1f2937' }}>
          <div style={{ fontWeight: 700, marginBottom: 6, color: '#e5e7eb' }}>Access & Security</div>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, color: '#cbd5f5', fontSize: 14 }}>
            IP allow list (comma-separated)
            <textarea value={ipAllow} onChange={e => setIpAllow(e.target.value)} rows={2} style={{ background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 8 }} />
          </label>
          <label style={{ display: 'flex', flexDirection: 'column', gap: 6, color: '#cbd5f5', fontSize: 14, marginTop: 8 }}>
            IP deny list (comma-separated)
            <textarea value={ipDeny} onChange={e => setIpDeny(e.target.value)} rows={2} style={{ background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 8 }} />
          </label>
          { (selectedWebhookId || selectedApiId) && (
            <div style={{ marginTop: 10, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
              ...
              {signaturePreview && <span style={{ color: '#cbd5f5', fontSize: 13 }}>New secret ({signaturePreview.method}): {signaturePreview.secret}</span>}
            </div>
          )}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 12, display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))', gap: 10 }}>
        <div style={{ background: '#111827', border: '1px solid #1f2937', borderRadius: 12, padding: 12 }}>
          ...
          <textarea value={schemaJson} onChange={e => setSchemaJson(e.target.value)} rows={6} style={{ width: '100%', background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 10 }} />
        ...
        <div style={{ background: '#111827', border: '1px solid #1f2937', borderRadius: 12, padding: 12 }}>
          ...
          <textarea value={testPayload} onChange={e => setTestPayload(e.target.value)} rows={5} style={{ width: '100%', background: '#0f172a', color: '#e5e7eb', border: '1px solid #1f2937', borderRadius: 10, padding: 10 }} />
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginTop: 6 }}>
            ...
            {recentPayloads.map((p, idx) => (
              <button key={idx} onClick={() => setTestPayload(p)} style={{ background: '#1f2937', color: '#cbd5f5', border: '1px solid #334155', borderRadius: 10, padding: '6px 10px', fontSize: 12, cursor: 'pointer' }}>Load recent #{idx + 1}</button>
            ))}
          </div>
          {simulation && (
      ...
      <div style={{ marginBottom: 20, background: '#111827', border: '1px solid #1f2937', borderRadius: 12, padding: 12 }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8, color: '#e5e7eb' }}>Webhooks</div>
        <div style={{ fontSize: 15, color: '#cbd5f5' }}>
          {loading ? <span>Loading…</span> : webhooks.length ? (
            <div style={{ display: 'grid', gap: 10 }}>
              {webhooks.map(w => (
                <div key={w.id} style={{ padding: 12, borderRadius: 10, border: '1px solid #1f2937', background: '#0b1221' }}>
                  <div style={{ fontWeight: 700, color: '#e5e7eb' }}>{w.url || 'Webhook'} <span style={{ color: '#9ca3af', fontWeight: 500 }}>· {w.env}/{w.version} · {w.status} · rev {w.revision}</span></div>
                  <div style={{ color: '#cbd5f5', fontSize: 14 }}>Enabled: {w.enabled === false ? 'No' : 'Yes'} · Guardrails: rate {w.guardrails?.rateLimit ?? '—'} req/min, concurrency {w.guardrails?.concurrencyLimit ?? '—'}, circuit breaker {w.guardrails?.circuitBreakerEnabled ? 'on' : 'off'}</div>
                  <div style={{ color: '#9ca3af', fontSize: 13 }}>Canary: {w.canaryPercent ?? 0}% · Shadow: {w.shadowMode ? 'on' : 'off'} · Perf budget: {w.performanceBudgetMs || '—'}ms</div>
                  <div style={{ color: '#9ca3af', fontSize: 13 }}>IP allow: {(w.ipAllow || []).join(', ') || '—'} · IP deny: {(w.ipDeny || []).join(', ') || '—'}</div>
                  <div style={{ color: '#9ca3af', fontSize: 13 }}>Last success: {w.lastSuccessAt || '—'} · Last failure: {w.lastFailureAt || '—'}</div>
                  {w.warnings?.length ? (
      ...
      <div style={{ marginBottom: 20, background: '#111827', border: '1px solid #1f2937', borderRadius: 12, padding: 12 }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8, color: '#e5e7eb' }}>APIs</div>
        <div style={{ fontSize: 15, color: '#cbd5f5' }}>
          {loading ? <span>Loading…</span> : apis.length ? (
            <div style={{ display: 'grid', gap: 10 }}>
              {apis.map(a => (
                <div key={a.id} style={{ padding: 12, borderRadius: 10, border: '1px solid #1f2937', background: '#0b1221' }}>
                  <div style={{ fontWeight: 700, color: '#e5e7eb' }}>{a.endpoint || 'API'} <span style={{ color: '#9ca3af', fontWeight: 500 }}>· {a.env}/{a.version} · {a.status} · rev {a.revision}</span></div>
                  <div style={{ color: '#cbd5f5', fontSize: 14 }}>Enabled: {a.enabled === false ? 'No' : 'Yes'} · Guardrails: rate {a.guardrails?.rateLimit ?? '—'} req/min, concurrency {a.guardrails?.concurrencyLimit ?? '—'}, circuit breaker {a.guardrails?.circuitBreakerEnabled ? 'on' : 'off'}</div>
                  <div style={{ color: '#9ca3af', fontSize: 13 }}>Canary: {a.canaryPercent ?? 0}% · Shadow: {a.shadowMode ? 'on' : 'off'} · Perf budget: {a.performanceBudgetMs || '—'}ms</div>
                  <div style={{ color: '#9ca3af', fontSize: 13 }}>IP allow: {(a.ipAllow || []).join(', ') || '—'} · IP deny: {(a.ipDeny || []).join(', ') || '—'}</div>
                  <div style={{ color: '#9ca3af', fontSize: 13 }}>Last success: {a.lastSuccessAt || '—'} · Last failure: {a.lastFailureAt || '—'}</div>
                  {a.warnings?.length ? (
                    <div style={{ color: '#fbbf24', fontSize: 14 }}>
                      <span style={{ background: '#f59e0b22', border: '1px solid #f59e0b55', borderRadius: 8, padding: '2px 8px', marginRight: 6 }}>At risk</span>
      ...
      <div style={{ marginBottom: 24, background: '#111827', border: '1px solid #1f2937', borderRadius: 12, padding: 12 }}>
      ...
      {(selectedWebhookId || selectedApiId) && (
        <div style={{ marginBottom: 20, background: '#0b1221', border: '1px solid #1f2937', borderRadius: 12, padding: 12 }}>
      ...
      <div style={{ marginBottom: 20, background: '#0b1221', border: '1px solid #1f2937', borderRadius: 12, padding: 12 }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8, color: '#e5e7eb' }}>Trash</div>
        <div style={{ display: 'grid', gap: 10 }}>
          {(trash.webhooks?.length ? trash.webhooks : []).map(t => (
            <div key={t.id} style={{ border: '1px dashed #334155', borderRadius: 10, padding: 10, color: '#cbd5f5' }}>
          ...
          {!trash.webhooks?.length && !trash.apis?.length && <div style={{ color: '#9ca3af' }}>Trash is empty.</div>}
        </div>
      </div>

      {lastDeleted && (
        <div style={{ marginBottom: 16, background: '#0b1221', border: '1px solid #1f2937', borderRadius: 12, padding: 12, display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 12 }}>
