...
export default function WebhookApiTriggers() {
  ...
  const handleImport = e => {
    ...
    reader.onload = evt => {
      setWebhooks(JSON.parse(evt.target.result));
  ...
  return (
    <div style={{ padding: 24 }}>
      <h2 style={{ fontWeight: 800, fontSize: 32, marginBottom: 18 }}>Webhook & API Triggers</h2>
      ...
      {/* Input for webhook or API config */}
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>Webhook/API Config</div>
        <input value={input} onChange={e => setInput(e.target.value)} placeholder="Enter webhook URL or API details..." style={{ fontSize: 16, padding: 8, borderRadius: 8, border: '1px solid #ccc', width: 420, marginBottom: 18 }} aria-label="Webhook/API Config" />
        <button style={{ background: '#0ea5e9', color: '#fff', border: 'none', borderRadius: 8, padding: '7px 18px', fontWeight: 600, fontSize: 15, cursor: 'pointer', marginLeft: 12 }}>Add Config</button>
      </div>
      {/* Webhooks Table */}
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>Webhooks</div>
        <div style={{ fontSize: 15, color: '#23263a' }}>
          {webhooks.length ? (
            <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', background: 'none', padding: 0, margin: 0 }}>{JSON.stringify(webhooks, null, 2)}</pre>
          ) : (
      ...
      {/* APIs Table */}
      <div style={{ marginBottom: 32 }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>APIs</div>
        <div style={{ fontSize: 15, color: '#23263a' }}>
          {apis.length ? (
            <pre style={{ whiteSpace: 'pre-wrap', wordBreak: 'break-word', background: 'none', padding: 0, margin: 0 }}>{JSON.stringify(apis, null, 2)}</pre>
          ) : (
