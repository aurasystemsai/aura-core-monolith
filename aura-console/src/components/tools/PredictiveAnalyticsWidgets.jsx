...
const Sparkline = ({ series, metric, reduceMotion }) => {
 ...
 <polyline
 fill="none"stroke="#22d3ee"strokeWidth="3"points={points.join("")}
...
export default function PredictiveAnalyticsWidgets() {
 ...
 <div
 key={widget.metric}
 className="paw-card"aria-label={`${widget.metric} widget`}
 ...
 {widget.metric === "churn"? `${(widget.current * 100).toFixed(1)}%`
 : widget.metric === "ltv"? `$${widget.current.toFixed(2)}`
 : widget.current}
 </div>
 ...
 className="paw-forecast-point"title={`${metric.toUpperCase()} on ${point.date}: ${point.predicted} (CI ${point.lower}-${point.upper})`}
 >
 <div className="paw-forecast-date">{point.date.slice(5)}</div>
 ...
 <polyline
 fill="none"stroke="#22d3ee"strokeWidth="2"points={telemetryTrend.points}
 strokeLinejoin="round"strokeLinecap="round"/>
 </svg>
 ...
 <select
 id="granularity"value={granularity}
 onChange={(e) => {
 ...
 <select
 id="peer-set"value={benchmarkPeerSet}
 onChange={(e) => {
 ...
 <button
 key={metric}
 type="button"className={`paw-chip-btn ${metricChips.includes(metric) ? "paw-chip-btn--active": ""}`}
 ...
 <input
 id="customMetrics"name="customMetrics"placeholder="Enter additional metrics (comma or newline separated)"value={form.customMetrics}
 ...
 <input
 id="alertThreshold"name="alertThreshold"type="number"min="0"max="100"value={form.alertThreshold}
 ...
 <input
 id="alertEmails"name="alertEmails"placeholder="ops@example.com, cmo@example.com"value={form.alertEmails}
 ...
 <input
 placeholder="Paste snapshot token"value={snapshotToken}
 ...
 <input
 placeholder="Email recipients (comma)"value={(routing[sev]?.email || []).join(", ")}
 onChange={(e) => handleRoutingChange(sev, "email", parseEmails(e.target.value))}
 />
 <input
 placeholder="Slack channels (comma)"value={(routing[sev]?.slack || []).join(", ")}
 onChange={(e) => handleRoutingChange(sev, "slack", parseEmails(e.target.value))}
 />
 <input
 placeholder="Webhook URLs (comma)"value={(routing[sev]?.webhook || []).join(", ")}
 ...
 Demand change (%):
 <input
 type="range"min="-40"max="40"step="5"value={scenario.demandDelta}
 onChange={(e) => {
 ...
 Budget change (%):
 <input
 type="range"min="-40"max="40"step="5"value={scenario.budgetDelta}
 onChange={(e) => {
 const next = Number(e.target.value);
 ...
 <div className="paw-inline-input">
 <input
 placeholder="Save current scenario as..."value={scenarioSaveName}
 onChange={(e) => setScenarioSaveName(e.target.value)}
 />
 <input
 placeholder="Tag (optional)"value={scenarioSaveTag}
 ...
 <button
 type="button"className="paw-chip-btn paw-chip-btn--danger"onClick={() => deleteSavedScenario(s.name)}
 ...
 <div
 key={idx}
 className="paw-run-bar"style={{ height }}
 ...
 <button
 key={sev}
 type="button"className={`paw-chip-btn ${anomalySeverity === sev ? "paw-chip-btn--active": ""}`}
 ...
 <div className="paw-actions-row">
 <button
 className="paw-button paw-button--ghost"type="button"onClick={() => exportCsv(filteredAnomalies, ["metric", "severity", "deltaPercent", "message"], "anomalies.csv")}
 >
 Export anomalies CSV
 </button>
 <button
 className="paw-button paw-button--ghost"type="button"onClick={() => downloadJsonFromRows(filteredAnomalies, "anomalies.json")}
 >
