...
export default function ReportingIntegrations() {
  ...
  return (
    <div style={{ padding: 24 }}>
      {devSandbox && (
        <div style={{ background: "#252638", border: "1px solid #2e3045", borderRadius: 12, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 12 }}>
          <div>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => setEnv("stage")} style={{ background: "#2e3045", color: "#f9fafb", borderRadius: 10, padding: "10px 14px", fontWeight: 800, cursor: "pointer", border: "1px solid #525578" }}>Switch to Stage</button>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        <select value={env} onChange={e => setEnv(e.target.value)} style={{ background: "#252638", color: "#f9fafb", border: "1px solid #2e3045", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
          <option value="dev">Dev</option>
          <option value="stage">Stage</option>
          <option value="prod">Prod</option>
        </select>
        <select value={activeShop} onChange={e => setActiveShop(e.target.value)} style={{ background: "#252638", color: "#f9fafb", border: "1px solid #2e3045", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
          {shops.map(s => <option key={s.domain} value={s.domain}>{s.domain}</option>)}
        </select>
        <select value={role} onChange={e => setRole(e.target.value)} style={{ background: '#252638', color: '#f9fafb', border: '1px solid #2e3045', borderRadius: 8, padding: '8px 12px', fontWeight: 700 }}>
        ...
        <input value={owner} onChange={e => setOwner(e.target.value)} style={{ background: '#252638', color: '#f9fafb', border: '1px solid #2e3045', borderRadius: 8, padding: '7px 10px', minWidth: 200 }} placeholder="owner email" />
      </div>
      {showOnboarding && onboardingContent}
      {/* Health */}
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12, minWidth: 220 }}>
        ...
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12, minWidth: 220 }}>
          ...
          <button onClick={enforcePerfBudget} disabled={isReadOnly} style={{ marginTop: 6, background: '#252638', color: isReadOnly ? '#525578' : '#22c55e', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Enforce budget</button>
        </div>
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12, minWidth: 220 }}>
          ...
          <button onClick={() => setRateLimit(r => ({ ...r, used: Math.max(0, r.used - 5) }))} style={{ marginTop: 6, background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Pause & backoff</button>
      ...
      <div style={{ marginBottom: 16, display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ display: 'grid', gap: 8, marginTop: 6 }}>
            {channelsSlo.map(c => (
              <div key={c.channel} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
        ...
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ display: 'grid', gap: 6, marginTop: 6, color: '#f9fafb' }}>
            ...
            <div>Drop policy: <select value={backpressure.dropPolicy} onChange={e => setBackpressure(b => ({ ...b, dropPolicy: e.target.value }))} style={{ background: '#252638', color: '#f9fafb', border: '1px solid #2e3045', borderRadius: 6, padding: '4px 6px' }}>
        ...
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          ...
          <button onClick={runContractTests} style={{ marginTop: 6, background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Run tests</button>
          <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
            {contractTests.map((t, idx) => (
              <div key={idx} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045' }}>
        ...
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          ...
          <button onClick={calculateDrift} style={{ marginTop: 8, background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Check drift</button>
          ...
          <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={snapshotConfigs} disabled={isReadOnly} style={{ background: '#252638', color: isReadOnly ? '#525578' : '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Snapshot now</button>
              <button onClick={() => restoreConfigSnapshot()} disabled={isReadOnly || configSnapshots.length === 0} style={{ background: '#252638', color: isReadOnly ? '#525578' : '#fbbf24', border: '1px dashed #f59e0b', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Restore last</button>
            </div>
            <div style={{ display: 'grid', gap: 6 }}>
              {configSnapshots.map(s => (
                <div key={s.id} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045', display: 'grid', gap: 4 }}>
                  ...
                  <button onClick={() => restoreConfigSnapshot(s.id)} disabled={isReadOnly} style={{ background: '#252638', color: isReadOnly ? '#525578' : '#22c55e', border: '1px solid #525578', borderRadius: 6, padding: '6px 8px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Restore</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ marginTop: 6, display: 'grid', gap: 6 }}>
            ...
            <button onClick={adjustRatePlan} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Recalculate</button>
          </div>
        </div>
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 800, color: '#f9fafb' }}>Retry policy</div>
          <div style={{ display: 'grid', gap: 6, marginTop: 6, color: '#f9fafb' }}>
            <div>Strategy: <select value={retryPolicy.strategy} onChange={e => setRetryPolicy(r => ({ ...r, strategy: e.target.value }))} style={{ background: '#252638', color: '#f9fafb', border: '1px solid #2e3045', borderRadius: 6, padding: '4px 6px' }}>
            ...
            <button onClick={applyRetryPolicy} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Apply</button>
          </div>
        </div>
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 800, color: '#f9fafb' }}>Synthetic checks</div>
          <button onClick={runSynthetic} style={{ marginTop: 6, background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Run synthetic</button>
          <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
          ...
          <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gap: 6 }}>
              {Object.entries(daypartHealth).map(([label, pct]) => (
                <div key={label} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={refreshDaypartHealth} style={{ background: '#252638', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Refresh</button>
        ...
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          ...
          <button onClick={checkTenantIsolation} style={{ marginTop: 6, background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Run check</button>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {["email", "sms", "webhook"].map(ch => (
              <button key={ch} onClick={() => setChannelView(ch)} style={{ background: channelView === ch ? '#6366f1' : '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>{ch.toUpperCase()}</button>
            ))}
          </div>
        </div>
        <div style={{ fontSize: 15, color: '#0d0d11' }}>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ marginTop: 10, background: '#252638', border: '1px solid #2e3045', borderRadius: 10, padding: 10 }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {webhooks.map(wh => (
              <div key={wh.topic} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap', background: '#252638', borderRadius: 8, padding: '8px 10px', border: '1px solid #2e3045' }}>
                <div>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  <button onClick={() => replayWebhook(wh.topic)} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Replay</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ marginTop: 10, background: '#252638', border: '1px solid #2e3045', borderRadius: 10, padding: 10 }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {deliveries.map(d => (
              <div key={d.id} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <button onClick={() => replayDeliveryWithOverride(d.id, 200)} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Replay</button>
      ...
      <div style={{ marginBottom: 24, background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={() => applyPresetPayload('email')} style={{ background: '#252638', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Email preset</button>
            ...
            <button onClick={() => applyPresetPayload('slack')} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Incident preset</button>
          </div>
        </div>
        <textarea value={inlinePayload} onChange={e => setInlinePayload(e.target.value)} rows={4} style={{ width: '100%', background: '#252638', color: '#f9fafb', border: '1px solid #2e3045', borderRadius: 10, padding: 10, fontFamily: 'monospace' }} />
        <div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button onClick={validateInlinePayload} style={{ background: '#252638', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Validate</button>
          <button onClick={simulateSend} disabled={isReadOnly} style={{ background: '#22c55e', color: isReadOnly ? '#525578' : '#252638', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 800 }}>Simulate send</button>
        ...
        <div style={{ marginTop: 6, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', color: '#a8adc4', fontSize: 12 }}>
          ...
          {['email','sms','webhook'].map(ch => (
            <label key={ch} style={{ display: 'flex', gap: 6, alignItems: 'center', background: '#252638', border: '1px solid #2e3045', borderRadius: 8, padding: '4px 8px' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <button onClick={() => setSyncHealth({ status: 'healthy', lastSuccess: Date.now(), lastError: null })} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '7px 14px', fontWeight: 700, cursor: 'pointer', marginLeft: 8 }}>Force sync</button>
      ...
      <div style={{ marginBottom: 24, background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
        ...
        <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {dlq.length === 0 ? (
            ...
            <div key={item.id} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <div>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <button onClick={() => moveDlqToQueue(item.id)} style={{ background: '#22c55e', color: '#252638', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Requeue</button>
                  <button onClick={() => ackDlq(item.id)} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>Ack</button>
                </div>
            </div>
          ))}
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <button onClick={runMockCheck} style={{ background: '#252638', color: '#fbbf24', border: '1px dashed #f59e0b', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Check mock server</button>
              <button onClick={drainDlq} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Drain DLQ</button>
      ...
      {importValidation && (
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12, marginBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          {pendingImport && (
            <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
              ...
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 220, background: '#252638', borderRadius: 8, padding: 10, border: '1px solid #2e3045' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
            <button onClick={() => setPendingImport(null)} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>Discard</button>
            <button disabled={!pendingImport || (importValidation.issues && importValidation.issues.some(i => i.toLowerCase().includes('must') || i.toLowerCase().includes('invalid')))} onClick={applyPendingImport} style={{ background: pendingImport ? '#22c55e' : '#525578', color: pendingImport ? '#252638' : '#525578', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: pendingImport ? 'pointer' : 'not-allowed' }}>Apply import</button>
          </div>
        </div>
      )}
            ...
            {history.slice(0, 3).map((h, idx) => (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap', background: '#252638', border: '1px solid #2e3045', borderRadius: 10, padding: '8px 10px' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => { restoreSnapshot(h); setTimeout(() => handleExport(), 0); }} style={{ background: '#22c55e', color: '#252638', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Export</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ marginBottom: 18, background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {Object.entries(channelCosts).map(([ch, stats]) => (
            <div key={ch} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#252638', borderRadius: 8, padding: '8px 10px', border: '1px solid #2e3045' }}>
              ...
              <button onClick={() => setChannelCosts(prev => ({ ...prev, [ch]: { ...stats, errors: Math.max(0, stats.errors - 1) } }))} style={{ background: '#2e3045', color: '#f9fafb', border: '1px solid #525578', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>Ack error</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 24, background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
        ...
        <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {secrets.map(s => (
            <div key={s.key} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <div>
        ...
        <div style={{ background: '#252638', border: '1px solid #2e3045', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
            {classification.map((c, idx) => (
              <div key={idx} style={{ background: '#252638', borderRadius: 8, padding: 8, border: '1px solid #2e3045', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
      ...
      <form onSubmit={e => { e.preventDefault(); handleFeedback(); }} style={{ marginTop: 32, background: '#3d4058', borderRadius: 12, padding: 20 }} aria-label="Send feedback">
        ...
        {error && (
          <div style={{ color: '#ef4444', marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <span>{error}</span>
            {quickFixForIssue(error) === 'retry' && <button onClick={() => { setError(''); handleFeedback(); }} style={{ background: '#22c55e', color: '#252638', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Retry</button>}
            {quickFixForIssue(error) === 'reset' && <button onClick={() => setNotifications([])} style={{ background: '#22c55e', color: '#252638', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Reset data</button>}
          </div>
        )}
      </form>
