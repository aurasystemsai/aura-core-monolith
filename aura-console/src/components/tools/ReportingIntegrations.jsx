...
export default function ReportingIntegrations() {
  ...
  return (
    <div style={{ padding: 24 }}>
      {devSandbox && (
        <div style={{ background: "#282828", border: "1px solid #343434", borderRadius: 12, padding: 12, display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 12 }}>
          <div>
            <div style={{ fontWeight: 800, color: "#f59e0b" }}>Sandbox mode</div>
            <div style={{ color: "#9a9a9a", fontSize: 13 }}>API-backed actions (feedback) are blocked in dev. Switch to Stage/Prod to send.</div>
          </div>
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            <button onClick={() => setEnv("stage")} style={{ background: "#343434", color: "#f0f0f0", borderRadius: 10, padding: "10px 14px", fontWeight: 800, cursor: "pointer", border: "1px solid #4a4a4a" }}>Switch to Stage</button>
      ...
      <div style={{ display: "flex", gap: 10, alignItems: "center", flexWrap: "wrap", marginBottom: 12 }}>
        <select value={env} onChange={e => setEnv(e.target.value)} style={{ background: "#282828", color: "#f0f0f0", border: "1px solid #343434", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
          <option value="dev">Dev</option>
          <option value="stage">Stage</option>
          <option value="prod">Prod</option>
        </select>
        <select value={activeShop} onChange={e => setActiveShop(e.target.value)} style={{ background: "#282828", color: "#f0f0f0", border: "1px solid #343434", borderRadius: 8, padding: "8px 12px", fontWeight: 700 }}>
          {shops.map(s => <option key={s.domain} value={s.domain}>{s.domain}</option>)}
        </select>
        <select value={role} onChange={e => setRole(e.target.value)} style={{ background: '#282828', color: '#f0f0f0', border: '1px solid #343434', borderRadius: 8, padding: '8px 12px', fontWeight: 700 }}>
        ...
        <div style={{ color: '#9a9a9a', fontSize: 13 }}>Owner routing</div>
        <input value={owner} onChange={e => setOwner(e.target.value)} style={{ background: '#282828', color: '#f0f0f0', border: '1px solid #343434', borderRadius: 8, padding: '7px 10px', minWidth: 200 }} placeholder="owner email" />
      </div>
      {showOnboarding && onboardingContent}
      {/* Health */}
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12, minWidth: 220 }}>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Sync health</div>
          <div style={{ fontWeight: 800, color: syncHealth.status === 'healthy' ? '#22c55e' : '#f87171' }}>{syncHealth.status}</div>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Last success: {syncHealth.lastSuccess ? new Date(syncHealth.lastSuccess).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}</div>
          {syncHealth.lastError && <div style={{ color: '#f87171', fontSize: 12 }}>Last error: {syncHealth.lastError}</div>}
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12, minWidth: 220 }}>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>SLOs</div>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Next sync: {slo.nextSync ? new Date(slo.nextSync).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}</div>
          <div style={{ color: '#9a9a9a', fontSize: 12, marginTop: 4 }}>Perf budget: {perfBudget.applyMs}ms apply · {perfBudget.syncMs}ms sync</div>
          <button onClick={enforcePerfBudget} disabled={isReadOnly} style={{ marginTop: 6, background: '#282828', color: isReadOnly ? '#555555' : '#22c55e', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Enforce budget</button>
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12, minWidth: 220 }}>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Shopify API</div>
          <div style={{ color: '#f0f0f0', fontWeight: 800 }}>{activeShop}</div>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Call limit: {rateLimit.used}/{rateLimit.total} · Cost: {apiCost.current}/{apiCost.limit}</div>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Restore rate: +{apiCost.restoreRate}/s</div>
          <button onClick={() => setRateLimit(r => ({ ...r, used: Math.max(0, r.used - 5) }))} style={{ marginTop: 6, background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Pause & backoff</button>
          ...
          {(syncProgress > 0 || replayProgress > 0) && (
            <div style={{ marginTop: 6, color: '#9a9a9a', fontSize: 12 }}>Progress: sync {syncProgress}% · replay {replayProgress}%</div>
          )}
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12, minWidth: 220 }}>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Session</div>
      ...
      <div style={{ marginBottom: 16, display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ display: 'grid', gap: 8, marginTop: 6 }}>
            {channelsSlo.map(c => (
              <div key={c.channel} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{c.channel.toUpperCase()}</div>
                  <div style={{ color: '#9a9a9a', fontSize: 12 }}>Latency {c.latency}ms · Error {c.errorRate}%</div>
                </div>
        ...
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 13, marginTop: 4 }}>Canary rollout with auto-rollback on error</div>
          <div style={{ marginTop: 8, display: 'grid', gap: 8 }}>
            <label style={{ color: '#f0f0f0', fontWeight: 700 }}>Traffic %: {rollout.percent}%</label>
            <input type="range" min="0" max="100" value={rollout.percent} onChange={e => setRollout(r => ({ ...r, percent: Number(e.target.value) }))} />
            <label style={{ color: '#f0f0f0', fontWeight: 700 }}>Error threshold %: {rollout.errorThreshold}%</label>
            ...
            <div style={{ color: '#9a9a9a', fontSize: 12 }}>Auto-rollback if error &gt; {rollout.errorThreshold}% during canary.</div>
          </div>
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ display: 'grid', gap: 6, marginTop: 6, color: '#f0f0f0' }}>
            ...
            <div>Drop policy: <select value={backpressure.dropPolicy} onChange={e => setBackpressure(b => ({ ...b, dropPolicy: e.target.value }))} style={{ background: '#282828', color: '#f0f0f0', border: '1px solid #343434', borderRadius: 6, padding: '4px 6px' }}>
              <option value="oldest">Drop oldest</option>
              <option value="newest">Drop newest</option>
              <option value="block">Block</option>
            </select></div>
            <div style={{ color: '#9a9a9a', fontSize: 12 }}>Backlog: {backpressure.queueDepth} · Drop: {backpressure.dropPolicy}</div>
          </div>
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 800, color: '#f0f0f0' }}>Contract tests</div>
          <button onClick={runContractTests} style={{ marginTop: 6, background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Run tests</button>
          <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
            {contractTests.map((t, idx) => (
              <div key={idx} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434' }}>
                <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{t.name}</div>
                <div style={{ color: t.status === 'pass' ? '#22c55e' : t.status === 'fail' ? '#f87171' : '#9a9a9a', fontSize: 13 }}>{t.status}{t.latency ? ` · ${t.latency}ms` : ''}</div>
              </div>
            ))}
          </div>
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 13 }}>Compare {env} vs stage for config drift.</div>
          <button onClick={calculateDrift} style={{ marginTop: 8, background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Check drift</button>
          {envDrift && <div style={{ marginTop: 6, color: envDrift.diff === 0 ? '#22c55e' : '#fbbf24', fontSize: 13 }}>{envDrift.summary}</div>}
          <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
            <div style={{ color: '#9a9a9a', fontSize: 12 }}>Config snapshots</div>
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={snapshotConfigs} disabled={isReadOnly} style={{ background: '#282828', color: isReadOnly ? '#555555' : '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Snapshot now</button>
              <button onClick={() => restoreConfigSnapshot()} disabled={isReadOnly || configSnapshots.length === 0} style={{ background: '#282828', color: isReadOnly ? '#555555' : '#fbbf24', border: '1px dashed #f59e0b', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Restore last</button>
            </div>
            <div style={{ display: 'grid', gap: 6 }}>
              {configSnapshots.map(s => (
                <div key={s.id} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434', display: 'grid', gap: 4 }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{s.label}</div>
                    <div style={{ color: '#9a9a9a', fontSize: 12 }}>{s.created}</div>
                  </div>
                  <div style={{ color: s.drift === 0 ? '#22c55e' : '#fbbf24', fontSize: 12 }}>Drift: {s.drift}</div>
                  <button onClick={() => restoreConfigSnapshot(s.id)} disabled={isReadOnly} style={{ background: '#282828', color: isReadOnly ? '#555555' : '#22c55e', border: '1px solid #4a4a4a', borderRadius: 6, padding: '6px 8px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 700 }}>Restore</button>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 13 }}>Forecast calls vs Shopify cost headers</div>
          <div style={{ marginTop: 6, display: 'grid', gap: 6 }}>
            <div style={{ color: '#f0f0f0' }}>Forecast: {ratePlan.forecast} calls/min</div>
            <div style={{ color: '#f0f0f0' }}>Suggested: {ratePlan.suggested} calls/min</div>
            <button onClick={adjustRatePlan} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Recalculate</button>
          </div>
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 800, color: '#f0f0f0' }}>Retry policy</div>
          <div style={{ display: 'grid', gap: 6, marginTop: 6, color: '#f0f0f0' }}>
            <div>Strategy: <select value={retryPolicy.strategy} onChange={e => setRetryPolicy(r => ({ ...r, strategy: e.target.value }))} style={{ background: '#282828', color: '#f0f0f0', border: '1px solid #343434', borderRadius: 6, padding: '4px 6px' }}>
            ...
            <button onClick={applyRetryPolicy} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Apply</button>
          </div>
        </div>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          <div style={{ fontWeight: 800, color: '#f0f0f0' }}>Synthetic checks</div>
          <button onClick={runSynthetic} style={{ marginTop: 6, background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Run synthetic</button>
          <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
            {syntheticChecks.map((s, idx) => (
              <div key={idx} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434' }}>
                <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{s.name}</div>
                <div style={{ color: s.status === 'ok' ? '#22c55e' : s.status === 'fail' ? '#f87171' : '#9a9a9a', fontSize: 13 }}>{s.status}{s.latency ? ` · ${s.latency}ms` : ''}</div>
              </div>
            ))}
          </div>
          <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
            <div style={{ color: '#9a9a9a', fontSize: 12 }}>Daypart health</div>
            <div style={{ display: 'grid', gap: 6 }}>
              {Object.entries(daypartHealth).map(([label, pct]) => (
                <div key={label} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
              <button onClick={refreshDaypartHealth} style={{ background: '#282828', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Refresh</button>
        ...
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 13 }}>Check cross-shop leakage risks</div>
          <button onClick={checkTenantIsolation} style={{ marginTop: 6, background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Run check</button>
      ...
      <div style={{ marginBottom: 32 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {["email", "sms", "webhook"].map(ch => (
              <button key={ch} onClick={() => setChannelView(ch)} style={{ background: channelView === ch ? '#6366f1' : '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>{ch.toUpperCase()}</button>
            ))}
          </div>
        </div>
        <div style={{ fontSize: 15, color: '#141414' }}>
        ...
        <div style={{ marginTop: 10, background: '#282828', border: '1px solid #343434', borderRadius: 10, padding: 10 }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 13 }}>Retries: auto with backoff · Suppress in sandbox: {devSandbox ? 'on' : 'off'}</div>
      ...
      <div style={{ marginBottom: 32 }}>
        ...
        <div style={{ marginTop: 10, background: '#282828', border: '1px solid #343434', borderRadius: 10, padding: 10 }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {webhooks.map(wh => (
              <div key={wh.topic} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap', background: '#282828', borderRadius: 8, padding: '8px 10px', border: '1px solid #343434' }}>
                <div>
                  <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{wh.topic}</div>
                  <div style={{ color: '#9a9a9a', fontSize: 12 }}>Last delivery: {wh.lastDelivery} · {wh.endpoint}</div>
                </div>
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  <div style={{ color: wh.status === 'ok' ? '#22c55e' : '#fbbf24', fontWeight: 700 }}>{wh.status}</div>
                  <button onClick={() => replayWebhook(wh.topic)} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Replay</button>
                </div>
              </div>
            ))}
          </div>
        ...
        <div style={{ marginTop: 10, background: '#282828', border: '1px solid #343434', borderRadius: 10, padding: 10 }}>
          ...
          <div style={{ display: 'grid', gap: 8 }}>
            {deliveries.map(d => (
              <div key={d.id} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <div>
                  <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{d.topic}</div>
                  <div style={{ color: '#9a9a9a', fontSize: 12 }}>Status {d.status} · {d.latency}ms · {d.when}</div>
                  <div style={{ color: d.outcome === 'ok' ? '#22c55e' : d.outcome === 'retrying' ? '#fbbf24' : '#f87171', fontSize: 12 }}>{d.outcome}</div>
                </div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <button onClick={() => replayDeliveryWithOverride(d.id, 200)} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Replay</button>
      ...
      <div style={{ marginBottom: 24, background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap', marginBottom: 8 }}>
          ...
          <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
            <button onClick={() => applyPresetPayload('email')} style={{ background: '#282828', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Email preset</button>
            ...
            <button onClick={() => applyPresetPayload('slack')} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Incident preset</button>
          </div>
        </div>
        <textarea value={inlinePayload} onChange={e => setInlinePayload(e.target.value)} rows={4} style={{ width: '100%', background: '#282828', color: '#f0f0f0', border: '1px solid #343434', borderRadius: 10, padding: 10, fontFamily: 'monospace' }} />
        <div style={{ marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
          <button onClick={validateInlinePayload} style={{ background: '#282828', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Validate</button>
          <button onClick={simulateSend} disabled={isReadOnly} style={{ background: '#22c55e', color: isReadOnly ? '#555555' : '#282828', border: 'none', borderRadius: 8, padding: '6px 10px', cursor: isReadOnly ? 'not-allowed' : 'pointer', fontWeight: 800 }}>Simulate send</button>
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Endpoint: {mockEndpoint}</div>
        ...
        <div style={{ marginTop: 6, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center', color: '#9a9a9a', fontSize: 12 }}>
          <span>Channel breakers</span>
          {['email','sms','webhook'].map(ch => (
            <label key={ch} style={{ display: 'flex', gap: 6, alignItems: 'center', background: '#282828', border: '1px solid #343434', borderRadius: 8, padding: '4px 8px' }}>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <button onClick={() => setSyncHealth({ status: 'healthy', lastSuccess: Date.now(), lastError: null })} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '7px 14px', fontWeight: 700, cursor: 'pointer', marginLeft: 8 }}>Force sync</button>
      ...
      <div style={{ marginBottom: 24, background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <label style={{ display: 'flex', gap: 6, alignItems: 'center', color: '#9a9a9a', fontSize: 13 }}>
        ...
        <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {dlq.length === 0 ? (
            ...
            <div key={item.id} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <div>
                <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{item.id}</div>
                <div style={{ color: '#9a9a9a', fontSize: 12 }}>Reason {item.reason} · Attempts {item.attempts}</div>
                <div style={{ color: '#9a9a9a', fontSize: 12 }}>Payload: {item.payload}</div>
              </div>
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <button onClick={() => moveDlqToQueue(item.id)} style={{ background: '#22c55e', color: '#282828', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Requeue</button>
                  <button onClick={() => ackDlq(item.id)} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>Ack</button>
                </div>
            </div>
          ))}
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <button onClick={runMockCheck} style={{ background: '#282828', color: '#fbbf24', border: '1px dashed #f59e0b', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Check mock server</button>
              <button onClick={drainDlq} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', cursor: 'pointer', fontWeight: 700 }}>Drain DLQ</button>
            <div style={{ color: '#9a9a9a', fontSize: 12 }}>Status: {mockServer.status} · Body: {mockServer.body}</div>
      ...
      {importValidation && (
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12, marginBottom: 12 }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
            <div style={{ fontWeight: 800, color: '#f0f0f0' }}>Dry run results</div>
            {pendingImport && <div style={{ color: '#9a9a9a', fontSize: 12 }}>{pendingImport.fileName}</div>}
          </div>
          ...
          {importValidation.diff && (
            <div style={{ color: '#9a9a9a', fontSize: 13, marginTop: 6 }}>Diff: {importValidation.diff.summary}</div>
          )}
          {pendingImport && (
            <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
              <div style={{ color: '#f0f0f0', fontWeight: 700 }}>Preview</div>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <div style={{ flex: 1, minWidth: 220, background: '#282828', borderRadius: 8, padding: 10, border: '1px solid #343434' }}>
                  <div style={{ color: '#9a9a9a', fontSize: 12 }}>Current</div>
                  <div style={{ color: '#f0f0f0', fontWeight: 800 }}>{notifications.length} notifications</div>
                </div>
                <div style={{ flex: 1, minWidth: 220, background: '#282828', borderRadius: 8, padding: 10, border: '1px solid #343434' }}>
                  <div style={{ color: '#9a9a9a', fontSize: 12 }}>Incoming</div>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 12, flexWrap: 'wrap' }}>
            <button onClick={() => setPendingImport(null)} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>Discard</button>
            <button disabled={!pendingImport || (importValidation.issues && importValidation.issues.some(i => i.toLowerCase().includes('must') || i.toLowerCase().includes('invalid')))} onClick={applyPendingImport} style={{ background: pendingImport ? '#22c55e' : '#4a4a4a', color: pendingImport ? '#282828' : '#555555', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: pendingImport ? 'pointer' : 'not-allowed' }}>Apply import</button>
          </div>
        </div>
      )}
            ...
            {history.slice(0, 3).map((h, idx) => (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 10, flexWrap: 'wrap', background: '#282828', border: '1px solid #343434', borderRadius: 10, padding: '8px 10px' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  ...
                  <button onClick={() => { restoreSnapshot(h); setTimeout(() => handleExport(), 0); }} style={{ background: '#22c55e', color: '#282828', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Export</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <div style={{ marginBottom: 18, background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
        ...
        <div style={{ display: 'grid', gap: 8 }}>
          {Object.entries(channelCosts).map(([ch, stats]) => (
            <div key={ch} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#282828', borderRadius: 8, padding: '8px 10px', border: '1px solid #343434' }}>
              <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{ch.toUpperCase()}</div>
              <div style={{ color: '#9a9a9a', fontSize: 12 }}>Cost {stats.cost} · Errors {stats.errors}</div>
              <button onClick={() => setChannelCosts(prev => ({ ...prev, [ch]: { ...stats, errors: Math.max(0, stats.errors - 1) } }))} style={{ background: '#343434', color: '#f0f0f0', border: '1px solid #4a4a4a', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>Ack error</button>
            </div>
          ))}
        </div>
      </div>
      ...
      <div style={{ marginBottom: 24, background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Rotate and audit secrets by channel</div>
        </div>
        <div style={{ display: 'grid', gap: 8, marginTop: 8 }}>
          {secrets.map(s => (
            <div key={s.key} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
              <div>
                <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{s.key}</div>
                <div style={{ color: '#9a9a9a', fontSize: 12 }}>Rotated: {s.rotatedAt || '—'}</div>
      ...
      <div style={{ marginBottom: 24, display: 'grid', gap: 12, gridTemplateColumns: 'repeat(auto-fit, minmax(260px, 1fr))' }}>
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Mask PII/PCI fields in previews</div>
          <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
            {classification.map((c, idx) => (
              <div key={idx} style={{ background: '#282828', borderRadius: 8, padding: 8, border: '1px solid #343434', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                <div>
                  <div style={{ color: '#f0f0f0', fontWeight: 700 }}>{c.field}</div>
                  <div style={{ color: '#9a9a9a', fontSize: 12 }}>{c.tag}</div>
                </div>
        ...
        <div style={{ background: '#282828', border: '1px solid #343434', borderRadius: 12, padding: 12 }}>
          ...
          <div style={{ color: '#9a9a9a', fontSize: 12 }}>Upstream/downstream visibility</div>
          <div style={{ display: 'grid', gap: 6, marginTop: 8 }}>
      ...
      <form onSubmit={e => { e.preventDefault(); handleFeedback(); }} style={{ marginTop: 32, background: '#3c3c3c', borderRadius: 12, padding: 20 }} aria-label="Send feedback">
        ...
        {error && (
          <div style={{ color: '#ef4444', marginTop: 8, display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            <span>{error}</span>
            {quickFixForIssue(error) === 'retry' && <button onClick={() => { setError(''); handleFeedback(); }} style={{ background: '#22c55e', color: '#282828', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Retry</button>}
            {quickFixForIssue(error) === 'reset' && <button onClick={() => setNotifications([])} style={{ background: '#22c55e', color: '#282828', border: 'none', borderRadius: 8, padding: '6px 10px', fontWeight: 800, cursor: 'pointer' }}>Reset data</button>}
          </div>
        )}
      </form>
