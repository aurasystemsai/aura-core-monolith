    ...
    <div style={{ padding: 24 }}>
      ...
      <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', marginBottom: 16 }}>
        ...
        <div style={{ background: '#0b1221', border: '1px solid #1a1a1a', borderRadius: 12, padding: 12, minWidth: 220 }}>
          ...
          <div style={{ color: '#888888', fontSize: 12 }}>Restore rate: +{apiCost.restoreRate}/s</div>
          <button onClick={() => setRateLimit(r => ({ ...r, used: Math.max(0, r.used - 5) }))} style={{ marginTop: 6, background: '#1a1a1a', color: '#f0f0f0', border: '1px solid #333333', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Pause & backoff</button>
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 8, gap: 8, flexWrap: 'wrap' }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
            {["email", "sms", "webhook"].map(ch => (
              <button key={ch} onClick={() => setChannelView(ch)} style={{ background: channelView === ch ? '#6366f1' : '#1a1a1a', color: '#f0f0f0', border: '1px solid #333333', borderRadius: 8, padding: '6px 10px', fontWeight: 700, cursor: 'pointer' }}>{ch.toUpperCase()}</button>
            ))}
          </div>
        </div>
        <div style={{ fontSize: 15, color: '#0a0a0a' }}>
            ...
            {webhooks.map(wh => (
              <div key={wh.topic} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap', background: '#111827', borderRadius: 8, padding: '8px 10px', border: '1px solid #1a1a1a' }}>
                ...
                <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                  ...
                  <button onClick={() => replayWebhook(wh.topic)} style={{ background: '#1a1a1a', color: '#f0f0f0', border: '1px solid #333333', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Replay</button>
                </div>
              </div>
            ))}
          </div>
            ...
            {deliveries.map(d => (
              <div key={d.id} style={{ background: '#111827', borderRadius: 8, padding: 8, border: '1px solid #1a1a1a', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                ...
                <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                  <button onClick={() => replayDeliveryWithOverride(d.id, 200)} style={{ background: '#1a1a1a', color: '#f0f0f0', border: '1px solid #333333', borderRadius: 8, padding: '6px 10px', cursor: 'pointer' }}>Replay</button>
      ...
      <div style={{ marginBottom: 24 }}>
        ...
        <button onClick={() => setSyncHealth({ status: 'healthy', lastSuccess: Date.now(), lastError: null })} style={{ background: '#1a1a1a', color: '#f0f0f0', border: '1px solid #333333', borderRadius: 8, padding: '7px 14px', fontWeight: 700, cursor: 'pointer', marginLeft: 8 }}>Force sync</button>
      ...
      {importValidation && (
        <div style={{ background: '#0b1221', border: '1px solid #1a1a1a', borderRadius: 12, padding: 12, marginBottom: 12 }}>
          ...
          {importValidation.diff && (
            <div style={{ color: '#888888', fontSize: 13, marginTop: 6 }}>Diff: {importValidation.diff.summary}</div>
          )}
          {pendingImport && (
            <div style={{ marginTop: 10, display: 'grid', gap: 6 }}>
