...
function CharBar({ value = "", min, max, label, error }) {
  ...
  const colour = empty ? "#4b5780" : ok ? "#4ade80" : over ? "#f87171" : "#fbbf24";
  ...
  return (
    <div style={{ marginTop: 5 }}>
      ...
      <div style={{ height: 5, background: "#283044", borderRadius: 2 }}>
...
function ScoreRing({ score }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      <div style={{ width: 96, height: 96, borderRadius: "50%", background: `conic-gradient(${colour} ${score * 3.6}deg, #283044 ${score * 3.6}deg)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 74, height: 74, borderRadius: "50%", background: "#1f2433", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
...
function GooglePreview({ title, description, url }) {
  return (
    <div style={{ background: "#4f46e5", borderRadius: 10, padding: "14px 18px", fontFamily: "Arial, sans-serif", border: "1px solid #f6f7fb" }}>
      ...
      <div style={{ fontSize: 18, color: "#4b5780", marginBottom: 4, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{title || <span style={{ color: "#999" }}>Your Page Title</span>}</div>
...
export default function OnPageSEOEngine({ initialUrl, onUrlConsumed }) {
  ...
  return (
    <div style={{ background: "#1f2433", minHeight: "100vh", color: "#f6f7fb", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #1f2433 0%, #283044 100%)", borderBottom: "1px solid #283044", padding: "28px 32px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {[{ label: "Analyses", value: history.length, color: "#4f46e5" }, { label: "Avg Score", value: history.filter(h => h.score).length ? Math.round(history.filter(h => h.score).reduce((s, h) => s + h.score, 0) / history.filter(h => h.score).length) : "", color: "#a78bfa" }, { label: "Events", value: analyticsData.length, color: "#fbbf24" }].map(s => (
              <div key={s.label} style={{ background: "#283044", border: "1px solid #283044", borderRadius: 10, padding: "10px 18px", textAlign: "center" }}>
        ...
        <div style={{ display: "flex", borderTop: "1px solid #283044" }}>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ background: "none", border: "none", padding: "12px 24px", cursor: "pointer", color: tab === t ? "#4f46e5" : "#64748b", fontWeight: tab === t ? 700 : 500, fontSize: 14, borderBottom: tab === t ? "2px solid #4f46e5" : "2px solid transparent", transition: "all 0.15s" }}>{t}</button>
          ))}
        </div>
      </div>
      ...
      <div style={{ padding: "28px 32px" }}>
        ...
        {tab === "Analyzer" && (
          <div style={{ display: "grid", gridTemplateColumns: result ? "1fr 400px" : "1fr", gap: 24 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {/* URL  the magic input */}
              <div style={{ background: "linear-gradient(135deg, #0f2d24, #1f2433)", border: "2px solid #4f46e5", borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <div style={{ flex: 1, position: "relative" }}>
                    <input
                      ...
                      style={{ width: "100%", background: "#1f2433", border: `1px solid ${fetching ? "#fbbf24" : fetchStatus?.ok ? "#4ade80" : "#283044"}`, borderRadius: 8, padding: "11px 14px", color: "#f6f7fb", fontSize: 15, outline: "none", boxSizing: "border-box" }}
                    />
                  ...
                  <button onClick={() => form.url.trim() && crawlUrl(form.url.trim())} disabled={fetching || !form.url.trim()}
                    style={{ background: fetching ? "#4b5780" : "#4f46e5", border: "none", borderRadius: 8, padding: "11px 18px", color: "#1f2433", fontWeight: 800, cursor: "pointer", fontSize: 14, whiteSpace: "nowrap", opacity: !form.url.trim() ? 0.5 : 1 }}>
                    {fetching ? "" : " Refetch"}
                  </button>
                </div>
                {fetchStatus && (
              ...
              {/* Title + Meta */}
              <div style={{ background: "#283044", border: `1px solid ${issueFields.has("title") || issueFields.has("metaDescription") ? "#f87171" : "#283044"}`, borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                    ...
                    <button onClick={() => { setAiPrompt(`Write 5 SEO-optimised titles for this page: ${form.url || form.h1 || form.title}`); setAiContext("title"); setTab("AI Assistant"); }}
                      style={{ background: "none", border: "1px solid #283044", borderRadius: 6, padding: "3px 10px", color: "#a78bfa", cursor: "pointer", fontSize: 11, fontWeight: 600 }}>AI Suggest</button>
                  </div>
                  <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                    style={{ width: "100%", background: "#1f2433", border: `1px solid ${issueFields.has("title") ? "#f87171" : "#283044"}`, borderRadius: 8, padding: "10px 14px", color: "#f6f7fb", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                ...
                <div>
                  ...
                  <textarea value={form.metaDescription} onChange={e => setForm(f => ({ ...f, metaDescription: e.target.value }))}
                    rows={3} style={{ width: "100%", background: "#1f2433", border: `1px solid ${issueFields.has("metaDescription") ? "#f87171" : "#283044"}`, borderRadius: 8, padding: "10px 14px", color: "#f6f7fb", fontSize: 14, outline: "none", resize: "vertical", lineHeight: 1.5, boxSizing: "border-box" }} />
              ...
              {/* Heading structure */}
              <div style={{ background: "#283044", border: `1px solid ${issueFields.has("h1") || issueFields.has("h2Count") || issueFields.has("h3Count") ? "#f87171" : "#283044"}`, borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
                  <div style={{ gridColumn: "span 2" }}>
                    ...
                    <input value={form.h1} onChange={e => setForm(f => ({ ...f, h1: e.target.value }))}
                      style={{ width: "100%", background: "#1f2433", border: `1px solid ${issueFields.has("h1") ? "#f87171" : "#283044"}`, borderRadius: 8, padding: "9px 12px", color: "#f6f7fb", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                  ...
                  {[{ key: "h2Count", label: "H2 Count" }, { key: "h3Count", label: "H3 Count" }].map(({ key, label }) => (
                    <div key={key}>
                      ...
                      <input type="number" value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder="0"
                        style={{ width: "100%", background: "#1f2433", border: `1px solid ${issueFields.has(key) ? "#f87171" : "#283044"}`, borderRadius: 8, padding: "9px 12px", color: "#f6f7fb", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Content & Links */}
              <div style={{ background: "#283044", border: `1px solid ${issueFields.has("wordCount") || issueFields.has("imagesWithAlt") ? "#fbbf24" : "#283044"}`, borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
                  {[{ key: "wordCount", label: "Word Count" }, { key: "internalLinks", label: "Internal Links" }, { key: "externalLinks", label: "External Links" }, { key: "imageCount", label: "Images Total" }, { key: "imagesWithAlt", label: "Images w/ Alt" }].map(({ key, label }) => (
                    <div key={key}>
                      ...
                      <input type="number" value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder="0"
                        style={{ width: "100%", background: "#1f2433", border: `1px solid ${issueFields.has(key) ? "#fbbf24" : "#283044"}`, borderRadius: 8, padding: "9px 12px", color: "#f6f7fb", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
              ...
              {/* Technical */}
              <div style={{ background: "#283044", border: `1px solid ${issueFields.has("canonicalUrl") || issueFields.has("schemaMarkup") ? "#60a5fa" : "#283044"}`, borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div>
                    ...
                    <input value={form.canonicalUrl} onChange={e => setForm(f => ({ ...f, canonicalUrl: e.target.value }))} placeholder="https://yourstore.myshopify.com/pages/slug"
                      style={{ width: "100%", background: "#1f2433", border: `1px solid ${issueFields.has("canonicalUrl") ? "#60a5fa" : "#283044"}`, borderRadius: 8, padding: "9px 12px", color: "#f6f7fb", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                  </div>
                  <div>
                    ...
                    <input value={form.keywords} onChange={e => setForm(f => ({ ...f, keywords: e.target.value }))} placeholder="ski wax, fast wax, snowboard accessories"
                      style={{ width: "100%", background: "#1f2433", border: "1px solid #283044", borderRadius: 8, padding: "9px 12px", color: "#f6f7fb", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                  </div>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button onClick={() => { setForm(EMPTY_FORM); setResult(null); setError(""); setFetchStatus(null); }}
                  style={{ background: "#283044", border: "1px solid #283044", borderRadius: 10, padding: "12px 20px", color: "#94a3b8", fontWeight: 600, cursor: "pointer", fontSize: 14 }}>Clear</button>
                <button onClick={runAnalysis} disabled={loading}
                  style={{ flex: 1, background: loading ? "#4b5780" : "linear-gradient(135deg, #4f46e5, #22d3ee)", border: "none", borderRadius: 10, padding: "13px", color: "#1f2433", fontWeight: 800, cursor: loading ? "wait" : "pointer", fontSize: 15 }}>
            ...
            {result && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                  <ScoreRing score={result.score} />
                  <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 6 }}>
                    {["high", "medium", "low"].map(sv => {
                      const count = result.issues.filter(i => i.severity === sv).length;
                      return count > 0 ? (
                        <div key={sv} style={{ display: "flex", justifyContent: "space-between", padding: "7px 12px", background: "#1f2433", borderRadius: 8, borderLeft: `3px solid ${sev[sv]}` }}>
                ...
                {result.issues.length > 0 && (
                  <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: 20 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {result.issues.map((issue, i) => (
                        <div key={i} style={{ padding: "10px 14px", background: "#1f2433", borderRadius: 8, borderLeft: `3px solid ${sev[issue.severity]}` }}>
                          <div style={{ display: "flex", alignItems: "flex-start", gap: 8 }}>
                            ...
                            <span style={{ fontSize: 13, color: "#f6f7fb", lineHeight: 1.4 }}>{issue.msg}</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: 20 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                    {[
                      ...
                      <div key={c.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 10px", background: "#1f2433", borderRadius: 7 }}>
        ...
        {tab === "History" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              <h2 style={{ margin: 0, fontSize: 20, fontWeight: 800, color: "#f6f7fb" }}>Analysis History</h2>
              <button onClick={loadHistory} style={{ background: "#283044", border: "1px solid #283044", borderRadius: 10, padding: "8px 16px", color: "#4f46e5", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Refresh</button>
            </div>
            ...
            : history.length === 0 ? (
              <div style={{ textAlign: "center", padding: "64px 0", color: "#64748b" }}>
                ...
                <button onClick={() => setTab("Analyzer")} style={{ marginTop: 16, background: "#4f46e5", border: "none", borderRadius: 10, padding: "10px 24px", color: "#1f2433", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>Run First Analysis</button>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[...history].reverse().map((item, i) => {
                  ...
                  return (
                    <div key={item.id || i} style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: "18px 22px", display: "flex", alignItems: "center", gap: 18 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 700, fontSize: 15, color: "#f6f7fb", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{item.title || item.url || "Untitled page"}</div>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <button onClick={() => copyText(JSON.stringify(item, null, 2))} style={{ background: "#283044", border: "1px solid #283044", borderRadius: 8, padding: "7px 14px", color: "#94a3b8", cursor: "pointer", fontSize: 13 }}>Copy</button>
                      </div>
                    </div>
                  );
                })}
        ...
        {tab === "AI Assistant" && (
          <div style={{ maxWidth: 800 }}>
            <h2 style={{ margin: "0 0 6px", fontSize: 20, fontWeight: 800, color: "#f6f7fb" }}>AI SEO Assistant</h2>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              {[{ key: "general", label: " General" }, { key: "title", label: " Titles" }, { key: "metaDesc", label: " Meta Descs" }, { key: "h1", label: " H1s" }, { key: "schema", label: " Schema" }, { key: "keywords", label: " Keywords" }].map(({ key, label }) => (
                <button key={key} onClick={() => setAiContext(key)} style={{ background: aiContext === key ? "#4f46e5" : "#283044", border: `1px solid ${aiContext === key ? "#4f46e5" : "#283044"}`, borderRadius: 8, padding: "8px 16px", color: aiContext === key ? "#1f2433" : "#94a3b8", cursor: "pointer", fontSize: 13, fontWeight: aiContext === key ? 700 : 500 }}>{label}</button>
              ))}
            </div>
            {result && (
              <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#64748b" }}>
            ...
            <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: 20, marginBottom: 16 }}>
              <textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} rows={4} placeholder="Describe your page or paste specific content for AI to optimise..."
                style={{ width: "100%", background: "#1f2433", border: "1px solid #283044", borderRadius: 8, padding: "10px 14px", color: "#f6f7fb", fontSize: 14, outline: "none", resize: "vertical", lineHeight: 1.5, boxSizing: "border-box" }} />
              <button onClick={runAI} disabled={aiLoading || !aiPrompt.trim()}
                style={{ marginTop: 12, background: aiLoading ? "#4b5780" : "linear-gradient(135deg, #4f46e5, #22d3ee)", border: "none", borderRadius: 10, padding: "11px 28px", color: "#1f2433", fontWeight: 800, cursor: aiLoading ? "wait" : "pointer", fontSize: 14, opacity: !aiPrompt.trim() ? 0.5 : 1 }}>
            ...
            {aiReply && (
              <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ background: "#1f2433", borderRadius: 10, padding: 16, color: "#f6f7fb", fontSize: 14, lineHeight: 1.7, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{aiReply}</div>
              </div>
            )}
          </div>
        )}
        ...
        {tab === "Analytics" && (
          <div>
            <h2 style={{ margin: "0 0 20px", fontSize: 20, fontWeight: 800, color: "#f6f7fb" }}>Analytics</h2>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16, marginBottom: 28 }}>
              {[{ label: "Total Analyses", value: history.length, color: "#4f46e5" }, { label: "Avg SEO Score", value: history.filter(h => h.score).length ? Math.round(history.filter(h => h.score).reduce((s, h) => s + h.score, 0) / history.filter(h => h.score).length) : "", color: "#a78bfa" }, { label: "Score  80", value: history.filter(h => h.score >= 80).length, color: "#4ade80" }, { label: "Score < 55", value: history.filter(h => h.score > 0 && h.score < 55).length, color: "#f87171" }, { label: "Events", value: analyticsData.length, color: "#fbbf24" }].map(s => (
                <div key={s.label} style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: "20px 22px" }}>
            ...
            {history.length > 0 && (
              <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: 24, marginBottom: 20 }}>
                <h3 style={{ margin: "0 0 16px", fontSize: 16, fontWeight: 700, color: "#f6f7fb" }}>Score Distribution</h3>
                {[{ label: "Excellent (80100)", min: 80, max: 100, colour: "#4ade80" }, { label: "Good (5579)", min: 55, max: 79, colour: "#a3e635" }, { label: "Needs Work (3554)", min: 35, max: 54, colour: "#fbbf24" }, { label: "Poor (034)", min: 0, max: 34, colour: "#f87171" }].map(band => {
                  ...
                  return (
                    <div key={band.label} style={{ marginBottom: 12 }}>
                      ...
                      <div style={{ height: 8, background: "#1f2433", borderRadius: 4 }}>
            ...
            {analyticsData.length > 0 && (
              <div style={{ background: "#283044", border: "1px solid #283044", borderRadius: 14, padding: 24 }}>
                <h3 style={{ margin: "0 0 14px", fontSize: 16, fontWeight: 700, color: "#f6f7fb" }}>Recent Events</h3>
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[...analyticsData].reverse().slice(0, 15).map((ev, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "8px 14px", background: "#1f2433", borderRadius: 8 }}>
