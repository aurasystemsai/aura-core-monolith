...
function AnalyzerTab({ data, keywords, fixLoading, fixResults, onGenerateFix }) {
 ...
 const catOrder = [
 { key: 'metaTags', label: 'Meta Tags', icon: '', weight: '30%'},
 { key: 'content', label: 'Content Quality', icon: '', weight: '25%'},
 { key: 'technical', label: 'Technical SEO', icon: '', weight: '20%'},
 ...
 {!data.twitterCard && <div style={{ ...S.muted, color: '#f97316', marginTop: '8px'}}> No twitter:card meta tag Twitter may not display a rich preview when shared.</div>}
 </div>
 )}
 ...
 {data.headingHierarchy.filter(h => h.level === 1).length > 1 && <div style={{ ...S.muted, color: '#ef4444', marginTop: '8px'}}> Multiple H1 tags found pages should have exactly one H1.</div>}
 {data.headingSkips && data.headingSkips.length > 0 && <div style={{ ...S.muted, color: '#f97316', marginTop: '4px'}}> Non-sequential heading levels: {data.headingSkips.join(', ')} don't skip heading levels.</div>}
 {(!data.headingSkips || data.headingSkips.length === 0) && data.headingHierarchy.length > 0 && data.headingHierarchy.some((h, i) => i > 0 && h.level > data.headingHierarchy[i-1].level + 1) && <div style={{ ...S.muted, color: '#f97316', marginTop: '4px'}}> Heading hierarchy has gaps (e.g. H2 H4 with no H3) keep heading levels sequential.</div>}
 </div>
 )}
 ...
 <div style={{ marginTop: '12px'}}>
 {data.urlAnalysis.issues.map((iss, i) => <div key={i} style={{ ...S.muted, color: '#f97316', marginBottom: '4px'}}> {iss}</div>)}
 </div>
 )}
 </div>
 )}
 ...
 <div style={S.elevated}><span style={{ fontSize: '13px', color: '#a1a1aa'}}>Language Tag</span><div style={{ fontSize: '13px', color: data.langTag ? '#22c55e': '#f97316', marginTop: '4px'}}>{data.langTag || 'Missing'}</div></div>
 ...
 <div style={S.elevated}><span style={{ fontSize: '13px', color: '#a1a1aa'}}>Author (E-E-A-T)</span><div style={{ fontSize: '13px', color: data.authorMeta ? '#22c55e': '#f97316', marginTop: '4px'}}>{data.authorMeta || 'No author meta'}</div></div>
 {/* Round 7: new technical signals */}
 <div style={S.elevated}><span style={{ fontSize: '13px', color: '#a1a1aa'}}>Favicon</span><div style={{ fontSize: '13px', color: data.hasFavicon ? '#22c55e': '#f97316', marginTop: '4px'}}>{data.hasFavicon ? 'Found': 'Missing'}</div></div>
 ...
 {data.titleEqualsDescription && <div style={S.elevated}><span style={{ fontSize: '13px', color: '#a1a1aa'}}>Title vs Desc</span><div style={{ fontSize: '13px', color: '#f97316', marginTop: '4px'}}> Identical</div></div>}
 {data.titleEqualsH1 && <div style={S.elevated}><span style={{ fontSize: '13px', color: '#a1a1aa'}}>Title vs H1</span><div style={{ fontSize: '13px', color: '#f97316', marginTop: '4px'}}> Same text</div></div>}
 ...
 {data.selfLinks > 0 && <div style={{ ...S.muted, color: '#f97316', marginTop: '8px'}}> {data.selfLinks} self-referencing link{data.selfLinks > 1 ? 's': ''} detected this page links to itself.</div>}
 ...
 {data.genericLinkCount > 0 && <div style={{ ...S.muted, color: '#f97316', marginTop: '8px'}}> {data.genericLinkCount} link{data.genericLinkCount > 1 ? 's': ''} use generic anchor text ({data.genericLinkAnchors?.slice(0, 5).map(a => `"${a}"`).join(', ')}) use descriptive text instead.</div>}
 ...
 {data.unsafeCrossOriginLinks > 0 && <div style={{ ...S.muted, color: '#f97316', marginTop: '8px'}}> {data.unsafeCrossOriginLinks} external link{data.unsafeCrossOriginLinks > 1 ? 's': ''} with target="_blank"missing rel="noopener"security risk.</div>}
 ...
 {data.plaintextEmails && data.plaintextEmails.length > 0 && <div style={{ ...S.muted, color: '#f97316', marginTop: '8px'}}> {data.plaintextEmails.length} plaintext email{data.plaintextEmails.length > 1 ? 's': ''} exposed: {data.plaintextEmails.slice(0, 3).join(', ')}. Use contact forms to prevent spam harvesting.</div>}
 </div>
 ...
 {isGeneric && <span style={{ fontSize: '11px', color: '#f97316', marginLeft: '6px'}}> generic</span>}
 </div>
 ...
 <span style={{ fontSize: '16px'}}>{data.imagesMissingDimensions === 0 ? '': ''}</span>
 <div style={{ fontSize: '13px', color: data.imagesMissingDimensions === 0 ? '#22c55e': '#f97316'}}>
 {data.imagesMissingDimensions === 0
 ? 'All images have width/height attributes good for CLS (Core Web Vitals).': `${data.imagesMissingDimensions} of ${data.imageCount} images missing width/height attributes causes Cumulative Layout Shift (CLS). Set explicit dimensions to prevent layout jumps.`}
 </div>
 </div>
 )}
 ...
 <span style={{ fontSize: '12px', color: img.descriptive ? '#22c55e': '#f97316', fontWeight: 500 }}>{img.descriptive ? '': ''} {img.filename}</span>
 ...
 <div style={{ fontSize: '13px', color: tag.val ? '#22c55e': '#f97316', marginTop: '4px'}}>{tag.val ? 'Set': 'Missing'}</div>
 </div>
 ))}
 </div>
 </div>
 ...
 <div style={S.muted}>Optimal keyword density is 0.53%. Above 3% may indicate keyword stuffing (penalty risk).</div>
 <div style={{ marginTop: '12px'}}>
 ...
 const statusLabel = kd.status === 'optimal'? 'Optimal': kd.status === 'stuffing'? 'Over-optimized': kd.status === 'low'? 'Low': 'Missing';
 return (
...
function AIAnalysisTab({ data, analysis, loading, error, onRun }) {
 ...
 <div style={S.card}>
 <h2 style={S.h2}> Technical Recommendations</h2>
...
function AIAssistantTab({ messages, input, setInput, loading, onSend }) {
 ...
 Try: "How do I optimize my product page for 'organic cotton t-shirt'?"or "What schema markup should I add to my collection page?"</div>
 </div>
 )}
 {messages.map((m, i) => (
...
function SERPPreview({ data }) {
 ...
 <div style={{ display: 'flex', gap: '4px'}}>
 <button onClick={() => setMode('desktop')} style={{ ...S.btnGhost, fontSize: '12px', ...(mode === 'desktop'? { background: '#4f46e5', color: '#fafafa', borderColor: '#4f46e5'} : {}) }}> Desktop</button>
 ...
 <div style={{ ...S.muted, marginTop: '8px'}}>
 Title: {titleLen} chars {titleLen > 60 ? 'may truncate': titleLen < 30 ? 'too short': ''} · Description: {descLen} chars {descLen > 160 ? 'may truncate': descLen < 120 ? 'too short': ''}
 </div>
 </div>
 );
}
...
function SecurityHeadersDashboard({ data }) {
 ...
 return (
 <div style={S.card}>
 <h2 style={S.h2}> Security & Performance Headers</h2>
 ...
 {perfSignals.map((p, i) => (
 <div key={i} style={{ ...S.elevated, padding: '8px', display: 'flex', gap: '6px', alignItems: 'center'}}>
 <span style={{ fontSize: '14px'}}>{p.ok ? '': ''}</span>
 <div>
...
function IssuesPanel({ issues, data, fixLoading, fixResults, onGenerateFix }) {
 ...
 const catLabels = { metaTags: 'Meta Tags', content: 'Content', technical: 'Technical', linksImages: 'Links/Images', keywords: 'Keywords'};
 ...
 { key: 'info', label: 'Info', color: '#71717a'},
 ].map(s => (
...
function CompetitorCompareTab({ urls, setUrls, results, loading, error, onRun, pageUrl }) {
 ...
 <span style={{ fontSize: '12px', color: '#a1a1aa'}}>{cat === 'metaTags'? 'Meta': cat === 'content'? 'Content': cat === 'technical'? 'Technical': cat === 'linksImages'? 'Links': 'Keywords'}</span>
 {results.map((r, i) => {
...
function BulkScanTab({ bulkUrls, setBulkUrls, bulkResults, bulkLoading, bulkError, onRun, sitemapDomain, setSitemapDomain, sitemapUrls, sitemapLoading, sitemapError, onDiscoverSitemap, onSelectUrl }) {
 ...
 {/* Sitemap Discovery */}
 <div style={S.card}>
 <h2 style={S.h2}> Sitemap Discovery</h2>
...
function LinkHealthTab({ data, linkCheckResults, linkCheckLoading, linkCheckError, onCheckLinks,
 ...
 <div style={S.elevated}>
 <div style={{ fontSize: '13px', fontWeight: 600, color: '#22c55e', marginBottom: '6px'}}> Images</div>
 <div style={{ fontSize: '12px', color: '#d4d4d8'}}>
...
function HTMLValidationPanel({ data }) {
 ...
 <div style={S.card}>
 <div style={S.rowSpread}>
 <h2 style={S.h2}> HTML Validation</h2>
 <span style={S.badge(scoreColor(score))}>{passing}/{checks.length} passing</span>
 </div>
...
function PaginationPanel({ data }) {
 ...
 <div style={S.card}>
 <div style={S.rowSpread}>
 <h2 style={S.h2}> Crawlability & Indexing</h2>
