...
function CharBar({ value = "", min, max, label, error }) {
  ...
  const colour = empty ? "#333333" : ok ? "#4ade80" : over ? "#f87171" : "#fbbf24";
  ...
  return (
    <div style={{ marginTop: 5 }}>
      ...
      <div style={{ height: 5, background: "#1e1e1e", borderRadius: 2 }}>
...
function ScoreRing({ score }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 6 }}>
      <div style={{ width: 96, height: 96, borderRadius: "50%", background: `conic-gradient(${colour} ${score * 3.6}deg, #1e1e1e ${score * 3.6}deg)`, display: "flex", alignItems: "center", justifyContent: "center" }}>
        <div style={{ width: 74, height: 74, borderRadius: "50%", background: "#111111", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center" }}>
...
export default function OnPageSEOEngine({ initialUrl, onUrlConsumed }) {
  ...
  return (
    <div style={{ background: "#111111", minHeight: "100vh", color: "#f0f0f0", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #111111 0%, #1e1e1e 100%)", borderBottom: "1px solid #1e1e1e", padding: "28px 32px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {[{ label: "Analyses", value: history.length, color: "#7fffd4" }, { label: "Avg Score", value: history.filter(h => h.score).length ? Math.round(history.filter(h => h.score).reduce((s, h) => s + h.score, 0) / history.filter(h => h.score).length) : "", color: "#a78bfa" }, { label: "Events", value: analyticsData.length, color: "#fbbf24" }].map(s => (
              <div key={s.label} style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 10, padding: "10px 18px", textAlign: "center" }}>
        ...
        {tab === "Analyzer" && (
          <div style={{ display: "grid", gridTemplateColumns: result ? "1fr 400px" : "1fr", gap: 24 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {/* URL  the magic input */}
              <div style={{ background: "linear-gradient(135deg, #0f2d24, #111111)", border: "2px solid #7fffd4", borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  <div style={{ flex: 1, position: "relative" }}>
                    <input
                      ...
                      style={{ width: "100%", background: "#111111", border: `1px solid ${fetching ? "#fbbf24" : fetchStatus?.ok ? "#4ade80" : "#1e1e1e"}`, borderRadius: 8, padding: "11px 14px", color: "#f0f0f0", fontSize: 15, outline: "none", boxSizing: "border-box" }}
                    />
                  ...
                  <button onClick={() => form.url.trim() && crawlUrl(form.url.trim())} disabled={fetching || !form.url.trim()}
                    style={{ background: fetching ? "#333333" : "#7fffd4", border: "none", borderRadius: 8, padding: "11px 18px", color: "#111111", fontWeight: 800, cursor: "pointer", fontSize: 14, whiteSpace: "nowrap", opacity: !form.url.trim() ? 0.5 : 1 }}>
                    {fetching ? "" : " Refetch"}
                  </button>
                </div>
                {fetchStatus && (
              ...
              {/* Title + Meta */}
              <div style={{ background: "#1e1e1e", border: `1px solid ${issueFields.has("title") || issueFields.has("metaDescription") ? "#f87171" : "#1e1e1e"}`, borderRadius: 14, padding: 20, display: "flex", flexDirection: "column", gap: 18 }}>
                ...
                <div>
                  ...
                  <input value={form.title} onChange={e => setForm(f => ({ ...f, title: e.target.value }))}
                    style={{ width: "100%", background: "#111111", border: `1px solid ${issueFields.has("title") ? "#f87171" : "#1e1e1e"}`, borderRadius: 8, padding: "10px 14px", color: "#f0f0f0", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                ...
                <div>
                  ...
                  <textarea value={form.metaDescription} onChange={e => setForm(f => ({ ...f, metaDescription: e.target.value }))}
                    rows={3} style={{ width: "100%", background: "#111111", border: `1px solid ${issueFields.has("metaDescription") ? "#f87171" : "#1e1e1e"}`, borderRadius: 8, padding: "10px 14px", color: "#f0f0f0", fontSize: 14, outline: "none", resize: "vertical", lineHeight: 1.5, boxSizing: "border-box" }} />
              ...
              {/* Heading structure */}
              <div style={{ background: "#1e1e1e", border: `1px solid ${issueFields.has("h1") || issueFields.has("h2Count") || issueFields.has("h3Count") ? "#f87171" : "#1e1e1e"}`, borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr 1fr", gap: 12 }}>
                  <div style={{ gridColumn: "span 2" }}>
                    ...
                    <input value={form.h1} onChange={e => setForm(f => ({ ...f, h1: e.target.value }))}
                      style={{ width: "100%", background: "#111111", border: `1px solid ${issueFields.has("h1") ? "#f87171" : "#1e1e1e"}`, borderRadius: 8, padding: "9px 12px", color: "#f0f0f0", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                  ...
                  {[{ key: "h2Count", label: "H2 Count" }, { key: "h3Count", label: "H3 Count" }].map(({ key, label }) => (
                    <div key={key}>
                      ...
                      <input type="number" value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder="0"
                        style={{ width: "100%", background: "#111111", border: `1px solid ${issueFields.has(key) ? "#f87171" : "#1e1e1e"}`, borderRadius: 8, padding: "9px 12px", color: "#f0f0f0", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                    </div>
                  ))}
                </div>
              </div>
              ...
              {/* Content & Links */}
              <div style={{ background: "#1e1e1e", border: `1px solid ${issueFields.has("wordCount") || issueFields.has("imagesWithAlt") ? "#fbbf24" : "#1e1e1e"}`, borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "repeat(5, 1fr)", gap: 12 }}>
                  {[{ key: "wordCount", label: "Word Count" }, { key: "internalLinks", label: "Internal Links" }, { key: "externalLinks", label: "External Links" }, { key: "imageCount", label: "Images Total" }, { key: "imagesWithAlt", label: "Images w/ Alt" }].map(({ key, label }) => (
                    <div key={key}>
                      ...
                      <input type="number" value={form[key]} onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))} placeholder="0"
                        style={{ width: "100%", background: "#111111", border: `1px solid ${issueFields.has(key) ? "#fbbf24" : "#1e1e1e"}`, borderRadius: 8, padding: "9px 12px", color: "#f0f0f0", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
              ...
              {/* Technical */}
              <div style={{ background: "#1e1e1e", border: `1px solid ${issueFields.has("canonicalUrl") || issueFields.has("schemaMarkup") ? "#60a5fa" : "#1e1e1e"}`, borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  <div>
                    ...
                    <input value={form.canonicalUrl} onChange={e => setForm(f => ({ ...f, canonicalUrl: e.target.value }))} placeholder="https://yourstore.myshopify.com/pages/slug"
                      style={{ width: "100%", background: "#111111", border: `1px solid ${issueFields.has("canonicalUrl") ? "#60a5fa" : "#1e1e1e"}`, borderRadius: 8, padding: "9px 12px", color: "#f0f0f0", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                  </div>
                  <div>
                    ...
                    <input value={form.keywords} onChange={e => setForm(f => ({ ...f, keywords: e.target.value }))} placeholder="ski wax, fast wax, snowboard accessories"
                      style={{ width: "100%", background: "#111111", border: "1px solid #1e1e1e", borderRadius: 8, padding: "9px 12px", color: "#f0f0f0", fontSize: 14, outline: "none", boxSizing: "border-box" }} />
                  </div>
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button onClick={() => { setForm(EMPTY_FORM); setResult(null); setError(""); setFetchStatus(null); }}
                  style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 10, padding: "12px 20px", color: "#94a3b8", fontWeight: 600, cursor: "pointer", fontSize: 14 }}>Clear</button>
                <button onClick={runAnalysis} disabled={loading}
                  style={{ flex: 1, background: loading ? "#333333" : "linear-gradient(135deg, #7fffd4, #22d3ee)", border: "none", borderRadius: 10, padding: "13px", color: "#111111", fontWeight: 800, cursor: loading ? "wait" : "pointer", fontSize: 15 }}>
            ...
            {result && (
              <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
                <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, display: "flex", flexDirection: "column", alignItems: "center", gap: 16 }}>
                  <ScoreRing score={result.score} />
                  <div style={{ width: "100%", display: "flex", flexDirection: "column", gap: 6 }}>
                    {["high", "medium", "low"].map(sv => {
                      const count = result.issues.filter(i => i.severity === sv).length;
                      return count > 0 ? (
                        <div key={sv} style={{ display: "flex", justifyContent: "space-between", padding: "7px 12px", background: "#111111", borderRadius: 8, borderLeft: `3px solid ${sev[sv]}` }}>
                ...
                {result.issues.length > 0 && (
                  <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: 20 }}>
                    ...
                    <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                      {result.issues.map((issue, i) => (
                        <div key={i} style={{ padding: "10px 14px", background: "#111111", borderRadius: 8, borderLeft: `3px solid ${sev[issue.severity]}` }}>
                ...
                <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: 20 }}>
                  ...
                  <div style={{ display: "flex", flexDirection: "column", gap: 5 }}>
                    {[
                      ...
                      <div key={c.label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "6px 10px", background: "#111111", borderRadius: 7 }}>
        ...
        {tab === "History" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button onClick={loadHistory} style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 10, padding: "8px 16px", color: "#7fffd4", cursor: "pointer", fontSize: 13, fontWeight: 600 }}> Refresh</button>
            </div>
            ...
            : history.length === 0 ? (
              <div style={{ textAlign: "center", padding: "64px 0", color: "#64748b" }}>
                ...
                <button onClick={() => setTab("Analyzer")} style={{ marginTop: 16, background: "#7fffd4", border: "none", borderRadius: 10, padding: "10px 24px", color: "#111111", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>Run First Analysis</button>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                {[...history].reverse().map((item, i) => {
                  ...
                  return (
                    <div key={item.id || i} style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: "18px 22px", display: "flex", alignItems: "center", gap: 18 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        ...
                        <button onClick={() => copyText(JSON.stringify(item, null, 2))} style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 8, padding: "7px 14px", color: "#94a3b8", cursor: "pointer", fontSize: 13 }}>Copy</button>
                      </div>
                    </div>
                  );
                })}
          ...
          <div style={{ maxWidth: 800 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              {[{ key: "general", label: " General" }, { key: "title", label: " Titles" }, { key: "metaDesc", label: " Meta Descs" }, { key: "h1", label: " H1s" }, { key: "schema", label: " Schema" }, { key: "keywords", label: " Keywords" }].map(({ key, label }) => (
                <button key={key} onClick={() => setAiContext(key)} style={{ background: aiContext === key ? "#7fffd4" : "#1e1e1e", border: `1px solid ${aiContext === key ? "#7fffd4" : "#1e1e1e"}`, borderRadius: 8, padding: "8px 16px", color: aiContext === key ? "#111111" : "#94a3b8", cursor: "pointer", fontSize: 13, fontWeight: aiContext === key ? 700 : 500 }}>{label}</button>
              ))}
            </div>
            {result && (
              <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#64748b" }}>
            ...
            <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: 20, marginBottom: 16 }}>
              <textarea value={aiPrompt} onChange={e => setAiPrompt(e.target.value)} rows={4} placeholder="Describe your page or paste specific content for AI to optimise..."
                style={{ width: "100%", background: "#111111", border: "1px solid #1e1e1e", borderRadius: 8, padding: "10px 14px", color: "#f0f0f0", fontSize: 14, outline: "none", resize: "vertical", lineHeight: 1.5, boxSizing: "border-box" }} />
              <button onClick={runAI} disabled={aiLoading || !aiPrompt.trim()}
                style={{ marginTop: 12, background: aiLoading ? "#333333" : "linear-gradient(135deg, #7fffd4, #22d3ee)", border: "none", borderRadius: 10, padding: "11px 28px", color: "#111111", fontWeight: 800, cursor: aiLoading ? "wait" : "pointer", fontSize: 14, opacity: !aiPrompt.trim() ? 0.5 : 1 }}>
            ...
            {aiReply && (
              <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: 20 }}>
                ...
                <div style={{ background: "#111111", borderRadius: 10, padding: 16, color: "#f0f0f0", fontSize: 14, lineHeight: 1.7, whiteSpace: "pre-wrap", wordBreak: "break-word" }}>{aiReply}</div>
              </div>
            )}
          </div>
        )}
          ...
          <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 16, marginBottom: 28 }}>
              {[{ label: "Total Analyses", value: history.length, color: "#7fffd4" }, { label: "Avg SEO Score", value: history.filter(h => h.score).length ? Math.round(history.filter(h => h.score).reduce((s, h) => s + h.score, 0) / history.filter(h => h.score).length) : "", color: "#a78bfa" }, { label: "Score  80", value: history.filter(h => h.score >= 80).length, color: "#4ade80" }, { label: "Score < 55", value: history.filter(h => h.score > 0 && h.score < 55).length, color: "#f87171" }, { label: "Events", value: analyticsData.length, color: "#fbbf24" }].map(s => (
                <div key={s.label} style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: "20px 22px" }}>
            ...
            {history.length > 0 && (
              <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24, marginBottom: 20 }}>
            ...
            {analyticsData.length > 0 && (
              <div style={{ background: "#1e1e1e", border: "1px solid #1e1e1e", borderRadius: 14, padding: 24 }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
                  {[...analyticsData].reverse().slice(0, 15).map((ev, i) => (
                    <div key={i} style={{ display: "flex", alignItems: "center", gap: 14, padding: "8px 14px", background: "#111111", borderRadius: 8 }}>
