...
export default function OnPageSEOEngine() {
  ...
  const fetchHistory = async () => {
    try {
      const res = await apiFetch("/api/on-page-seo-engine/history");
  ...
  const handleRun = async () => {
    ...
    setResponse("");
    try {
      const res = await apiFetch("/api/on-page-seo-engine/ai/analyze", {
        method: "POST",
      ...
      // Save to history
      await apiFetch("/api/on-page-seo-engine/history", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ page: input, report: data.seoReport })
      });
      fetchHistory();
    } catch (err) {
  ...
  const handleImport = e => {
    ...
    reader.onload = async evt => {
      try {
        const res = await apiFetch("/api/on-page-seo-engine/import", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
  ...
  const handleFeedback = async () => {
    ...
    setError("");
    try {
      await apiFetch("/api/on-page-seo-engine/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", background: "#fff", borderRadius: 18, boxShadow: "0 2px 24px #0002", padding: 36, fontFamily: 'Inter, sans-serif' }}>
      <h2 style={{ fontWeight: 800, fontSize: 32, marginBottom: 8 }}>On-Page SEO Engine</h2>
      <div style={{ marginBottom: 10, color: "#0ea5e9", fontWeight: 600 }}>
        <span role="img" aria-label="sparkles">✨</span> Generate, manage, and analyze on-page SEO with AI and analytics.
      </div>
      <textarea
        value={input}
        ...
        placeholder="Type your page or prompt here..."
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 18 }}>
        ...
        {exported && <a href={exported} download="on-page-seo-history.json" style={{ marginLeft: 8, color: "#0ea5e9", fontWeight: 600 }}>Download</a>}
      </div>
      {imported && <div style={{ color: "#22c55e", marginBottom: 8 }}>Imported: {imported}</div>}
      {response && (
        <div style={{ background: "#f1f5f9", borderRadius: 10, padding: 16, marginBottom: 12, color: "#23263a" }}>
          <div style={{ fontWeight: 600, marginBottom: 4 }}>AI SEO Report:</div>
          <div>{response}</div>
        </div>
      )}
  ...
  const handleExport = () => {
    ...
    a.href = url;
    a.download = "onpage-seo-report.txt";
    a.click();
  ...
  const handleFeedback = async () => {
    ...
    try {
      await fetch("/api/on-page-seo-engine/feedback", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ feedback })
      });
  ...
  return (
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <h2 style={{ fontWeight: 700, fontSize: 28, marginBottom: 12 }}>On-Page SEO Engine</h2>
        <button
      ...
      <p style={{ color: darkMode ? "#e0e7ff" : "#444", marginBottom: 18 }}>
        Paste a page URL or HTML below. The AI will analyze and provide on-page SEO recommendations for all channels. <span style={{ fontWeight: 600 }}>All features are fully accessible.</span>
      </p>
      <textarea
        value={input}
        onChange={e => setInput(e.target.value)}
        ...
        placeholder="Paste your page URL or HTML here..."
        aria-label="Page input"
      />
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <label style={{ fontWeight: 600, marginLeft: 18 }}>Internal Links:</label>
        <input type="checkbox" checked={internalLinks} onChange={e => setInternalLinks(e.target.checked)} />
      </div>
      <div style={{ marginBottom: 18 }}>
      ...
      {serp && (
        <div style={{ marginTop: 24, background: darkMode ? "#334155" : "#e0e7ff", borderRadius: 12, padding: 18 }}>
          <div style={{ fontWeight: 600, fontSize: 16, marginBottom: 8 }}>SERP Features</div>
          <div style={{ fontSize: 16 }}>{JSON.stringify(serp)}</div>
        </div>
      )}
      {accessibility && (
        <div style={{ marginTop: 24, background: darkMode ? "#334155" : "#e0e7ff", borderRadius: 12, padding: 18 }}>
          ...
          <ul style={{ paddingLeft: 18 }}>
            {history.map((h, i) => (
              <li key={i} style={{ marginBottom: 10 }}>
                <div><b>Page:</b> {h.page.slice(0, 60)}{h.page.length > 60 ? "..." : ""}</div>
                ...
                <div><b>Internal Links:</b> {h.internalLinks ? "Yes" : "No"}</div>
                ...
                <div><b>SERP:</b> {h.serp ? JSON.stringify(h.serp) : "-"}</div>
