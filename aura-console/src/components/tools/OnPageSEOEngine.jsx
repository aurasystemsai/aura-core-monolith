...
function GooglePreview({ title, description, url }) {
  return (
    <div style={{ background: "#818cf8", borderRadius: 10, padding: "14px 18px", fontFamily: "Arial, sans-serif", border: "1px solid #fafafa" }}>
    ...
    <div style={{ background: "#18181b", minHeight: "100vh", color: "#fafafa", fontFamily: "Inter, system-ui, sans-serif" }}>
      ...
      <div style={{ background: "linear-gradient(135deg, #18181b 0%, #27272a 100%)", borderBottom: "1px solid #27272a", padding: "28px 32px 0" }}>
        <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
          <div>
            <h1 style={{ fontSize: 28, fontWeight: 900, color: "#818cf8", margin: 0 }}>On-Page SEO Engine</h1>
          ...
          <div style={{ display: "flex", gap: 12 }}>
            {[{ label: "Analyses", value: history.length, color: "#818cf8" }, { label: "Avg Score", value: history.filter(h => h.score).length ? Math.round(history.filter(h => h.score).reduce((s, h) => s + h.score, 0) / history.filter(h => h.score).length) : "", color: "#a78bfa" }, { label: "Events", value: analyticsData.length, color: "#fbbf24" }].map(s => (
        ...
        <div style={{ display: "flex", borderTop: "1px solid #27272a" }}>
          {TABS.map(t => (
            <button key={t} onClick={() => setTab(t)} style={{ background: "none", border: "none", padding: "12px 24px", cursor: "pointer", color: tab === t ? "#818cf8" : "#64748b", fontWeight: tab === t ? 700 : 500, fontSize: 14, borderBottom: tab === t ? "2px solid #818cf8" : "2px solid transparent", transition: "all 0.15s" }}>{t}</button>
          ))}
        </div>
      </div>
        ...
        {tab === "Analyzer" && (
          <div style={{ display: "grid", gridTemplateColumns: result ? "1fr 400px" : "1fr", gap: 24 }}>
            <div style={{ display: "flex", flexDirection: "column", gap: 16 }}>
              ...
              {/* URL  the magic input */}
              <div style={{ background: "linear-gradient(135deg, #0f2d24, #18181b)", border: "2px solid #818cf8", borderRadius: 14, padding: 20 }}>
                <label style={{ display: "block", fontWeight: 800, color: "#818cf8", marginBottom: 10, fontSize: 13, textTransform: "uppercase", letterSpacing: "0.07em" }}>
                ...
                <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
                  ...
                  <button onClick={() => form.url.trim() && crawlUrl(form.url.trim())} disabled={fetching || !form.url.trim()}
                    style={{ background: fetching ? "#52525b" : "#818cf8", border: "none", borderRadius: 8, padding: "11px 18px", color: "#18181b", fontWeight: 800, cursor: "pointer", fontSize: 14, whiteSpace: "nowrap", opacity: !form.url.trim() ? 0.5 : 1 }}>
                    {fetching ? "" : " Refetch"}
                  </button>
                </div>
                {fetchStatus && (
              ...
              <div style={{ display: "flex", gap: 12 }}>
                <button onClick={() => { setForm(EMPTY_FORM); setResult(null); setError(""); setFetchStatus(null); }}
                  style={{ background: "#27272a", border: "1px solid #27272a", borderRadius: 10, padding: "12px 20px", color: "#94a3b8", fontWeight: 600, cursor: "pointer", fontSize: 14 }}>Clear</button>
                <button onClick={runAnalysis} disabled={loading}
                  style={{ flex: 1, background: loading ? "#52525b" : "linear-gradient(135deg, #818cf8, #22d3ee)", border: "none", borderRadius: 10, padding: "13px", color: "#18181b", fontWeight: 800, cursor: loading ? "wait" : "pointer", fontSize: 15 }}>
        ...
        {tab === "History" && (
          <div>
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
              ...
              <button onClick={loadHistory} style={{ background: "#27272a", border: "1px solid #27272a", borderRadius: 10, padding: "8px 16px", color: "#818cf8", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Refresh</button>
            </div>
            ...
            : history.length === 0 ? (
              <div style={{ textAlign: "center", padding: "64px 0", color: "#64748b" }}>
                ...
                <button onClick={() => setTab("Analyzer")} style={{ marginTop: 16, background: "#818cf8", border: "none", borderRadius: 10, padding: "10px 24px", color: "#18181b", fontWeight: 700, cursor: "pointer", fontSize: 14 }}>Run First Analysis</button>
              </div>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
                  ...
                  return (
                    <div key={item.id || i} style={{ background: "#27272a", border: "1px solid #27272a", borderRadius: 14, padding: "18px 22px", display: "flex", alignItems: "center", gap: 18 }}>
                      ...
                      <div style={{ display: "flex", gap: 8 }}>
                        <button onClick={() => loadFromHistory(item)} style={{ background: "rgba(127,255,212,0.1)", border: "1px solid rgba(127,255,212,0.3)", borderRadius: 8, padding: "7px 14px", color: "#818cf8", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Load</button>
                        <button onClick={() => copyText(JSON.stringify(item, null, 2))} style={{ background: "#27272a", border: "1px solid #27272a", borderRadius: 8, padding: "7px 14px", color: "#94a3b8", cursor: "pointer", fontSize: 13 }}>Copy</button>
                      </div>
                    </div>
                  );
                })}
          ...
          <div style={{ maxWidth: 800 }}>
            ...
            <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 20 }}>
              {[{ key: "general", label: " General" }, { key: "title", label: " Titles" }, { key: "metaDesc", label: " Meta Descs" }, { key: "h1", label: " H1s" }, { key: "schema", label: " Schema" }, { key: "keywords", label: " Keywords" }].map(({ key, label }) => (
                <button key={key} onClick={() => setAiContext(key)} style={{ background: aiContext === key ? "#818cf8" : "#27272a", border: `1px solid ${aiContext === key ? "#818cf8" : "#27272a"}`, borderRadius: 8, padding: "8px 16px", color: aiContext === key ? "#18181b" : "#94a3b8", cursor: "pointer", fontSize: 13, fontWeight: aiContext === key ? 700 : 500 }}>{label}</button>
              ))}
            </div>
            {result && (
              <div style={{ background: "#27272a", border: "1px solid #27272a", borderRadius: 10, padding: "12px 16px", marginBottom: 16, fontSize: 13, color: "#64748b" }}>
                <span style={{ color: "#818cf8", fontWeight: 600 }}>Context:</span> {result.input.title || result.input.url}  Score: <span style={{ color: result.score >= 80 ? "#4ade80" : "#fbbf24", fontWeight: 700 }}>{result.score}</span>  {result.issues.length} issues
              </div>
            )}
            <div style={{ background: "#27272a", border: "1px solid #27272a", borderRadius: 14, padding: 20, marginBottom: 16 }}>
              ...
              <button onClick={runAI} disabled={aiLoading || !aiPrompt.trim()}
                style={{ marginTop: 12, background: aiLoading ? "#52525b" : "linear-gradient(135deg, #818cf8, #22d3ee)", border: "none", borderRadius: 10, padding: "11px 28px", color: "#18181b", fontWeight: 800, cursor: aiLoading ? "wait" : "pointer", fontSize: 14, opacity: !aiPrompt.trim() ? 0.5 : 1 }}>
            ...
            {aiReply && (
              <div style={{ background: "#27272a", border: "1px solid #27272a", borderRadius: 14, padding: 20 }}>
                <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
                  <span style={{ fontWeight: 700, color: "#818cf8", fontSize: 15 }}>AI Response</span>
                  <button onClick={() => copyText(aiReply)} style={{ background: "rgba(127,255,212,0.1)", border: "1px solid rgba(127,255,212,0.3)", borderRadius: 8, padding: "5px 14px", color: "#818cf8", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Copy</button>
                </div>
      ...
      <style>{`input:focus,textarea:focus{border-color:#818cf8!important;box-shadow:0 0 0 3px rgba(127,255,212,0.1);}button:active{transform:scale(0.97);}`}</style>
    </div>
  );
}
