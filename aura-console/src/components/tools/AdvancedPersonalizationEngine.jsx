          ...
          <div style={{ display: "grid", gap: 8 }}>
            {recommendations.map((rec) => (
              <div key={rec.id} style={{ ...paletteCard, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
                ...
                <button onClick={() => promoteRecommendation(rec)} style={{ background: palette.accent, color: "#0f172a", border: "none", borderRadius: 10, padding: "8px 12px", fontWeight: 800, cursor: "pointer" }}>Promote</button>
              </div>
            ))}
          </div>
        </div>
