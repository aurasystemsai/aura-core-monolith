  ...
  return (
    <div style={{ maxWidth: 900, margin: "40px auto", background: "#23263a", color: "#f3f4f6", borderRadius: 18, boxShadow: "0 2px 24px #0002", padding: 36, fontFamily: 'Inter, sans-serif' }}>
      ...
      <div style={{ marginTop: 24, background: "#f3f4f6", borderRadius: 12, padding: 18 }}>
        <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>Review History</div>
        <ul style={{ paddingLeft: 18 }}>
          {reviews.map(r => (
            <li key={r.id} style={{ marginBottom: 10 }}>
              <div><b>ID:</b> {r.id}</div>
              <div><b>Prompt:</b> {r.prompt}</div>
              <div><b>Content:</b> {r.content || JSON.stringify(r)}</div>
            </li>
          ))}
        </ul>
      </div>
      <div style={{ marginTop: 24, background: "#f3f4f6", borderRadius: 12, padding: 18 }}>
        ...
        <div style={{ fontSize: 15, color: "#23263a" }}>
          {analytics.length ? (
            ...
            <span>No analytics yet. Generate or import reviews to see results.</span>
          )}
        </div>
      </div>
