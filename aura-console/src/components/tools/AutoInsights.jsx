    ...
    <div className="aura-card flagship-autoinsights-dashboard" style={{ padding: 0, background: "var(--surface-card)", borderRadius: 24, boxShadow: "0 8px 32px #0006" }}>
      ...
      <div className="autoinsights-header" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "36px 48px 0 48px" }}>
        ...
        <div style={{ display: 'flex', gap: 12, alignItems: 'center', flexWrap: 'wrap' }}>
          <select value={env} onChange={e => setEnv(e.target.value)} style={{ background: '#18181b', color: '#fafafa', border: '1px solid #27272a', borderRadius: 10, padding: '10px 12px', fontWeight: 800 }}>
            <option value="dev">Dev</option>
            <option value="stage">Stage</option>
            <option value="prod">Prod</option>
          </select>
          <select value={shop} onChange={e => setShop(e.target.value)} style={{ background: '#18181b', color: '#fafafa', border: '1px solid #27272a', borderRadius: 10, padding: '10px 12px', fontWeight: 800 }}>
          ...
          <div className="insight-score-card" style={{ background: "#18181b", borderRadius: 18, padding: "18px 32px", boxShadow: "0 2px 16px #0003", textAlign: "center" }}>
          ...
          <div style={{ background: '#18181b', borderRadius: 12, padding: '10px 14px', minWidth: 180 }}>
            ...
            <div style={{ color: '#a1a1aa', fontSize: 12 }}>Last sync {syncHealth.lastSuccess ? new Date(syncHealth.lastSuccess).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' }) : '—'}</div>
          </div>
          <div style={{ background: '#18181b', borderRadius: 12, padding: '10px 14px', minWidth: 200 }}>
            <div style={{ color: '#a1a1aa', fontSize: 12 }}>Freshness SLA: {freshness.sla}m</div>
            <div style={{ fontWeight: 800, color: freshness.status === 'healthy' ? '#22c55e' : '#fbbf24' }}>Age: {freshness.minutes}m</div>
            <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
              <button className="aura-btn" style={{ background: '#27272a', color: '#fafafa', border: '1px solid #52525b' }} onClick={checkFreshness}>Check</button>
          ...
          <div style={{ background: '#18181b', borderRadius: 12, padding: '10px 14px', minWidth: 220 }}>
            <div style={{ color: '#a1a1aa', fontSize: 12 }}>Source freshness map</div>
            <div style={{ color: '#fafafa', fontWeight: 700 }}>Orders {Math.round((Date.now() - sourceFreshness.orders) / 60000)}m · Events {Math.round((Date.now() - sourceFreshness.events) / 60000)}m</div>
            <div style={{ color: '#a1a1aa', fontSize: 12 }}>Attribution {(Math.round((Date.now() - sourceFreshness.attribution) / 60000))}m</div>
            <div style={{ display: 'flex', gap: 6, marginTop: 6 }}>
              <button className="aura-btn" style={{ background: '#27272a', color: '#fafafa', border: '1px solid #52525b' }} onClick={refreshSources}>Refresh sources</button>
              <button className="aura-btn" style={{ background: '#18181b', color: '#fbbf24', border: '1px solid #52525b' }} onClick={() => setIncidentMode(true)}>Open incident</button>
            </div>
          </div>
          <button className="aura-btn" style={{ background: '#4f46e5', color: '#09090b', fontWeight: 800 }} onClick={simulateIngest}>Refresh data</button>
        </div>
      </div>
      ...
      <div className="autoinsights-segmentation" style={{ padding: "32px 48px", display: "flex", gap: 32 }}>
        <div style={{ flex: 2 }}>
          ...
          <div style={{ display: "flex", gap: 18, marginBottom: 18 }}>
            ...
            <button className="aura-btn" style={{ background: "#4f46e5", color: "#09090b", fontWeight: 700 }} onClick={handleFilter}>Filter</button>
          </div>
          ...
          <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <button className="aura-btn" style={{ background: '#27272a', color: '#fafafa', border: '1px solid #52525b' }} onClick={() => addPin(`Shared ${segment}`, 'Shared pin set')}>Share pins</button>
            ...
            <div style={{ color: '#a1a1aa', fontSize: 12 }}>Seg: {segment} · Dim: {dimension} · Metric: {metricPreset}</div>
          </div>
        </div>
        {/* Alerts & Integrations */}
          ...
          <div style={{ flex: 1, display: "flex", flexDirection: "column", gap: 18 }}>
            ...
            <div style={{ background: "#09090b", borderRadius: 18, padding: 18 }}>
              ...
              <div style={{ marginTop: 10, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                ...
                <button className="aura-btn" style={{ background: '#27272a', color: '#fafafa', border: '1px solid #52525b' }} onClick={() => recordHistory('Scheduled weekly PDF')}>Schedule weekly PDF</button>
              </div>
            </div>
          </div>
        </div>
        <div style={{ marginTop: 16, display: 'flex', gap: 12, flexWrap: 'wrap', background: '#18181b', borderRadius: 14, padding: 14, border: '1px solid #27272a' }}>
          ...
          <div style={{ color: '#a1a1aa', fontSize: 13 }}>Maps to Shopify events (checkout, orders, carts) for {shop}</div>
        </div>
      </div>
      ...
      <div style={{ padding: "0 48px 24px" }}>
        <div style={{ display: 'grid', gap: 18, gridTemplateColumns: 'minmax(240px, 1.2fr) minmax(240px, 1fr)' }}>
          <div style={{ background: '#09090b', borderRadius: 18, padding: 18 }}>
            ...
            {pinboard.length === 0 ? (
              <div style={{ color: '#a1a1aa', fontSize: 13 }}>No pins yet. Ctrl+P to pin current preset.</div>
            ) : (
              <div style={{ display: 'grid', gap: 10 }}>
                {pinboard.map(pin => (
                  <div key={pin.id} style={{ background: '#18181b', borderRadius: 10, padding: 10, border: '1px solid #27272a' }}>
                    <div style={{ color: '#fafafa', fontWeight: 700 }}>{pin.title}</div>
                    <div style={{ color: '#a1a1aa', fontSize: 12 }}>{pin.note}</div>
                    {pin.comments && pin.comments.length > 0 && <div style={{ color: '#4f46e5', fontSize: 12 }}>Comments: {pin.comments.join(' · ')}</div>}
                    <div style={{ color: '#a1a1aa', fontSize: 12 }}>{pin.shop} · {pin.env}</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <div style={{ background: '#09090b', borderRadius: 18, padding: 18 }}>
            ...
            <div style={{ marginTop: 8, display: 'grid', gap: 8 }}>
              {attribution.map((a, idx) => (
                <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#18181b', borderRadius: 10, padding: 10, border: '1px solid #27272a' }}>
                  <div>
                    <div style={{ color: '#fafafa', fontWeight: 700 }}>{a.factor}</div>
                    <div style={{ color: '#a1a1aa', fontSize: 12 }}>Weight {(a.weight * 100).toFixed(0)}%</div>
                  </div>
      ...
      <div style={{ padding: "0 48px 24px" }}>
        <div style={{ display: 'grid', gap: 18, gridTemplateColumns: 'minmax(240px, 1fr) minmax(240px, 1fr)' }}>
          ...
          <div style={{ background: '#09090b', borderRadius: 18, padding: 18 }}>
            ...
            <div style={{ display: 'grid', gap: 10 }}>
              {anomalies.map(a => (
                <div key={a.id} style={{ display: 'grid', gap: 8, background: '#18181b', borderRadius: 10, padding: '10px 12px', border: '1px solid #27272a' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    <div>
                      <div style={{ color: '#fafafa', fontWeight: 700 }}>{a.label}</div>
                      <div style={{ color: '#a1a1aa', fontSize: 12 }}>{a.time} · {a.severity}</div>
                    </div>
                    {suppressedAnomalies.includes(a.id) && <div style={{ color: '#fbbf24', fontSize: 12 }}>Suppressed</div>}
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    <button className="aura-btn" style={{ background: '#27272a', color: '#fafafa', border: '1px solid #52525b' }} onClick={() => recordHistory(`Acknowledged anomaly: ${a.label}`)}>Acknowledge</button>
                    <button className="aura-btn" style={{ background: '#0ea5e9', color: '#fff' }} onClick={() => routeAlert('slack')}>Route to Slack</button>
                    <button className="aura-btn" style={{ background: '#4f46e5', color: '#09090b' }} onClick={() => routeAlert('email')}>Email alert</button>
      ...
      <div style={{ padding: "0 48px 36px", display: 'grid', gap: 18, gridTemplateColumns: 'minmax(260px, 1fr) minmax(260px, 1fr)' }}>
        <div style={{ background: '#09090b', borderRadius: 18, padding: 18 }}>
          ...
          <div style={{ marginTop: 10, background: '#18181b', borderRadius: 10, padding: 12, border: '1px solid #27272a' }}>
            <div style={{ color: '#fafafa', fontWeight: 700 }}>Delta callouts</div>
            <div style={{ color: '#a1a1aa', fontSize: 13 }}>Conversion +4.2 pts; AOV -1.1%; LTV +3.6%</div>
          </div>
          <button className="aura-btn" style={{ marginTop: 10 }} onClick={() => recordHistory('Exported comparison CSV')}>Export comparison CSV</button>
        </div>
        <div style={{ background: '#09090b', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
          ...
          <div style={{ color: '#a1a1aa', fontSize: 13 }}>Actions consider current filters and segment.</div>
        </div>
        <div style={{ background: '#09090b', borderRadius: 18, padding: 18, display: 'grid', gap: 10 }}>
          ...
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
            ...
            <span style={{ color: '#a1a1aa', fontSize: 13 }}>% shift in spend</span>
      ...
      <div style={{ padding: "0 48px 24px" }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
          ...
          <button className="aura-btn" style={{ background: '#27272a', color: '#fafafa', border: '1px solid #52525b' }} onClick={() => setShowDebug(v => !v)}>{showDebug ? 'Hide' : 'Show'} traces</button>
        </div>
        {showDebug ? (
          traceEvents.length === 0 ? <div style={{ color: '#a1a1aa', marginTop: 8 }}>No traces yet. Ctrl+D toggles. Filters, pins, ingests are traced.</div> : (
            <div style={{ marginTop: 10, display: 'grid', gap: 8 }}>
              {traceEvents.map((t, idx) => (
                <div key={idx} style={{ background: '#18181b', borderRadius: 10, padding: 10, border: '1px solid #27272a' }}>
                  <div style={{ color: '#fafafa', fontWeight: 700 }}>{t.event}</div>
                  <div style={{ color: '#a1a1aa', fontSize: 12 }}>{new Date(t.at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · {t.env} · {t.shop}</div>
                  <div style={{ color: '#a1a1aa', fontSize: 12, wordBreak: 'break-word' }}>{JSON.stringify(t.meta)}</div>
                </div>
              ))}
            </div>
          )
        ) : (
          <div style={{ color: '#a1a1aa', marginTop: 8 }}>Tracing is off. Toggle to view recent actions.</div>
        )}
      </div>
        ...
        <div style={{ padding: "0 48px 36px" }}>
          ...
          <div style={{ display: 'grid', gap: 10 }}>
            {history.slice(0, 5).map((h, idx) => (
              <div key={idx} style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', background: '#18181b', border: '1px solid #27272a', borderRadius: 12, padding: '10px 12px' }}>
                <div>
                  <div style={{ fontWeight: 700, color: '#fafafa' }}>{h.summary}</div>
                  <div style={{ color: '#a1a1aa', fontSize: 12 }}>{new Date(h.at).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })} · {h.env}</div>
                </div>
                <button className="aura-btn" style={{ background: '#27272a', color: '#fafafa', border: '1px solid #52525b' }} onClick={() => recordHistory(`Replayed: ${h.summary}`)}>Replay</button>
              </div>
            ))}
          </div>
        </div>
