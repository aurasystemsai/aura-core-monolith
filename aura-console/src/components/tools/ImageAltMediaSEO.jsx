        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 10 }}>
          ...
          <div style={{ marginBottom: 24, background: "linear-gradient(135deg, #1e293b 0%, #334155 100%)", borderRadius: 20, padding: 24, border: "2px solid #475569", boxShadow: "0 8px 24px rgba(0,0,0,0.3)" }}>
            ...
            <div style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap" }}>
              ...
              <button onClick={handleAiImproveSelected} aria-label="Automatically generate AI alt text for all selected images" disabled={!roleCanApply || !selectedImageIds.length || loading} style={{ background: roleCanApply && selectedImageIds.length ? "linear-gradient(135deg, #ef4444 0%, #dc2626 100%)" : "#334155", color: "#fff", border: selectedImageIds.length ? "3px solid #fbbf24" : "none", borderRadius: 12, padding: "12px 24px", fontWeight: 900, fontSize: 15, cursor: (!roleCanApply || !selectedImageIds.length || loading) ? "not-allowed" : "pointer", boxShadow: roleCanApply && selectedImageIds.length ? "0 8px 24px rgba(239, 68, 68, 0.5)" : "none", transition: "all 0.2s", transform: "translateY(0)" }} onMouseEnter={e => { if (roleCanApply && selectedImageIds.length && !loading) e.target.style.transform = "translateY(-2px) scale(1.02)"; }} onMouseLeave={e => e.target.style.transform = "translateY(0) scale(1)"}>🤖 AI IMPROVE ALL ({selectedImageIds.length})</button>
