...
function SeverityBadge({ sev }) {
  return (
    <span style={{
      background: SEV_BG[sev] || "#2e3045",
      color: SEV_COLORS[sev] || "#94a3b8",
      border: `1px solid ${SEV_COLORS[sev] || "#525578"}`,
...
function IssueCard({ issue, pageUrl }) {
  ...
  return (
    <div style={{ background: SEV_BG[issue.severity] || "#2e3045", borderRadius: 8, padding: "10px 14px", border: `1px solid ${SEV_COLORS[issue.severity] || "#525578"}22`, marginBottom: 8 }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
        ...
        <button
          ...
          disabled={fixing}
          style={{ marginLeft: "auto", background: "#6366f1", color: "#0d0d11", border: "none", borderRadius: 6, padding: "3px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer" }}
      ...
      {suggestion && (
        <div style={{ marginTop: 8, background: "#2e3045", borderRadius: 6, padding: "8px 12px", border: "1px solid #2e3045", fontSize: 13, color: "#6366f1", whiteSpace: "pre-wrap", lineHeight: 1.6 }}>
          {suggestion}
          <button onClick={() => navigator.clipboard?.writeText(suggestion)} style={{ display: "block", marginTop: 6, background: "transparent", border: "1px solid #525578", borderRadius: 5, padding: "2px 10px", color: "#64748b", fontSize: 11, cursor: "pointer" }}>Copy</button>
        </div>
      )}
...
function KeywordPresencePanel({ keywords, keywordPresence }) {
  ...
  return (
    <div style={{ background: "#252638", borderRadius: 8, padding: "10px 14px", marginBottom: 10, border: "1px solid #2e3045" }}>
      ...
      <div style={{ display: "flex", flexWrap: "wrap", gap: 8 }}>
        {(keywordPresence || []).map(kp => (
          <div key={kp.keyword} style={{ background: "#0d0d11", borderRadius: 8, padding: "6px 12px", border: "1px solid #2e3045", fontSize: 13 }}>
...
function PageSection({ page, keywords }) {
  ...
  return (
    <div style={{ background: "#0d0d11", borderRadius: 10, border: "1px solid #2e3045", marginBottom: 12 }}>
      <button
        onClick={() => setOpen(o => !o)}
    ...
    <div style={{ background: "#0d0d11", color: "#f3f4f6", borderRadius: 18, boxShadow: "0 2px 24px #0002", padding: 36, fontFamily: "Inter, sans-serif" }}>
      ...
      <div style={{ marginBottom: 14 }}>
        ...
        <input
          ...
          style={{ width: "100%", fontSize: 15, padding: "10px 14px", borderRadius: 8, border: "1px solid #525578", background: "#2e3045", color: "#f3f4f6", boxSizing: "border-box" }}
      ...
      <div style={{ marginBottom: 18 }}>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6, background: "#2e3045", borderRadius: 8, border: "1px solid #525578", padding: "8px 10px", minHeight: 44, alignItems: "center" }}>
          {keywords.map(kw => (
            <span key={kw} style={{ background: "#2e3045", color: "#6366f1", borderRadius: 20, padding: "3px 11px 3px 12px", fontSize: 13, fontWeight: 600, display: "flex", alignItems: "center", gap: 6 }}>
              {kw}
      ...
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        <button onClick={handleCrawl} disabled={loading || !input.trim()} style={{ background: loading ? "#525578" : "#6366f1", color: "#0d0d11", border: "none", borderRadius: 8, padding: "11px 26px", fontWeight: 700, fontSize: 15, cursor: loading || !input.trim() ? "not-allowed" : "pointer" }}>
          {loading ? "Crawling…" : "️ Crawl & Analyze"}
        </button>
        <button onClick={() => fileInputRef.current?.click()} style={{ background: "#fbbf24", color: "#0d0d11", border: "none", borderRadius: 8, padding: "11px 20px", fontWeight: 700, fontSize: 14, cursor: "pointer" }}>Import</button>
        ...
        <div style={{ marginBottom: 24 }}>
          ...
          <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
            {[
              ...
              <div key={s.label} style={{ background: "#2e3045", borderRadius: 10, padding: "10px 18px", border: "1px solid #2e3045", textAlign: "center" }}>
          ...
          {/* Per-page sections */}
          <div style={{ fontWeight: 700, fontSize: 16, color: "#6366f1", marginBottom: 10 }}>Page-by-Page Analysis</div>
          {pages.length === 0 ? (
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 20 }}>
        <div style={{ background: "#2e3045", borderRadius: 10, padding: "10px 18px", border: "1px solid #2e3045" }}>
          <div style={{ fontSize: 10, color: "#64748b", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>Total Crawls</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: "#6366f1", marginTop: 2 }}>{history.length}</div>
        </div>
        <div style={{ background: "#2e3045", borderRadius: 10, padding: "10px 18px", border: "1px solid #2e3045" }}>
          <div style={{ fontSize: 10, color: "#64748b", fontWeight: 700, textTransform: "uppercase", letterSpacing: 1 }}>Events</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: "#6366f1", marginTop: 2 }}>{analytics.length}</div>
        </div>
      </div>

      {/* History */}
      {history.length > 0 && (
        <div style={{ background: "#252638", borderRadius: 12, padding: 18 }}>
          <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 10, color: "#6366f1" }}>Crawl History</div>
          {history.map(h => {
            const r = h.result || {};
            return (
              <div key={h.id} style={{ background: "#0d0d11", borderRadius: 8, padding: "12px 16px", border: "1px solid #2e3045", marginBottom: 8 }}>
