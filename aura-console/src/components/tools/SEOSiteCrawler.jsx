...
const S = {
  ...
  inset:   { background: "#09090b", border: "1px solid #27272a", borderRadius: 10, padding: "12px 14px" },
...
const ISSUE_CATEGORIES = [
  ...
  { id:"meta",         label:"Meta & Titles",     types:["missing-title","title-too-long","title-too-short","missing-meta-description","meta-too-long","duplicate-title","duplicate-meta"] },
...
const GEO_CHECKS = [
  ...
  { id:"citations",     label:"Outbound authority citations",         pts:5,  desc:"Citing authoritative sources increases your own citation likelihood." },
  { id:"clear-dates",   label:"Publication and modified dates",       pts:5,  desc:"datePublished and dateModified in Article schema help AI systems assess content freshness." },
];
...
function HealthGauge({ score }) {
  ...
  const arc = Math.round((score / 100) * 204);
  return (
...
function Coverage({ affected, total, label, sev }) {
  sev = sev || "medium";
  const pct = total > 0 ? Math.round((affected / total) * 100) : 0;
  const color = sev === "high" ? "#ef4444" : sev === "good" ? "#22c55e" : "#f59e0b";
  return (
...
export default function SEOSiteCrawler() {
  ...
  const addKeyword = () => {
    ...
    if (kws.length) setKeywords(p => [...p, ...kws]);
    setKwInput("");
  };
  const addExclude = () => {
  ...
  const exportReport = (format) => {
    ...
    if (format === "json") {
      content2 = JSON.stringify({ site:url, crawledAt:new Date().toISOString(), healthScore, pages }, null, 2);
      mime = "application/json"; ext = "json";
    } else {
      const rows = [["URL","Title","Status","Issues","High","Medium","Low","Words","Response(ms)"]];
      pages.forEach(p => {
        const iss = p.issues || [];
        rows.push([p.url, p.title || "", p.statusCode || 200, iss.length,
          iss.filter(i => i.severity === "high").length,
          iss.filter(i => i.severity === "medium").length,
          iss.filter(i => i.severity === "low").length,
          p.wordCount || "", p.responseTime || ""]);
      });
      content2 = rows.map(r => r.map(c => '"' + String(c || "").replace(/"/g, '""') + '"').join(",")).join("\n");
      mime = "text/csv"; ext = "csv";
    }
    const blob = new Blob([content2], { type:mime });
    const a = document.createElement("a");
    a.href = URL.createObjectURL(blob);
    a.download = "seo-crawl-" + new Date().toISOString().split("T")[0] + "." + ext;
    a.click();
  };
  ...
  const intlScore          = hreflangPages.length === 0 ? null : Math.max(0, Math.round(100 - (missingReturnTag.length * 20) - (invalidHreflang.length * 15) - (missingHtmlLang.length / Math.max(pages.length, 1) * 20)));
  ...
  const a11yScore      = pages.length ? Math.round(100 - (a11yViolations.length / pages.length) * 60 - (missingHtmlLang.length / pages.length) * 20) : 100;
  ...
  const onPageScore = Math.max(0, Math.round(100 - (missingTitlePages.length / Math.max(pages.length, 1) * 25) - (missingH1Pages.length / Math.max(pages.length, 1) * 20) - (thinContentPages.length / Math.max(pages.length, 1) * 15)));
  const perfScore   = pages.length ? Math.max(0, Math.round(100 - (slowPages.length / pages.length * 40) - (avgResponseTime > 2000 ? 30 : avgResponseTime > 1000 ? 15 : 0))) : 100;
  ...
  const quickWins = [
    ...
    thinContentPages.length > 0   && { label:thinContentPages.length + " thin content pages",         severity:"medium", action:() => { setContentView("readability"); setTab("content"); } },
    slowPages.length > 0          && { label:slowPages.length + " slow pages over 2.5s",              severity:"medium", action:() => setTab("performance") },
  ].filter(Boolean);
    ...
    <div style={S.page}>
      ...
      <div style={{ marginBottom:20 }}>
        ...
        <p style={{ fontSize:14, color:"#71717a", marginTop:4, marginBottom:0 }}>
          World-class technical SEO — 15 audit modules: security, international, redirects, robots, content quality, accessibility, AI/GEO readiness and more.
        </p>
      </div>
        ...
        <div style={{ marginTop:result ? 0 : 24 }}>
          ...
          {tab === "dashboard" && result && (
            <div style={{ marginTop:20 }}>
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:12, marginBottom:16 }}>
                <SubScore label="Technical"     score={healthScore} />
                <SubScore label="On-Page"       score={onPageScore} />
                <SubScore label="Security"      score={Math.min(100, secScore)} color="#60a5fa" />
                <SubScore label="Performance"   score={perfScore} />
                <SubScore label="Accessibility" score={Math.max(0, a11yScore)} />
              ...
              <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(300px,1fr))", gap:16 }}>
                <div style={S.card}>
                  ...
                  {quickWins.length === 0
                    ? <div style={{ color:"#4ade80", fontWeight:700, fontSize:13 }}>No critical issues — site is technically healthy!</div>
                    : quickWins.map((w, i) => (
              ...
              {selectedPage ? (
                <div style={{ ...S.card, marginBottom:20 }}>
                  ...
                  {(selectedPage.issues || []).length === 0
                    ? <div style={{ color:"#4ade80", fontWeight:700, fontSize:13 }}>No issues on this page</div>
                ...
                <div style={{ background:"#18181b", border:"1px solid #27272a", borderRadius:12, overflow:"hidden" }}>
                  <div style={{ overflowX:"auto" }}>
                    <table style={{ width:"100%", borderCollapse:"collapse" }}>
                      <thead>
                        <tr>{["URL / Title","Status","Issues","Words","Response","Links","Indexable"].map(h => <th key={h} style={S.th}>{h}</th>)}</tr>
                      </thead>
                      <tbody>
            ...
            <div style={{ marginTop:20 }}>
              ...
              {secView === "headers" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:16, lineHeight:1.6 }}>Security headers protect users from XSS, clickjacking, and protocol downgrade attacks.</p>
                  {SEC_HEADERS.map(h => {
              ...
              {secView === "mixed" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>Mixed content occurs when an HTTPS page loads resources over HTTP. Chrome marks these as "Not Secure" and blocks active mixed content.</p>
                  {mixedContent.length === 0
                    ? <div style={{ color:"#4ade80", fontWeight:700, fontSize:13 }}>✓ No mixed content detected</div>
                    : mixedContent.map((p, i) => (
                      <div key={i} style={{ padding:"10px 0", borderBottom:"1px solid #1f1f22" }}>
                        ...
                        {(p.mixedResources || []).length > 0 && <div style={{ fontSize:11, color:"#71717a", marginTop:4 }}>Mixed resources: {(p.mixedResources || []).slice(0, 3).join(", ")}</div>}
                      </div>
                    ))}
                </div>
              )}
              {secView === "forms" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>Forms submitting over HTTP expose user data. Password, payment, and personal data forms must use HTTPS.</p>
                  {insecureForms.length === 0
                    ? <div style={{ color:"#4ade80", fontWeight:700, fontSize:13 }}>✓ No insecure forms detected</div>
          ...
          {tab === "international" && result && (
            <div style={{ marginTop:20 }}>
              ...
              <div style={{ ...S.card, display:"flex", gap:20, alignItems:"center", flexWrap:"wrap", marginBottom:16 }}>
                {intlScore != null
                  ? <SubScore label="Hreflang Score" score={intlScore} color="#818cf8" />
                  : <div style={{ fontSize:13, color:"#52525b", padding:"10px 0" }}>No hreflang detected on this site</div>}
                <div style={{ flex:1, display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(110px,1fr))", gap:10 }}>
                  {[
                ...
                <div>
                  {hreflangPages.length === 0 ? (
                    <div style={S.card}>
                      ...
                      <p style={{ fontSize:13, color:"#71717a", lineHeight:1.6 }}>No hreflang annotations found. If you target multiple countries or languages, hreflang is essential — without it Google may show the wrong language version in search results.</p>
                    </div>
                  ) : (
                    <>
                      {missingReturnTag.length > 0 && (
                        <div style={{ ...S.card, border:"1px solid #f8717130" }}>
                          ...
                          <p style={{ fontSize:13, color:"#71717a", marginBottom:12, lineHeight:1.6 }}>Hreflang must be reciprocal. If page A points to page B as its Spanish translation, page B must also point back to page A. Missing return tags cause Google to ignore the entire hreflang cluster.</p>
                      ...
                      {invalidHreflang.length > 0 && (
                        <div style={{ ...S.card, border:"1px solid #fbbf2430" }}>
                          ...
                          <p style={{ fontSize:13, color:"#71717a", marginBottom:12, lineHeight:1.6 }}>Hreflang codes must use ISO 639-1 (en, es, de) optionally combined with ISO 3166-1 region codes (en-GB, es-MX). Invalid codes are silently ignored by Google.</p>
              ...
              {intlView === "htmllang" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>The HTML lang attribute tells search engines and screen readers the page language. Bing uses HTML lang (not hreflang) for targeting. WCAG 2.1 requires it for accessibility.</p>
              ...
              {intlView === "guide" && (
                <div style={S.card}>
                  ...
                  {[
                    { t:"Use absolute URLs",                   d:"Always use absolute URLs in hreflang — relative URLs are not supported and will be ignored." },
                    ...
                    { t:"Use ISO 639-1 language codes",        d:"Languages: en, es, de, fr, zh, ja. Regions: en-GB, en-US, es-MX. Language always comes before region." },
                    { t:"Add x-default for unmatched regions", d:"x-default specifies the fallback page for users in regions not covered by your cluster. Usually your primary English page." },
                    ...
                    { t:"Canonicals must align with hreflang", d:"Pages referenced in hreflang must be indexable and self-canonicalised. Pointing hreflang to a noindex URL breaks the cluster." },
          ...
          {tab === "redirects" && result && (
            <div style={{ marginTop:20 }}>
              ...
              {redirView === "chains" && (
                <div>
                  <div style={S.card}>
                    ...
                    <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>A redirect chain is when A redirects to B which redirects to C. Each extra hop loses PageRank, adds latency, and risks Googlebot abandoning the chain. Fix all chains to single direct 301 redirects.</p>
                    {redirectChains.length === 0
                      ? <div style={{ color:"#4ade80", fontWeight:700, fontSize:13 }}>✓ No redirect chains found</div>
                  ...
                  {redirectLoops.length > 0 && (
                    <div style={{ ...S.card, border:"1px solid #f8717130" }}>
                      ...
                      <p style={{ fontSize:13, color:"#71717a", marginBottom:12, lineHeight:1.6 }}>Redirect loops (A to B to A) completely block Googlebot and return errors to users. Fix immediately.</p>
              ...
              {redirView === "canonicals" && (
                <div>
                  {[
                    { list:canonicalLoops,     title:"Canonical Loops",             sev:"critical", desc:"The canonical points to a URL that canonicalises back to the original — an infinite loop that confuses Googlebot." },
                    ...
                    { list:canonicalToNoindex, title:"Canonical points to noindex", sev:"high",     desc:"A canonical pointing to a noindex page effectively asks Google not to index your content." },
                    { list:canonicalIssues.filter(p => !["loop","points-to-404","points-to-noindex"].includes(p.canonicalIssue)), title:"Other Canonical Issues", sev:"medium", desc:"Relative canonical URLs, canonical outside head, mismatched HTML vs HTTP header canonical." },
                    ...
                    <div key={title} style={S.card}>
                      ...
                      {list.length === 0
                        ? <div style={{ color:"#4ade80", fontSize:13 }}>✓ None found</div>
              ...
              {redirView === "urls" && (
                <div style={S.card}>
                  ...
                  {[
                    ...
                    { list:pages.filter(p => /[_]/.test((p.url || "").replace(/^https?:\/\/[^/]+/, ""))),                  label:"URLs with underscores",            fix:"Use hyphens instead of underscores — underscores join words rather than separating them." },
                    ...
                    { list:pages.filter(p => /[^\x00-\x7F]/.test(p.url || "")),                                            label:"URLs with non-ASCII characters",  fix:"Use ASCII-only URL slugs. Non-ASCII characters get percent-encoded, which looks ugly and breaks analytics." },
                  ].map(({ list, label, fix }) => (
            ...
            <div style={{ marginTop:20 }}>
                ...
                <div>
                  ...
                  <div style={S.card}>
                    ...
                    {[
                      ...
                      { t:"Never block CSS or JS",     d:"Google needs to render your pages. Blocking CSS/JS causes Googlebot to see a broken version of your site." },
                      ...
                      { t:"Separate AI crawler rules", d:"Control GPTBot, ClaudeBot, and PerplexityBot access separately using their specific User-agent names." },
                    ...
                    <>
                      <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))", gap:10, marginBottom:16 }}>
                        {[
                          ...
                          { label:"In Sitemap / 404",          value:result.sitemapStats.notFound || 0, color:(result.sitemapStats.notFound || 0) > 0 ? "#f87171" : "#4ade80" },
                      ...
                      {(result.sitemapStats.notFound || 0) > 0 && <div style={{ background:"#3f1315", border:"1px solid #f8717130", borderRadius:8, padding:"10px 14px", fontSize:13, color:"#f87171", marginBottom:10 }}>{result.sitemapStats.notFound} URLs in your sitemap return 404. Remove deleted pages from your sitemap immediately.</div>}
                      {(result.sitemapStats.noindex || 0) > 0 && <div style={{ background:"#3d2a0a", border:"1px solid #fbbf2430", borderRadius:8, padding:"10px 14px", fontSize:13, color:"#fbbf24" }}>{result.sitemapStats.noindex} noindex pages are listed in your sitemap. Sitemaps should only contain indexable pages.</div>}
                    </>
              ...
              {robotsView === "budget" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:16, lineHeight:1.6 }}>Crawl budget is the number of pages Googlebot will crawl on your site within a given time. Even small Shopify stores benefit from these practices.</p>
                  {[
                    { impact:"High",   t:"Block parameterised URLs",          d:"?sort=, ?page=, ?filter= create duplicate URL spaces. Use robots.txt Disallow or canonical tags to prevent infinite crawl spirals." },
                    { impact:"High",   t:"Fix all 301 chains to single hops", d:"Every extra redirect hop costs 1 crawl request. Fix chains so every 301 jumps directly to the final destination." },
                    { impact:"High",   t:"Remove 404s from internal links",   d:"Googlebot must crawl a 404 to confirm it is broken. Removing internal links to 404s frees crawl budget." },
                    { impact:"Medium", t:"Consolidate faceted navigation",    d:"Filter combinations can create millions of URLs. Use noindex or canonical for filter pages." },
                    { impact:"Medium", t:"Improve server response time",      d:"Google gives faster servers more crawl budget. Under 200ms TTFB is ideal." },
                    { impact:"Low",    t:"Use XML sitemap for prioritisation", d:"Frequently update your sitemap lastmod dates. Google uses this as a signal for recrawl priority." },
                  ].map(({ impact, t, d }) => (
            ...
            <div style={{ marginTop:20 }}>
              ...
              {structView === "orphans" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>Pages with zero inbound internal links. Googlebot cannot discover them through crawling and they receive no internal PageRank.</p>
                ...
                <div style={S.card}>
                  ...
                  {Object.entries(depthDist).sort((a, b) => Number(a[0]) - Number(b[0])).map(([d, count]) => (
                    <div key={d} style={{ marginBottom:14 }}>
                      ...
                      {Number(d) > 3 && <div style={{ fontSize:11, color:"#71717a", marginTop:3 }}>Consider flattening site hierarchy — deep pages get less crawl budget</div>}
                    </div>
                  ))}
            ...
            <div style={{ marginTop:20 }}>
              <div style={S.card}>
                ...
                <div style={{ display:"flex", gap:12, flexWrap:"wrap" }}>
                  {result.cwv
                    ? Object.keys(CWV_THRESHOLDS).map(key => <CWVChip key={key} metric={key} value={result.cwv[key]} />)
              ...
              <div style={S.card}>
                ...
                {pages.filter(p => p.responseTime).length === 0
                  ? <div style={{ fontSize:13, color:"#52525b" }}>Performance data requires a crawl with performance profiling enabled.</div>
                  : [...pages].sort((a, b) => {
              ...
              <div style={S.card}>
                <div style={S.sT}>CWV Reference Thresholds</div>
            ...
            <div style={{ marginTop:20 }}>
              ...
              {contentView === "readability" && (
                <div>
                  <div style={S.card}>
                    ...
                    <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>Google's quality guidelines favour content written for humans at an accessible reading level. Target Flesch-Kincaid score 60-70 for general ecommerce content.</p>
                    <div style={{ display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10, marginBottom:16 }}>
                      {[
                  ...
                  <div style={S.card}>
                    ...
                    {[
                      ...
                      { label:"100 to 300 words",  count:pages.filter(p => (p.wordCount || 0) >= 100 && (p.wordCount || 0) < 300).length, color:"#f59e0b", note:"Borderline thin" },
                      { label:"300 to 600 words",  count:pages.filter(p => (p.wordCount || 0) >= 300 && (p.wordCount || 0) < 600).length, color:"#22c55e", note:"Good for product pages" },
                      { label:"600 to 1200 words", count:pages.filter(p => (p.wordCount || 0) >= 600 && (p.wordCount || 0) < 1200).length,color:"#4ade80", note:"Good for collections" },
                      { label:"1200 or more",      count:pages.filter(p => (p.wordCount || 0) >= 1200).length,                           color:"#818cf8", note:"In-depth content" },
                ...
                <div>
                  <div style={S.card}>
                    ...
                    {duplicateMeta.length === 0
                      ? <div style={{ color:"#4ade80", fontWeight:700, fontSize:13 }}>✓ All meta descriptions are unique</div>
                  ...
                  <div style={S.card}>
                    ...
                    {duplicateTitles.length === 0
                      ? <div style={{ color:"#4ade80", fontWeight:700, fontSize:13 }}>✓ All title tags are unique</div>
              ...
              {contentView === "anchors" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>Generic anchors ("click here", "read more", "here") provide no SEO value and should be replaced with keyword-rich descriptive text.</p>
                  {[
                    { anchor:"click here", severity:"high",   fix:"Replace with descriptive text: e.g. 'View our summer collection'" },
                    { anchor:"here",       severity:"high",   fix:"Replace with the topic of the linked page" },
                    { anchor:"read more",  severity:"medium", fix:"Replace with: 'Read our guide to [topic]'" },
                    { anchor:"learn more", severity:"medium", fix:"Replace with: 'Learn how to [action]'" },
                    { anchor:"this page",  severity:"medium", fix:"Replace with the actual page name or primary keyword" },
                    { anchor:"link",       severity:"low",    fix:"Replace with descriptive text about the destination" },
              ...
              {contentView === "eeat" && (
                <div style={S.card}>
                  ...
                  <p style={{ fontSize:13, color:"#71717a", marginBottom:14, lineHeight:1.6 }}>E-E-A-T (Experience, Expertise, Authoritativeness, Trustworthiness) is Google's quality framework. Strong E-E-A-T signals are critical for YMYL topics and are increasingly important for all content.</p>
                  {[
                    { label:"Author bylines on content pages",          pass:pages.some(p => p.hasAuthor),                             detail:"Each piece of content should have a clear author attribution with bio, credentials, and Person schema." },
                    ...
                    { label:"Product reviews with aggregateRating",     pass:pages.some(p => (p.schemaTypes || []).includes("AggregateRating")), detail:"Reviews are strong trust signals. Implement aggregateRating in Product schema for star ratings in search." },
                    ...
                    { label:"No thin content on key pages",             pass:thinContentPages.length === 0,                            detail:"Thin pages undermine overall site quality. Google penalises sites with many thin pages." },
          ...
          {tab === "accessibility" && result && (
            <div style={{ marginTop:20 }}>
              <div style={{ display:"flex", gap:8, marginBottom:16, flexWrap:"wrap" }}>
                {[{id:"violations",l:"WCAG Violations"},{id:"overview",l:"Compliance Overview"},{id:"guide",l:"A11y Guide"}].map(v => (
                  <button key={v.id} style={S.btn(a11yView === v.id ? "primary" : null)} onClick={() => setA11yView(v.id)}>{v.l}</button>
                ))}
              </div>
              <div style={{ ...S.card, display:"flex", gap:20, alignItems:"center", flexWrap:"wrap", marginBottom:16 }}>
                <SubScore label="A11y Score" score={Math.max(0, a11yScore)} color="#a78bfa" />
                <div style={{ flex:1, display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(130px,1fr))", gap:10 }}>
                  {[
                    ...
                    { label:"Missing HTML lang",       value:missingHtmlLang.length,                                      color:missingHtmlLang.length > 0 ? "#f87171" : "#4ade80" },
              ...
              {a11yView === "overview" && (
                <div style={S.card}>
                  ...
                  {[
                    { label:"All images have descriptive alt text",   pass:!allIssues.find(i => (i.type || "").includes("alt")),           detail:"WCAG 1.1.1. Required for screen readers and image search." },
                    { label:"HTML lang attribute on all pages",       pass:missingHtmlLang.length === 0,                                   detail:"WCAG 3.1.1. Enables screen readers to use the correct language." },
                    { label:"Sufficient colour contrast",             pass:!pages.some(p => (p.a11yIssues || []).find(v => (v.rule || "").includes("color-contrast"))), detail:"WCAG 1.4.3. Text must have 4.5:1 contrast ratio against its background." },
                    { label:"Form inputs have labels",                pass:!pages.some(p => (p.a11yIssues || []).find(v => (v.rule || "").includes("label"))),          detail:"WCAG 1.3.1. Every form input needs an associated label element." },
                    { label:"Links have descriptive text",            pass:!pages.some(p => (p.a11yIssues || []).find(v => (v.rule || "").includes("link-name"))),      detail:"WCAG 2.4.4. Link text must describe the destination." },
                    ...
                    { label:"Headings not skipped (H1 to H2 to H3)", pass:!allIssues.find(i => (i.type || "").includes("heading-order")),  detail:"WCAG 1.3.1. Heading levels must be used in order." },
                    { label:"Viewport zoom not disabled",             pass:!pages.some(p => (p.a11yIssues || []).find(v => (v.rule || "").includes("meta-viewport"))),  detail:"WCAG 1.4.4. user-scalable=no prevents users with low vision from zooming in." },
              ...
              {a11yView === "guide" && (
                <div style={S.card}>
                  ...
                  {[
                    ...
                    { t:"Google uses accessibility as a quality signal", d:"Pages with poor accessibility score lower in Google's quality evaluation. Core Web Vitals include interaction metrics closely tied to accessibility." },
                    { t:"WCAG 2.1 is the international standard",        d:"Web Content Accessibility Guidelines 2.1 Level AA is the target for most commercial websites. It covers visual, motor, auditory, and cognitive disabilities." },
                    ...
                    { t:"AI agent accessibility (2025 and beyond)",      d:"Google's AI Overviews and LLM crawlers use the same accessibility signals as screen readers. Semantic HTML and ARIA labels directly affect AI content extraction." },
            ...
            <div style={{ marginTop:20 }}>
                    ...
                    {GEO_CHECKS.map(check => {
                      ...
                      return (
                        <div key={check.id} style={{ display:"flex", gap:10, padding:"9px 0", borderBottom:"1px solid #1f1f22" }}>
                          <span style={{ color:pass ? "#4ade80" : "#f87171", fontSize:16, flexShrink:0 }}>{pass ? "✓" : "✗"}</span>
                          <div style={{ flex:1, fontSize:13, fontWeight:600, color:pass ? "#e4e4e7" : "#a1a1aa" }}>{check.label}</div>
                          <span style={{ fontWeight:700, color:pass ? "#4ade80" : "#52525b", fontSize:13 }}>{pass ? "+" : ""}{check.pts} pts</span>
                        </div>
                      );
                    })}
                  </div>
              ...
              {geoView === "llm" && (
                <div style={S.card}>
                  ...
                  {[
                    { bot:"GPTBot",        company:"OpenAI",     uses:"ChatGPT and GPT-4 training and browsing. Millions of daily citations in ChatGPT responses." },
                    { bot:"ChatGPT-User",  company:"OpenAI",     uses:"ChatGPT browse-with-Bing feature. Used for real-time information retrieval." },
                    { bot:"ClaudeBot",     company:"Anthropic",  uses:"Claude AI model training and real-time knowledge retrieval." },
                    { bot:"PerplexityBot", company:"Perplexity", uses:"Perplexity AI search — one of the fastest-growing AI search engines with 50M+ users." },
                    { bot:"Googlebot",     company:"Google",     uses:"Google Search, AI Overviews, and Gemini AI. Blocking this removes you from Google entirely." },
                    { bot:"Bingbot",       company:"Microsoft",  uses:"Bing Search and Copilot AI. Microsoft Copilot uses Bing index for all AI answers." },
                  ].map(({ bot, company, uses }) => {
                    const blocked = (result.robotsStats?.blocked || []).includes(bot);
                    return (
                      <div key={bot} style={{ padding:"10px 0", borderBottom:"1px solid #1f1f22" }}>
                        ...
                        {blocked && <div style={{ marginLeft:26, marginTop:6, fontSize:12, color:"#fbbf24" }}>Fix: Remove the Disallow rule for {bot} in robots.txt to allow AI citation opportunities.</div>}
                      </div>
                    );
                  })}
                </div>
                  ...
                  <div style={S.card}>
                    ...
                    <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                      <div>
                        ...
                        {["Optimise for keyword rankings","Target Google's 10 blue links","Focus on backlink building","Measure organic click traffic","Title and meta for SERP snippets"].map(t => <div key={t} style={{ fontSize:12, color:"#a1a1aa", marginBottom:6, display:"flex", gap:8 }}><span style={{ color:"#52525b" }}>{">"}</span>{t}</div>)}
                      </div>
                      <div>
                        ...
                        {["Optimise for AI citation frequency","Target AI overview slots","Focus on E-E-A-T and structured data","Measure brand mentions in AI responses","FAQ and HowTo schema for AI extraction"].map(t => <div key={t} style={{ fontSize:12, color:"#c7d2fe", marginBottom:6, display:"flex", gap:8 }}><span style={{ color:"#818cf8" }}>{">"}</span>{t}</div>)}
                      </div>
                    </div>
                  </div>
                  <div style={S.card}>
                    ...
                    {[
                      { t:"Add FAQPage schema to product pages",       d:"Add a FAQ section answering common questions about each product, wrapped in FAQPage JSON-LD." },
                      { t:"Implement Product schema fully",             d:"Ensure Product JSON-LD includes name, description, brand, offers, aggregateRating, and image." },
                      { t:"Create comparison and buyer guide content",  d:"Best X for Y and How to choose X content is heavily cited by AI in response to buying intent queries." },
                      { t:"Add author schema to blog posts",            d:"Give each blog author a Person JSON-LD with jobTitle, knowsAbout, and sameAs social profiles." },
                      { t:"Use direct answer format",                   d:"Start product descriptions with a clear, direct sentence answering what this product is. AI systems cite this sentence directly." },
          ...
          {tab === "schedules" && (
            <div style={{ marginTop:20 }}>
              ...
              {schedules.length === 0
                ? <EmptyState icon="Calendar" title="No scheduled crawls yet" description="Create a schedule above to automatically monitor SEO health." />
                : schedules.map((s, i) => (
            ...
            <div style={{ marginTop:20 }}>
              <div style={S.card}>
                ...
                {[
                  { priority:"P0 — Crawlability Blockers", sev:"critical", items:[
                    { issue:"Noindex on live pages",      fix:"Remove noindex from all pages you want ranked. Check theme templates, app injections, and page-level settings." },
                    ...
                    { issue:"500/503 server errors",      fix:"5xx errors cause Googlebot to reduce crawl frequency and directly harm rankings. Fix server errors immediately." },
                    { issue:"Redirect loops",             fix:"A to B to A redirect loops completely block Googlebot. Detect with this crawler and fix immediately by breaking the cycle." },
                  ]},
                  { priority:"P1 — Indexability Signals", sev:"high", items:[
                    { issue:"Missing/duplicate title tags", fix:"Title is the strongest on-page signal. Max 60 chars. Primary keyword first. Unique on every page." },
                    { issue:"Missing canonical tags",       fix:"Self-canonical every page. Shopify creates duplicate URLs — canonical must point to /products/slug." },
                    { issue:"Missing meta descriptions",    fix:"150-160 chars. Include a call to action and primary keyword. Critical for CTR and AI citation excerpts." },
                  ...
                  { priority:"P2 — Content Quality", sev:"medium", items:[
                    { issue:"Missing H1 headings",          fix:"One H1 per page containing the primary keyword. H1 and title tag can differ — both are important." },
                    ...
                    { issue:"Missing image alt text",       fix:"Describe the image naturally including the product name. Alt text serves both accessibility and image search SEO." },
                    { issue:"Duplicate content",            fix:"Shopify creates canonical duplicates. Always canonical to the shortest canonical URL (/products/slug)." },
                  ]},
                  { priority:"P3 — Authority & Visibility", sev:"low", items:[
                    ...
                    { issue:"Slow response times",          fix:"CDN, image compression (WebP/AVIF), browser caching, app performance. Each 100ms reduction improves Core Web Vitals." },
                    ...
                    { issue:"Missing GEO signals",          fix:"Add FAQPage schema, author attribution, and direct-answer content formats. AI systems cite structured pages 3x more." },
                  ]},
              ...
              <div style={S.card}>
                ...
                {[
                  ...
                  { issue:"Paginated collections indexed",    fix:"/collections/t-shirts?page=2 creates thin duplicates. Canonical to the first page." },
                  ...
                  { issue:"App-injected noindex",             fix:"Some apps accidentally inject noindex. Audit every page type after installing new apps." },
                  ...
                  { issue:"Missing aggregateRating schema",   fix:"Reviews are your strongest trust signal in search results. Implement aggregateRating in Product JSON-LD." },
                  { issue:"GPTBot / ClaudeBot blocked",       fix:"If you block AI crawlers in robots.txt, your content cannot be cited by ChatGPT or Claude. Unblock for citation opportunities." },
                ].map(({ issue, fix }) => (
              ...
              <div style={{ display:"grid", gridTemplateColumns:"1fr 1fr", gap:16 }}>
                <div style={S.card}>
                  <div style={S.sT}>Health Score Calculation</div>
                  {[
