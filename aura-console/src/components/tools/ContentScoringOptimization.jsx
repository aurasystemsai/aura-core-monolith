...
export default function ContentScoringOptimization() {
  ...
  const handleScore = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/content-scoring-optimization/score", {
        method: "POST",
      ...
      setHistory(prev => [{ content, result: data.result }, ...prev].slice(0, 10));
    } catch (err) {
  ...
  const onboardingContent = (
    <div style={{ padding: 24, background: darkMode ? "#23263a" : "#f1f5f9", borderRadius: 12, marginBottom: 18 }}>
      <h3 style={{ fontWeight: 700, fontSize: 22 }}>Welcome to Content Scoring & Optimization</h3>
      <ul style={{ margin: "16px 0 0 18px", color: darkMode ? "#a3e635" : "#334155", fontSize: 16 }}>
        <li>Grade content, analyze NLP/topic coverage, and get suggestions</li>
        <li>Export, share, and review scoring history</li>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <h2 style={{ fontWeight: 800, fontSize: 32, margin: 0 }}>Content Scoring & Optimization</h2>
      ...
      <div style={{ marginBottom: 10, color: darkMode ? "#a3e635" : "#0ea5e9", fontWeight: 600 }}>
        <span role="img" aria-label="content">📝</span> Grade and optimize your content.
      </div>
      ...
      <textarea
        ...
        placeholder="Paste your content here for scoring and optimization..."
        aria-label="Content input"
      />
      <button onClick={handleScore} disabled={loading || !content} style={{ background: "#a3e635", color: "#23263a", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 16, cursor: "pointer", marginBottom: 18 }}>{loading ? "Scoring..." : "Score & Optimize"}</button>
      ...
      {result && (
        <div style={{ background: darkMode ? "#23263a" : "#f1f5f9", borderRadius: 10, padding: 16, marginBottom: 12, color: darkMode ? "#a3e635" : "#23263a" }}>
          <div style={{ fontWeight: 600, marginBottom: 4 }}>Scoring Result:</div>
      ...
      {history.length > 0 && (
        <div style={{ marginTop: 24, background: darkMode ? "#334155" : "#f3f4f6", borderRadius: 12, padding: 18 }}>
          <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>Scoring History</div>
