...
export default function BlogDraftEngine() {
  ...
  const handleRun = async () => {
    ...
    setResponse("");
    try {
      const res = await fetch("/api/blog-draft-engine/ai/generate", {
        method: "POST",
  ...
  return (
    <div style={{ maxWidth: 600, margin: "40px auto", background: "#fff", borderRadius: 16, boxShadow: "0 2px 16px #0001", padding: 32 }}>
      <h2 style={{ fontWeight: 700, fontSize: 28, marginBottom: 12 }}>Blog Draft Engine</h2>
      <p style={{ color: "#444", marginBottom: 18 }}>
        Enter a blog topic or prompt below. The AI will generate a draft blog post.
      </p>
      <textarea
        value={input}
