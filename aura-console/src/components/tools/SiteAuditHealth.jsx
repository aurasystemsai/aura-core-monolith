  ...
  return (
    <div style={{ background: "#09090b", color: "#f3f4f6", borderRadius: 18, boxShadow: "0 2px 24px #0002", padding: 36, fontFamily: "Inter, sans-serif" }}>
      ...
      <input
        ...
        type="text"
        style={{ width: "100%", fontSize: 16, padding: 12, borderRadius: 8, border: "1px solid #52525b", marginBottom: 16, background: "#27272a", color: "#f3f4f6", boxSizing: "border-box" }}
        placeholder="https://yoursite.com"
        aria-label="Site URL"
        onKeyDown={e => e.key === "Enter" && !loading && input && handleAudit()}
      />
      <div style={{ display: "flex", gap: 10, marginBottom: 20, flexWrap: "wrap" }}>
        <button onClick={handleAudit} disabled={loading || !input} style={{ background: "#818cf8", color: "#09090b", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 15, cursor: loading || !input ? "not-allowed" : "pointer", opacity: loading || !input ? 0.6 : 1 }}>
          {loading ? "Auditing…" : "Run Audit"}
        </button>
        <button onClick={() => fileInputRef.current?.click()} style={{ background: "#52525b", color: "#f3f4f6", border: "1px solid #475569", borderRadius: 8, padding: "10px 18px", fontWeight: 600, fontSize: 14, cursor: "pointer" }}>Import</button>
        <input ref={fileInputRef} type="file" accept=".json" style={{ display: "none" }} onChange={handleImport} />
        <button onClick={handleExport} disabled={!history.length} style={{ background: "#52525b", color: "#f3f4f6", border: "1px solid #475569", borderRadius: 8, padding: "10px 18px", fontWeight: 600, fontSize: 14, cursor: history.length ? "pointer" : "not-allowed", opacity: history.length ? 1 : 0.5 }}>Export JSON</button>
        {exported && <a href={exported} download="site-audit-history.json" style={{ alignSelf: "center", color: "#818cf8", fontWeight: 600, fontSize: 13 }}>Download</a>}
      </div>
      ...
      {response && (
        <div style={{ background: "#27272a", borderRadius: 12, padding: 20, marginBottom: 20, border: "1px solid #27272a" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
            <span style={{ fontWeight: 700, color: "#818cf8", fontSize: 15 }}>Audit Report</span>
      ...
      <div style={{ display: "flex", gap: 12, marginBottom: 20, flexWrap: "wrap" }}>
        {[{ label: "Total Audits", value: history.length }, { label: "Events Tracked", value: analytics.length }].map(s => (
          <div key={s.label} style={{ background: "#27272a", borderRadius: 10, padding: "14px 22px", border: "1px solid #27272a", minWidth: 130 }}>
            ...
            <div style={{ fontSize: 28, fontWeight: 800, color: "#818cf8", marginTop: 4 }}>{s.value}</div>
          </div>
        ))}
      </div>
      {/* History */}
      <div style={{ background: "#18181b", borderRadius: 12, padding: 18, marginBottom: 20 }}>
        <div style={{ fontWeight: 700, fontSize: 16, marginBottom: 12, color: "#818cf8" }}>Audit History</div>
        {history.length === 0 ? (
