...
export default function SiteAuditHealth() {
  ...
  const handleAudit = async () => {
    ...
    setResult(null);
    try {
      const res = await fetch("/api/site-audit-health/audit", {
        method: "POST",
      ...
      setHistory(prev => [{ url, result: data.result }, ...prev].slice(0, 10));
    } catch (err) {
  ...
  const onboardingContent = (
    <div style={{ padding: 24, background: darkMode ? "#23263a" : "#f1f5f9", borderRadius: 12, marginBottom: 18 }}>
      <h3 style={{ fontWeight: 700, fontSize: 22 }}>Welcome to Site Audit & Health</h3>
      <ul style={{ margin: "16px 0 0 18px", color: darkMode ? "#a3e635" : "#334155", fontSize: 16 }}>
        <li>Audit web vitals, mobile, security, structured data, and duplicate content</li>
        <li>Export, share, and review audit history</li>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
        <h2 style={{ fontWeight: 800, fontSize: 32, margin: 0 }}>Site Audit & Health</h2>
      ...
      <div style={{ marginBottom: 10, color: darkMode ? "#a3e635" : "#0ea5e9", fontWeight: 600 }}>
        <span role="img" aria-label="audit">🩺</span> Audit site health, vitals, and compliance.
      </div>
      ...
      <input
        ...
        aria-label="Site audit input"
      />
      <button onClick={handleAudit} disabled={loading || !url} style={{ background: "#a3e635", color: "#23263a", border: "none", borderRadius: 8, padding: "10px 22px", fontWeight: 700, fontSize: 16, cursor: "pointer", marginBottom: 18 }}>{loading ? "Auditing..." : "Audit"}</button>
      ...
      {result && (
        <div style={{ background: darkMode ? "#23263a" : "#f1f5f9", borderRadius: 10, padding: 16, marginBottom: 12, color: darkMode ? "#a3e635" : "#23263a" }}>
          <div style={{ fontWeight: 600, marginBottom: 4 }}>Audit Result:</div>
      ...
      {history.length > 0 && (
        <div style={{ marginTop: 24, background: darkMode ? "#334155" : "#f3f4f6", borderRadius: 12, padding: 18 }}>
          <div style={{ fontWeight: 700, fontSize: 18, marginBottom: 8 }}>Audit History</div>
