...
export default function AutomationScheduler() {
  ...
  return (
    <div className="automation-scheduler" style={{ background: 'var(--background-secondary)', color: 'var(--text-primary)', borderRadius: 16, padding: 32, maxWidth: 600, margin: '40px auto' }}>
      ...
      {showForm && (
        <form onSubmit={handleSubmit} style={{ marginBottom: 32, background: '#1a1c25', padding: 24, borderRadius: 12 }}>
      ...
      <div>
        ...
        {loading ? (
          ...
          <ul style={{ listStyle: 'none', padding: 0 }}>
            {schedules.map(s => (
              <li key={s.id} style={{ background: '#1a1c25', marginBottom: 12, borderRadius: 8, padding: 16, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
