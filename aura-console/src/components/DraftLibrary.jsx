...
function DraftLibrary({ coreUrl, projectId }) {
 ...
 <button
 className="button button--ghost button--tiny"type="button"onClick={loadList}
 ...
 className="button button--ghost"style={{
 textAlign: "left",
 padding: 10,
 ...
 type="button"onClick={() => setFormat("text")}
 ...
 <button
 className="button button--ghost button--tiny"type="button"disabled={!selected}
 onClick={() =>
 ...
 {format === "html"? "HTML output (paste into an HTML editor). Use this if your CMS supports HTML.": "Plain text output (best for most CMS editors). Use this for Shopify, WordPress, etc."}
 </div>

 <pre className="raw-json-pre"style={{ minHeight: 420 }}>
 {selected
 ? format === "html"? selected.articleHtml || "// No HTML saved for this draft.": selected.articleText || "// No text saved for this draft.": "// Select a draft from the left."}
 </pre>
 </div>
 </div>
 </div>
