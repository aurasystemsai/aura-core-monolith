...
function ScoreRing({ score, size = 88 }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 6 }}>
      <div style={{ width: size, height: size, borderRadius: '50%', background: `conic-gradient(${colour} ${score * 3.6}deg, #2e2e2e ${score * 3.6}deg)`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 20px ${colour}33` }}>
        <div style={{ width: inner, height: inner, borderRadius: '50%', background: '#141414', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center' }}>
...
function CharBar({ value = '', min, max, sweetMin, sweetMax, label }) {
  ...
  const colour = len === 0 ? '#4a4a4a' : over ? '#f87171' : perfect ? '#4ade80' : inRange ? '#a3e635' : '#fbbf24';
  ...
  return (
    <div style={{ marginTop: 6 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 4 }}>
        <span style={{ fontSize: 11, color: '#777777' }}>{label}</span>
      ...
      <div style={{ height: 5, background: '#2e2e2e', borderRadius: 3, overflow: 'hidden' }}>
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 12, padding: isMobile ? '12px 14px' : '16px 20px', fontFamily: 'Arial, sans-serif', boxShadow: '0 2px 12px #0001' }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8 }}>
        ...
        <div>
          <div style={{ fontSize: 13, color: '#2a2a2a', fontWeight: 500 }}>{shop || 'yourstore.myshopify.com'}</div>
      ...
      <div style={{ fontSize: isMobile ? 16 : 20, color: '#4a4a4a', marginBottom: 6, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
...
function GenBtn({ onClick, loading, small }) {
  return (
    <button onClick={onClick} disabled={loading} style={{ background: loading ? '#3c3c3c' : '#ffffff', border: 'none', borderRadius: small ? 6 : 8, padding: small ? '5px 12px' : '7px 16px', color: '#000000', fontWeight: 700, fontSize: small ? 12 : 13, cursor: loading ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0, opacity: loading ? 0.5 : 1 }}>
...
function FieldBlock({ label, hint, badge, children }) {
  return (
    <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 12, padding: 18 }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
        <div>
          <div style={{ fontWeight: 600, color: '#cccccc', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.06em' }}>{label}</div>
          {hint && <div style={{ fontSize: 11, color: '#777777', marginTop: 2 }}>{hint}</div>}
        </div>
        {badge}
      </div>
      {children}
...
export default function ProductSeoEngine() {
  ...
  return (
    <div style={{ background: '#141414', minHeight: '100vh', color: '#f0f0f0', fontFamily: '"Inter", system-ui, sans-serif' }}>
      ...
      <div style={{ background: '#0f0f0f', borderBottom: '1px solid #2e2e2e', padding: '0 32px' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 22, paddingBottom: 18 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 14 }}>
            ...
            <div>
              ...
              <p style={{ margin: '2px 0 0', fontSize: 12, color: '#777777' }}>AI-powered SEO optimisation and direct Shopify publishing</p>
            </div>
          </div>
          <div style={{ display: 'flex', gap: 8, alignItems: 'center' }}>
            {[
              { label: 'Products', value: products.length, colour: '#ffffff' },
              { label: 'Optimised', value: Object.keys(stored).length, colour: '#bbbbbb' },
              ...
              <div key={st.label} style={{ background: '#212121', border: '1px solid #323232', borderRadius: 10, padding: '9px 16px', textAlign: 'center', minWidth: 76 }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: st.colour }}>{st.value}</div>
                <div style={{ fontSize: 11, color: '#777777', marginTop: 1 }}>{st.label}</div>
              </div>
            ))}
            <button onClick={loadProducts} style={{ background: '#212121', border: '1px solid #323232', borderRadius: 10, padding: '9px 16px', color: '#bbbbbb', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Refresh</button>
          </div>
        </div>
        <div style={{ display: 'flex', borderTop: '1px solid #2e2e2e' }}>
          {TABS.map(t => (
            <button key={t.id} onClick={() => setTab(t.id)} style={{ background: 'none', border: 'none', borderBottom: tab === t.id ? '2px solid #ffffff' : '2px solid transparent', padding: '13px 22px', cursor: 'pointer', color: tab === t.id ? '#ffffff' : '#777777', fontWeight: tab === t.id ? 600 : 400, fontSize: 13, transition: 'color 0.15s' }}>
              {t.label}
            </button>
          ))}
        </div>
      ...
      <div style={{ padding: '28px 32px', maxWidth: 1400, margin: '0 auto' }}>
          ...
          <div>
            <div style={{ display: 'flex', gap: 10, marginBottom: 24, flexWrap: 'wrap', alignItems: 'center' }}>
              <input placeholder="Search products..." value={search} onChange={e => setSearch(e.target.value)}
                style={{ flex: 1, minWidth: 200, background: '#282828', border: '1px solid #3c3c3c', borderRadius: 10, padding: '11px 14px', color: '#f0f0f0', fontSize: 14, outline: 'none' }} />
              <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ background: '#282828', border: '1px solid #3c3c3c', borderRadius: 10, padding: '11px 14px', color: '#f0f0f0', fontSize: 14, cursor: 'pointer' }}>
                <option value="all">All Status</option>
                <option value="active">Active</option>
                <option value="draft">Draft</option>
                <option value="archived">Archived</option>
              </select>
              <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ background: '#282828', border: '1px solid #3c3c3c', borderRadius: 10, padding: '11px 14px', color: '#f0f0f0', fontSize: 14, cursor: 'pointer' }}>
            ...
            {loadingProducts ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
                {[...Array(8)].map((_, i) => <div key={i} style={{ background: '#141414', borderRadius: 16, height: 130, animation: 'pulse 1.5s ease infinite' }} />)}
              </div>
            ) : filteredProducts.length === 0 ? (
              <div style={{ textAlign: 'center', padding: '80px 0', color: '#777777' }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#777777', marginBottom: 8 }}>No products found</div>
                ...
                {filteredProducts.map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => selectProduct(p)}
                      style={{ background: '#282828', border: '1px solid ' + (hasData ? '#2e2e2e' : '#2a1515'), borderRadius: 16, padding: 18, cursor: 'pointer', transition: 'all 0.2s', position: 'relative' }}
                      ...
                      onMouseLeave={e => { e.currentTarget.style.borderColor = hasData ? '#2e2e2e' : '#2a1515'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ''; }}
                    >
                        <div style={{ position: 'absolute', top: 14, right: 14, background: hasData ? pb : '#282828', border: '1px solid ' + (hasData ? pc : '#2e2e2e') + '40', borderRadius: 8, padding: '4px 10px', textAlign: 'center' }}>
                        <div style={{ fontSize: 16, fontWeight: 900, color: hasData ? pc : '#4a4a4a' }}>{hasData ? ps : '-'}</div>
                        <div style={{ fontSize: 9, color: (hasData ? pc : '#4a4a4a') + 'aa', fontWeight: 600 }}>SEO</div>
                      </div>
                      <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', paddingRight: 56 }}>
                        {p.image
                          ? <img src={p.image} alt={p.title} style={{ width: 56, height: 56, borderRadius: 12, objectFit: 'cover', flexShrink: 0, border: '1px solid #2e2e2e' }} />
                          : <div style={{ width: 56, height: 56, borderRadius: 12, background: '#212121', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#777777', flexShrink: 0 }}>IMG</div>}
                        <div style={{ minWidth: 0, flex: 1 }}>
                          <div style={{ fontWeight: 700, fontSize: 15, color: '#f0f0f0', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</div>
                          <div style={{ fontSize: 12, color: '#777777', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.vendor} - {p.handle}</div>
                      ...
                      {hasData && (
                        <div style={{ marginTop: 12, paddingTop: 10, borderTop: '1px solid #2e2e2e' }}>
                          <div style={{ height: 3, background: '#2e2e2e', borderRadius: 2, overflow: 'hidden' }}>
          ...
          <div>
            {!selectedProduct ? (
              <div style={{ textAlign: 'center', padding: '80px 0' }}>
                <div style={{ fontSize: 20, fontWeight: 700, color: '#777777', marginBottom: 20 }}>Select a product first</div>
              ...
              <div>
                <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 16, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, flexWrap: 'wrap' }}>
                  {selectedProduct.image && <img src={selectedProduct.image} alt={selectedProduct.title} style={{ width: 52, height: 52, borderRadius: 12, objectFit: 'cover', border: '1px solid #2e2e2e' }} />}
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontWeight: 800, fontSize: 17, color: '#f0f0f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{selectedProduct.title}</div>
                    <div style={{ fontSize: 12, color: '#777777', marginTop: 2 }}>{selectedProduct.vendor} - /products/{selectedProduct.handle}</div>
                  </div>
                  {savedAt && <div style={{ fontSize: 11, color: '#777777', whiteSpace: 'nowrap' }}>Auto-saved {savedAt.toLocaleTimeString()}</div>}
                  <div style={{ display: 'flex', gap: 10, flexShrink: 0, flexWrap: 'wrap' }}>
                    <button onClick={generateAll} disabled={generating}
                      style={{ background: generating ? '#3c3c3c' : '#ffffff', border: 'none', borderRadius: 12, padding: '10px 22px', color: generating ? '#777777' : '#000000', fontWeight: 700, cursor: generating ? 'wait' : 'pointer', fontSize: 14 }}>
                    ...
                    <button onClick={() => { saveLocal(selectedProduct.id, editor); setSavedAt(new Date()); showToast('Draft saved'); }}
                      style={{ background: '#141414', border: '1px solid #2e2e2e', borderRadius: 12, padding: '10px 18px', color: '#9a9a9a', fontWeight: 600, cursor: 'pointer', fontSize: 13 }}>Save Draft</button>
                    <button onClick={pushToShopify} disabled={pushing || !editor.seoTitle}
                      style={{ background: pushing || !editor.seoTitle ? '#3c3c3c' : 'linear-gradient(135deg, #7c3aed, #6d28d9)', border: 'none', borderRadius: 12, padding: '10px 22px', color: '#fff', fontWeight: 700, cursor: pushing ? 'wait' : 'pointer', fontSize: 14 }}>
                      {pushing ? 'Pushing...' : 'Push to Shopify'}
                    </button>
                  </div>
                </div>

                <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: '#282828', border: '1px solid #2e2e2e', borderRadius: 12, padding: 4, width: 'fit-content' }}>
                  {[['fields', 'Fields'], ['preview', 'SERP Preview'], ['checklist', 'SEO Checklist']].map(([id, lbl]) => (
                    <button key={id} onClick={() => setEditorTab(id)} style={{ background: editorTab === id ? 'rgba(255,255,255,0.08)' : 'none', border: editorTab === id ? '1px solid rgba(255,255,255,0.15)' : '1px solid transparent', borderRadius: 8, padding: '7px 16px', color: editorTab === id ? '#ffffff' : '#777777', fontWeight: editorTab === id ? 600 : 400, fontSize: 13, cursor: 'pointer' }}>
                      {lbl}
                    </button>
                  ))}
                </div>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      ...
                      <FieldBlock label="SEO Title" hint="Shown in browser tab and Google search results" badge={<GenBtn onClick={() => generateField('seoTitle')} loading={!!fieldGenerating.seoTitle} />}>
                        <input value={editor.seoTitle} onChange={e => setEditor(ed => ({ ...ed, seoTitle: e.target.value }))}
                          ...
                          style={{ width: '100%', background: '#181818', border: '1px solid ' + (editor.seoTitle.length > 60 ? '#f87171' : '#2e2e2e'), borderRadius: 10, padding: '11px 14px', color: '#f0f0f0', fontSize: 15, outline: 'none', boxSizing: 'border-box' }} />
                      ...
                      <FieldBlock label="Meta Description" hint="Shown below title in search results - Google truncates at ~155 chars on desktop, no hard limit" badge={<GenBtn onClick={() => generateField('seoDescription')} loading={!!fieldGenerating.seoDescription} />}>
                        <textarea value={editor.seoDescription} onChange={e => setEditor(ed => ({ ...ed, seoDescription: e.target.value }))}
                          ...
                          rows={4}
                          style={{ width: '100%', background: '#181818', border: '1px solid ' + (editor.seoDescription.length > 160 ? '#f87171' : '#2e2e2e'), borderRadius: 10, padding: '11px 14px', color: '#f0f0f0', fontSize: 14, outline: 'none', resize: 'vertical', lineHeight: 1.6, boxSizing: 'border-box' }} />
                      ...
                      <FieldBlock label="URL Handle / Slug" hint="Use lowercase letters, numbers and hyphens only" badge={<GenBtn onClick={() => generateField('handle')} loading={!!fieldGenerating.handle} small />}>
                        <div style={{ display: 'flex', background: '#181818', border: '1px solid #2e2e2e', borderRadius: 10, overflow: 'hidden' }}>
                          <span style={{ padding: '11px 12px', color: '#555555', fontSize: 13, borderRight: '1px solid #2e2e2e', whiteSpace: 'nowrap' }}>/products/</span>
                          <input value={editor.handle} onChange={e => setEditor(ed => ({ ...ed, handle: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') }))}
                            style={{ flex: 1, background: 'none', border: 'none', padding: '11px 14px', color: '#f0f0f0', fontSize: 14, outline: 'none' }} />
                        </div>
                      </FieldBlock>
                      ...
                      <FieldBlock label="Focus Keywords" hint="Guide the AI copy - not a meta tag ranking signal (Google ignores meta keywords since 2009)" badge={<GenBtn onClick={() => generateField('keywords')} loading={!!fieldGenerating.keywords} small />}>
                        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, background: '#181818', border: '1px solid #2e2e2e', borderRadius: 10, padding: '8px 10px', minHeight: 50, alignItems: 'center' }}>
                          {chips.map(kw => {
                          ...
                          <input value={kwInput} onChange={e => setKwInput(e.target.value)}
                            ...
                            style={{ flex: 1, minWidth: 120, background: 'none', border: 'none', color: '#f0f0f0', fontSize: 13, outline: 'none', padding: '3px 4px' }} />
                        </div>
                        {chips.length > 0 && <div style={{ marginTop: 8, fontSize: 11, color: '#777777' }}>Green = in title and desc / Yellow = in one / Red = in neither</div>}
                      </FieldBlock>
                      ...
                      <FieldBlock label="Image Alt Text" hint="Describe the main product image for accessibility and SEO" badge={<GenBtn onClick={() => generateField('altText')} loading={!!fieldGenerating.altText} small />}>
                        <input value={editor.altText} onChange={e => setEditor(ed => ({ ...ed, altText: e.target.value }))}
                          ...
                          style={{ width: '100%', background: '#181818', border: '1px solid #2e2e2e', borderRadius: 10, padding: '11px 14px', color: '#f0f0f0', fontSize: 14, outline: 'none', boxSizing: 'border-box' }} />
                      </FieldBlock>
                    </div>

                    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
                      <div style={{ background: '#282828', border: '1px solid ' + scoreColour + '30', borderRadius: 16, padding: 22 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 18 }}>
                          <ScoreRing score={score} />
                          <div>
                            <div style={{ fontWeight: 800, fontSize: 18, color: '#f0f0f0' }}>SEO Health Score</div>
                            <div style={{ fontSize: 13, color: '#777777', marginTop: 3 }}>Updates as you type</div>
                        ...
                        {(() => {
                          ...
                          return [
                            ...
                            <div key={item.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', background: '#181818', borderRadius: 8, marginBottom: 5 }}>
                              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                                <span style={{ fontSize: 11, color: item.ok ? '#4ade80' : '#f87171', fontWeight: 900 }}>{item.ok ? 'Y' : 'N'}</span>
                                <span style={{ fontSize: 12, color: '#bbbbbb' }}>{item.label}</span>
                              </div>
                              <div style={{ display: 'flex', gap: 8 }}>
                                <span style={{ fontSize: 11, color: item.ok ? '#4ade80' : '#777777' }}>{item.msg}</span>
                                <span style={{ fontSize: 11, fontWeight: 700, color: item.earned > 0 ? '#7fffd4' : '#555555', background: item.earned > 0 ? '#7fffd410' : 'none', border: '1px solid ' + (item.earned > 0 ? '#7fffd420' : '#2e2e2e'), borderRadius: 4, padding: '1px 6px' }}>{item.earned}/{item.pts}</span>
                              </div>
                            </div>
                          ));
                        })()}
                      ...
                      {chips.length > 0 && (
                        <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 16, padding: 18 }}>
                          <div style={{ fontWeight: 600, fontSize: 13, color: '#bbbbbb', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Keyword Presence</div>
                          {chips.slice(0, 8).map(kw => {
                            const inT = kwInTitleMatch(kw);
                            const inD = kwInDescMatch(kw);
                            return (
                              <div key={kw} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#181818', borderRadius: 8, marginBottom: 5 }}>
                                <span style={{ fontSize: 12, color: '#bbbbbb', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 130 }}>"{kw}"</span>
                  ...
                  <div>
                    <div style={{ display: 'flex', gap: 8, marginBottom: 20 }}>
                      {[['desktop', 'Desktop'], ['mobile', 'Mobile']].map(([d, lbl]) => (
                        <button key={d} onClick={() => setSerpDevice(d)} style={{ background: serpDevice === d ? 'rgba(255,255,255,0.08)' : '#282828', border: '1px solid ' + (serpDevice === d ? 'rgba(255,255,255,0.2)' : '#2e2e2e'), borderRadius: 10, padding: '8px 18px', color: serpDevice === d ? '#ffffff' : '#777777', fontWeight: serpDevice === d ? 600 : 400, fontSize: 13, cursor: 'pointer' }}>{lbl}</button>
                      ))}
                    </div>
                    <div style={{ maxWidth: serpDevice === 'mobile' ? 400 : 680, margin: '0 auto' }}>
                      <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 14, padding: 20, marginBottom: 16 }}>
                        <div style={{ fontSize: 12, color: '#777777', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>Google Search Result Preview</div>
                      ...
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 12 }}>
                        <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 12, padding: 14 }}>
                          <div style={{ fontSize: 11, color: '#777777', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Title</div>
                        ...
                        <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 12, padding: 14 }}>
                          <div style={{ fontSize: 11, color: '#777777', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Description</div>
                ...
                {editorTab === 'checklist' && (
                  <div style={{ maxWidth: 680 }}>
                    <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 16, padding: 22 }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
                        <div style={{ fontWeight: 800, fontSize: 17, color: '#f0f0f0' }}>SEO Checklist</div>
                      ...
                      <div style={{ height: 6, background: '#2e2e2e', borderRadius: 3, overflow: 'hidden', marginBottom: 18 }}>
                      ...
                      {checklist.map((item, i) => (
                        <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 14px', background: item.ok ? '#071a10' : '#141414', border: '1px solid ' + (item.ok ? '#4ade8020' : '#2e2e2e'), borderRadius: 10, marginBottom: 6 }}>
                          ...
                          <div>
                            <div style={{ fontSize: 13, fontWeight: 600, color: item.ok ? '#f0f0f0' : '#bbbbbb' }}>{item.label}</div>
        ...
        {tab === 'bulk' && (
          <div>
            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 24, gap: 16, flexWrap: 'wrap' }}>
              <div>
                <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#f0f0f0' }}>Bulk SEO Generation</h2>
                <p style={{ margin: '6px 0 0', fontSize: 14, color: '#777777' }}>Select products and generate SEO for all at once then publish</p>
              </div>
              <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
                <button onClick={() => setBulkSelected(new Set(products.map(p => p.id)))} style={{ background: '#141414', border: '1px solid #2e2e2e', borderRadius: 10, padding: '10px 16px', color: '#9a9a9a', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>Select All ({products.length})</button>
                <button onClick={() => setBulkSelected(new Set())} style={{ background: '#141414', border: '1px solid #2e2e2e', borderRadius: 10, padding: '10px 16px', color: '#9a9a9a', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>Clear</button>
                <button onClick={runBulkGenerate} disabled={bulkGenerating || !bulkSelected.size}
                  style={{ background: bulkGenerating || !bulkSelected.size ? '#3c3c3c' : '#ffffff', border: 'none', borderRadius: 10, padding: '10px 22px', color: bulkGenerating || !bulkSelected.size ? '#777777' : '#000000', fontWeight: 700, cursor: bulkGenerating ? 'wait' : 'pointer', fontSize: 14, opacity: !bulkSelected.size ? 0.5 : 1 }}>
                ...
                {bulkResults.filter(r => r.status === 'ok').length > 0 && (
                  <button onClick={bulkPushAll} disabled={bulkPushing} style={{ background: bulkPushing ? '#3c3c3c' : 'linear-gradient(135deg, #7c3aed, #6d28d9)', border: 'none', borderRadius: 10, padding: '10px 22px', color: '#fff', fontWeight: 700, cursor: bulkPushing ? 'wait' : 'pointer', fontSize: 14 }}>
            ...
            {bulkGenerating && (
              <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 14, padding: 20, marginBottom: 20 }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
                  <span style={{ fontSize: 14, color: '#bbbbbb', fontWeight: 600 }}>Generating SEO...</span>
            ...
            {bulkResults.length > 0 ? (
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 14 }}>
                {bulkResults.map((r, i) => {
                  ...
                  return (
                    <div key={i} style={{ background: '#282828', border: '1px solid ' + (r.status === 'ok' ? '#4ade8025' : '#f8717125'), borderRadius: 14, padding: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 10 }}>
                        ...
                        <div style={{ flex: 1, minWidth: 0 }}>
                          <div style={{ fontWeight: 700, fontSize: 14, color: '#f0f0f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{r.product.title}</div>
                          {r.status === 'ok'
                            ? <div style={{ fontSize: 12, color: '#777777', marginTop: 2 }}>Score: <span style={{ color: rc, fontWeight: 700 }}>{r.score}/100</span></div>
                      ...
                      {r.status === 'ok' && (
                        <div style={{ fontSize: 12, lineHeight: 1.7, borderTop: '1px solid #2e2e2e', paddingTop: 10 }}>
                          <div><span style={{ color: '#777777' }}>Title: </span><span style={{ color: '#bbbbbb' }}>{r.fields.seoTitle}</span></div>
                          <div style={{ marginTop: 4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}><span style={{ color: '#777777' }}>Desc: </span><span style={{ color: '#bbbbbb' }}>{r.fields.seoDescription}</span></div>
                        </div>
                      )}
                    </div>
                  );
                ...
                {products.map(p => {
                  ...
                  return (
                    <div key={p.id} onClick={() => { const n = new Set(bulkSelected); sel ? n.delete(p.id) : n.add(p.id); setBulkSelected(n); }}
                      style={{ background: '#282828', border: '2px solid ' + (sel ? '#ffffff' : '#2e2e2e'), borderRadius: 12, padding: '14px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, transition: 'all 0.15s' }}>
                      <div style={{ width: 22, height: 22, borderRadius: 6, border: '2px solid ' + (sel ? '#ffffff' : '#555555'), background: sel ? '#ffffff' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: 14, color: '#f0f0f0', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.title}</div>
                        <div style={{ fontSize: 11, color: '#777777' }}>{p.status} - Score: {seoScore(stored[p.id] || {})}</div>
                      </div>
                    </div>
                  );
                })}
        ...
        {tab === 'analytics' && (
          <div>
            <h2 style={{ margin: '0 0 24px', fontSize: 22, fontWeight: 800, color: '#f0f0f0' }}>SEO Analytics Overview</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: 14, marginBottom: 28 }}>
              {[
                ...
                <div key={st.label} style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 16, padding: '18px 20px' }}>
                  <div style={{ fontSize: 32, fontWeight: 800, color: st.colour, lineHeight: 1 }}>{st.value}</div>
                  <div style={{ fontSize: 12, color: '#777777', marginTop: 8, fontWeight: 400 }}>{st.label}</div>
                </div>
              ))}
            </div>

            <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 16, padding: 24, marginBottom: 20 }}>
              <h3 style={{ margin: '0 0 18px', fontSize: 16, fontWeight: 700, color: '#f0f0f0' }}>Score Distribution</h3>
              {[
                ...
                return (
                  <div key={band.label} style={{ marginBottom: 14 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                      <span style={{ fontSize: 13, color: '#bbbbbb' }}>{band.label}</span>
                      <span style={{ fontSize: 13, color: band.colour, fontWeight: 700 }}>{count} products - {pct}%</span>
                    </div>
                    <div style={{ height: 10, background: '#282828', borderRadius: 5, overflow: 'hidden' }}>
            ...
            <div style={{ background: '#282828', border: '1px solid #2e2e2e', borderRadius: 16, padding: 24 }}>
              <h3 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 700, color: '#f0f0f0' }}>Needs Attention</h3>
              {products.filter(p => seoScore(stored[p.id] || {}) < 50).length === 0 ? (
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                  {products.filter(p => seoScore(stored[p.id] || {}) < 50).slice(0, 10).map(p => {
                    ...
                    return (
                      <div key={p.id} onClick={() => selectProduct(p)}
                      style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', background: '#181818', borderRadius: 10, cursor: 'pointer' }}
