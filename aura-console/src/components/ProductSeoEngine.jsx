...
function ScoreRing({ score, size = 88 }) {
 ...
 <div style={{ width: size, height: size, borderRadius: '50%', background: `conic-gradient(${colour} ${score * 3.6}deg, #27272a ${score * 3.6}deg)`, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: `0 0 20px ${colour}33` }}>
 <div style={{ width: inner, height: inner, borderRadius: '50%', background: '#09090b', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center'}}>
...
function CharBar({ value = '', min, max, sweetMin, sweetMax, label }) {
 ...
 const colour = len === 0 ? '#52525b': over ? '#f87171': perfect ? '#4ade80': inRange ? '#a3e635': '#fbbf24';
 const statusText = len === 0 ? 'Empty': over ? 'Too long': len < min ? 'Too short': perfect ? 'Perfect': `Good - aim for ${sweetMin || min}+`;
 return (
...
function SerpPreview({ title, description, handle, shop, device }) {
 ...
 <div>
 <div style={{ fontSize: 13, color: '#27272a', fontWeight: 500 }}>{shop || 'yourstore.myshopify.com'}</div>
 ...
 <div style={{ fontSize: isMobile ? 16 : 20, color: '#52525b', marginBottom: 6, lineHeight: 1.3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>
...
function GenBtn({ onClick, loading, small }) {
 return (
 <button onClick={onClick} disabled={loading} style={{ background: loading ? '#3f3f46': '#ffffff', border: 'none', borderRadius: small ? 6 : 8, padding: small ? '5px 12px': '7px 16px', color: '#000000', fontWeight: 700, fontSize: small ? 12 : 13, cursor: loading ? 'not-allowed': 'pointer', display: 'flex', alignItems: 'center', gap: 5, flexShrink: 0, opacity: loading ? 0.5 : 1 }}>
...
function FieldBlock({ label, hint, badge, children }) {
 return (
 <div style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 12, padding: 18 }}>
 <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
 <div>
 <div style={{ fontWeight: 600, color: '#a1a1aa', fontSize: 12, textTransform: 'uppercase', letterSpacing: '0.06em'}}>{label}</div>
...
export default function ProductSeoEngine() {
 ...
 <div style={{ background: '#0f0f0f', borderBottom: '1px solid #27272a', padding: '0 32px'}}>
 ...
 <p style={{ margin: '2px 0 0', fontSize: 12, color: '#71717a'}}>AI-powered SEO optimisation and direct Shopify publishing</p>
 </div>
 </div>
 <div style={{ display: 'flex', gap: 8, alignItems: 'center'}}>
 {[
 { label: 'Products', value: products.length, colour: '#ffffff'},
 { label: 'Optimised', value: Object.keys(stored).length, colour: '#a1a1aa'},
 ...
 <div key={st.label} style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 10, padding: '9px 16px', textAlign: 'center', minWidth: 76 }}>
 <div style={{ fontSize: 20, fontWeight: 700, color: st.colour }}>{st.value}</div>
 <div style={{ fontSize: 11, color: '#71717a', marginTop: 1 }}>{st.label}</div>
 </div>
 ))}
 <button onClick={loadProducts} style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 10, padding: '9px 16px', color: '#a1a1aa', cursor: 'pointer', fontSize: 13, fontWeight: 600 }}>Refresh</button>
 </div>
 </div>
 <div style={{ display: 'flex', borderTop: '1px solid #27272a'}}>
 {TABS.map(t => (
 <button key={t.id} onClick={() => setTab(t.id)} style={{ background: 'none', border: 'none', borderBottom: tab === t.id ? '2px solid #ffffff': '2px solid transparent', padding: '13px 22px', cursor: 'pointer', color: tab === t.id ? '#ffffff': '#71717a', fontWeight: tab === t.id ? 600 : 400, fontSize: 13, transition: 'color 0.15s'}}>
 {t.label}
 </button>
 ))}
 </div>
 ...
 style={{ flex: 1, minWidth: 200, background: '#18181b', border: '1px solid #3f3f46', borderRadius: 10, padding: '11px 14px', color: '#fafafa', fontSize: 14, outline: 'none'}} />
 <select value={statusFilter} onChange={e => setStatusFilter(e.target.value)} style={{ background: '#18181b', border: '1px solid #3f3f46', borderRadius: 10, padding: '11px 14px', color: '#fafafa', fontSize: 14, cursor: 'pointer'}}>
 <option value="all">All Status</option>
 <option value="active">Active</option>
 <option value="draft">Draft</option>
 <option value="archived">Archived</option>
 </select>
 <select value={sortBy} onChange={e => setSortBy(e.target.value)} style={{ background: '#18181b', border: '1px solid #3f3f46', borderRadius: 10, padding: '11px 14px', color: '#fafafa', fontSize: 14, cursor: 'pointer'}}>
 ...
 {[...Array(8)].map((_, i) => <div key={i} style={{ background: '#09090b', borderRadius: 16, height: 130, animation: 'pulse 1.5s ease infinite'}} />)}
 </div>
 ) : filteredProducts.length === 0 ? (
 <div style={{ textAlign: 'center', padding: '80px 0', color: '#71717a'}}>
 <div style={{ fontSize: 20, fontWeight: 700, color: '#71717a', marginBottom: 8 }}>No products found</div>
 ...
 style={{ background: '#18181b', border: '1px solid '+ (hasData ? '#27272a': '#2a1515'), borderRadius: 16, padding: 18, cursor: 'pointer', transition: 'all 0.2s', position: 'relative'}}
 ...
 onMouseLeave={e => { e.currentTarget.style.borderColor = hasData ? '#27272a': '#2a1515'; e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = ''; }}
 >
 <div style={{ position: 'absolute', top: 14, right: 14, background: hasData ? pb : '#18181b', border: '1px solid '+ (hasData ? pc : '#27272a') + '40', borderRadius: 8, padding: '4px 10px', textAlign: 'center'}}>
 <div style={{ fontSize: 16, fontWeight: 900, color: hasData ? pc : '#52525b'}}>{hasData ? ps : '-'}</div>
 <div style={{ fontSize: 9, color: (hasData ? pc : '#52525b') + 'aa', fontWeight: 600 }}>SEO</div>
 </div>
 <div style={{ display: 'flex', gap: 14, alignItems: 'flex-start', paddingRight: 56 }}>
 {p.image
 ? <img src={p.image} alt={p.title} style={{ width: 56, height: 56, borderRadius: 12, objectFit: 'cover', flexShrink: 0, border: '1px solid #27272a'}} />
 : <div style={{ width: 56, height: 56, borderRadius: 12, background: '#18181b', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 11, color: '#71717a', flexShrink: 0 }}>IMG</div>}
 <div style={{ minWidth: 0, flex: 1 }}>
 <div style={{ fontWeight: 700, fontSize: 15, color: '#fafafa', marginBottom: 3, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{p.title}</div>
 <div style={{ fontSize: 12, color: '#71717a', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{p.vendor} - {p.handle}</div>
 ...
 <div style={{ fontSize: 20, fontWeight: 700, color: '#71717a', marginBottom: 20 }}>Select a product first</div>
 ...
 <div>
 <div style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 16, padding: '16px 20px', display: 'flex', alignItems: 'center', gap: 16, marginBottom: 24, flexWrap: 'wrap'}}>
 {selectedProduct.image && <img src={selectedProduct.image} alt={selectedProduct.title} style={{ width: 52, height: 52, borderRadius: 12, objectFit: 'cover', border: '1px solid #27272a'}} />}
 <div style={{ flex: 1, minWidth: 0 }}>
 <div style={{ fontWeight: 800, fontSize: 17, color: '#fafafa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{selectedProduct.title}</div>
 <div style={{ fontSize: 12, color: '#71717a', marginTop: 2 }}>{selectedProduct.vendor} - /products/{selectedProduct.handle}</div>
 </div>
 {savedAt && <div style={{ fontSize: 11, color: '#71717a', whiteSpace: 'nowrap'}}>Auto-saved {savedAt.toLocaleTimeString()}</div>}
 <div style={{ display: 'flex', gap: 10, flexShrink: 0, flexWrap: 'wrap'}}>
 <button onClick={generateAll} disabled={generating}
 style={{ background: generating ? '#3f3f46': '#ffffff', border: 'none', borderRadius: 12, padding: '10px 22px', color: generating ? '#71717a': '#000000', fontWeight: 700, cursor: generating ? 'wait': 'pointer', fontSize: 14 }}>
 ...
 style={{ background: '#09090b', border: '1px solid #27272a', borderRadius: 12, padding: '10px 18px', color: '#a1a1aa', fontWeight: 600, cursor: 'pointer', fontSize: 13 }}>Save Draft</button>
 <button onClick={pushToShopify} disabled={pushing || !editor.seoTitle}
 style={{ background: pushing || !editor.seoTitle ? '#3f3f46': 'linear-gradient(135deg, #7c3aed, #6d28d9)', border: 'none', borderRadius: 12, padding: '10px 22px', color: '#fff', fontWeight: 700, cursor: pushing ? 'wait': 'pointer', fontSize: 14 }}>
 {pushing ? 'Pushing...': 'Push to Shopify'}
 </button>
 </div>
 </div>

 <div style={{ display: 'flex', gap: 4, marginBottom: 20, background: '#18181b', border: '1px solid #27272a', borderRadius: 12, padding: 4, width: 'fit-content'}}>
 {[['fields', 'Fields'], ['preview', 'SERP Preview'], ['checklist', 'SEO Checklist']].map(([id, lbl]) => (
 <button key={id} onClick={() => setEditorTab(id)} style={{ background: editorTab === id ? 'rgba(255,255,255,0.08)': 'none', border: editorTab === id ? '1px solid rgba(255,255,255,0.15)': '1px solid transparent', borderRadius: 8, padding: '7px 16px', color: editorTab === id ? '#ffffff': '#71717a', fontWeight: editorTab === id ? 600 : 400, fontSize: 13, cursor: 'pointer'}}>
 {lbl}
 </button>
 ))}
 </div>
 ...
 placeholder="e.g. Premium Snowboard - Fast and Responsive"style={{ width: '100%', background: '#09090b', border: '1px solid '+ (editor.seoTitle.length > 60 ? '#f87171': '#27272a'), borderRadius: 10, padding: '11px 14px', color: '#fafafa', fontSize: 15, outline: 'none', boxSizing: 'border-box'}} />
 ...
 placeholder="Compelling description with your main keyword and a call-to-action."rows={4}
 style={{ width: '100%', background: '#09090b', border: '1px solid '+ (editor.seoDescription.length > 160 ? '#f87171': '#27272a'), borderRadius: 10, padding: '11px 14px', color: '#fafafa', fontSize: 14, outline: 'none', resize: 'vertical', lineHeight: 1.6, boxSizing: 'border-box'}} />
 ...
 <div style={{ display: 'flex', background: '#09090b', border: '1px solid #27272a', borderRadius: 10, overflow: 'hidden'}}>
 <span style={{ padding: '11px 12px', color: '#52525b', fontSize: 13, borderRight: '1px solid #27272a', whiteSpace: 'nowrap'}}>/products/</span>
 <input value={editor.handle} onChange={e => setEditor(ed => ({ ...ed, handle: e.target.value.toLowerCase().replace(/[^a-z0-9-]/g, '-') }))}
 style={{ flex: 1, background: 'none', border: 'none', padding: '11px 14px', color: '#fafafa', fontSize: 14, outline: 'none'}} />
 </div>
 </FieldBlock>
 ...
 <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, background: '#09090b', border: '1px solid #27272a', borderRadius: 10, padding: '8px 10px', minHeight: 50, alignItems: 'center'}}>
 {chips.map(kw => {
 ...
 style={{ flex: 1, minWidth: 120, background: 'none', border: 'none', color: '#fafafa', fontSize: 13, outline: 'none', padding: '3px 4px'}} />
 </div>
 {chips.length > 0 && <div style={{ marginTop: 8, fontSize: 11, color: '#71717a'}}>Green = in title and desc / Yellow = in one / Red = in neither</div>}
 </FieldBlock>
 ...
 placeholder="e.g. Red premium freestyle snowboard with carbon fibre base"style={{ width: '100%', background: '#09090b', border: '1px solid #27272a', borderRadius: 10, padding: '11px 14px', color: '#fafafa', fontSize: 14, outline: 'none', boxSizing: 'border-box'}} />
 </FieldBlock>
 </div>

 <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
 <div style={{ background: '#18181b', border: '1px solid '+ scoreColour + '30', borderRadius: 16, padding: 22 }}>
 <div style={{ display: 'flex', alignItems: 'center', gap: 20, marginBottom: 18 }}>
 <ScoreRing score={score} />
 <div>
 <div style={{ fontWeight: 800, fontSize: 18, color: '#fafafa'}}>SEO Health Score</div>
 <div style={{ fontSize: 13, color: '#71717a', marginTop: 3 }}>Updates as you type</div>
 ...
 <div key={item.label} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '7px 10px', background: '#09090b', borderRadius: 8, marginBottom: 5 }}>
 ...
 <div style={{ display: 'flex', gap: 8 }}>
 <span style={{ fontSize: 11, color: item.ok ? '#4ade80': '#71717a'}}>{item.msg}</span>
 <span style={{ fontSize: 11, fontWeight: 700, color: item.earned > 0 ? '#4f46e5': '#52525b', background: item.earned > 0 ? '#4f46e510': 'none', border: '1px solid '+ (item.earned > 0 ? '#4f46e520': '#27272a'), borderRadius: 4, padding: '1px 6px'}}>{item.earned}/{item.pts}</span>
 </div>
 </div>
 ));
 })()}
 ...
 <div style={{ fontWeight: 600, fontSize: 13, color: '#a1a1aa', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: 12 }}>Keyword Presence</div>
 {chips.slice(0, 8).map(kw => {
 const inT = kwInTitleMatch(kw);
 const inD = kwInDescMatch(kw);
 return (
 <div key={kw} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '6px 10px', background: '#09090b', borderRadius: 8, marginBottom: 5 }}>
 <span style={{ fontSize: 12, color: '#a1a1aa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', maxWidth: 130 }}>"{kw}"</span>
 ...
 <button key={d} onClick={() => setSerpDevice(d)} style={{ background: serpDevice === d ? 'rgba(255,255,255,0.08)': '#18181b', border: '1px solid '+ (serpDevice === d ? 'rgba(255,255,255,0.2)': '#27272a'), borderRadius: 10, padding: '8px 18px', color: serpDevice === d ? '#ffffff': '#71717a', fontWeight: serpDevice === d ? 600 : 400, fontSize: 13, cursor: 'pointer'}}>{lbl}</button>
 ))}
 </div>
 ...
 <div style={{ fontSize: 12, color: '#71717a', marginBottom: 12, textTransform: 'uppercase', letterSpacing: '0.06em', fontWeight: 600 }}>Google Search Result Preview</div>
 ...
 <div style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 12, padding: 14 }}>
 <div style={{ fontSize: 11, color: '#71717a', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Title</div>
 ...
 <div style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 12, padding: 14 }}>
 <div style={{ fontSize: 11, color: '#71717a', fontWeight: 600, textTransform: 'uppercase', marginBottom: 8 }}>Description</div>
 ...
 <div style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 16, padding: 22 }}>
 <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 16 }}>
 <div style={{ fontWeight: 800, fontSize: 17, color: '#fafafa'}}>SEO Checklist</div>
 ...
 <div style={{ height: 6, background: '#27272a', borderRadius: 3, overflow: 'hidden', marginBottom: 18 }}>
 ...
 <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 12, padding: '10px 14px', background: item.ok ? '#071a10': '#09090b', border: '1px solid '+ (item.ok ? '#4ade8020': '#27272a'), borderRadius: 10, marginBottom: 6 }}>
 ...
 <div>
 <div style={{ fontSize: 13, fontWeight: 600, color: item.ok ? '#fafafa': '#a1a1aa'}}>{item.label}</div>
 ...
 <div>
 <h2 style={{ margin: 0, fontSize: 22, fontWeight: 800, color: '#fafafa'}}>Bulk SEO Generation</h2>
 <p style={{ margin: '6px 0 0', fontSize: 14, color: '#71717a'}}>Select products and generate SEO for all at once then publish</p>
 </div>
 <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap'}}>
 <button onClick={() => setBulkSelected(new Set(products.map(p => p.id)))} style={{ background: '#09090b', border: '1px solid #27272a', borderRadius: 10, padding: '10px 16px', color: '#a1a1aa', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>Select All ({products.length})</button>
 <button onClick={() => setBulkSelected(new Set())} style={{ background: '#09090b', border: '1px solid #27272a', borderRadius: 10, padding: '10px 16px', color: '#a1a1aa', cursor: 'pointer', fontSize: 13, fontWeight: 500 }}>Clear</button>
 <button onClick={runBulkGenerate} disabled={bulkGenerating || !bulkSelected.size}
 style={{ background: bulkGenerating || !bulkSelected.size ? '#3f3f46': '#ffffff', border: 'none', borderRadius: 10, padding: '10px 22px', color: bulkGenerating || !bulkSelected.size ? '#71717a': '#000000', fontWeight: 700, cursor: bulkGenerating ? 'wait': 'pointer', fontSize: 14, opacity: !bulkSelected.size ? 0.5 : 1 }}>
 ...
 <button onClick={bulkPushAll} disabled={bulkPushing} style={{ background: bulkPushing ? '#3f3f46': 'linear-gradient(135deg, #7c3aed, #6d28d9)', border: 'none', borderRadius: 10, padding: '10px 22px', color: '#fff', fontWeight: 700, cursor: bulkPushing ? 'wait': 'pointer', fontSize: 14 }}>
 ...
 <span style={{ fontSize: 14, color: '#a1a1aa', fontWeight: 600 }}>Generating SEO...</span>
 ...
 return (
 <div key={i} style={{ background: '#18181b', border: '1px solid '+ (r.status === 'ok'? '#4ade8025': '#f8717125'), borderRadius: 14, padding: 16 }}>
 ...
 <div style={{ flex: 1, minWidth: 0 }}>
 <div style={{ fontWeight: 700, fontSize: 14, color: '#fafafa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{r.product.title}</div>
 {r.status === 'ok'? <div style={{ fontSize: 12, color: '#71717a', marginTop: 2 }}>Score: <span style={{ color: rc, fontWeight: 700 }}>{r.score}/100</span></div>
 ...
 {r.status === 'ok'&& (
 <div style={{ fontSize: 12, lineHeight: 1.7, borderTop: '1px solid #27272a', paddingTop: 10 }}>
 <div><span style={{ color: '#71717a'}}>Title: </span><span style={{ color: '#a1a1aa'}}>{r.fields.seoTitle}</span></div>
 <div style={{ marginTop: 4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical'}}><span style={{ color: '#71717a'}}>Desc: </span><span style={{ color: '#a1a1aa'}}>{r.fields.seoDescription}</span></div>
 </div>
 )}
 </div>
 );
 ...
 style={{ background: '#18181b', border: '2px solid '+ (sel ? '#ffffff': '#27272a'), borderRadius: 12, padding: '14px 16px', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 12, transition: 'all 0.15s'}}>
 <div style={{ width: 22, height: 22, borderRadius: 6, border: '2px solid '+ (sel ? '#ffffff': '#52525b'), background: sel ? '#ffffff': 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
 ...
 <div style={{ flex: 1, minWidth: 0 }}>
 <div style={{ fontWeight: 600, fontSize: 14, color: '#fafafa', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap'}}>{p.title}</div>
 <div style={{ fontSize: 11, color: '#71717a'}}>{p.status} - Score: {seoScore(stored[p.id] || {})}</div>
 </div>
 </div>
 );
 })}
 ...
 {tab === 'analytics'&& (
 <div>
 <h2 style={{ margin: '0 0 24px', fontSize: 22, fontWeight: 800, color: '#fafafa'}}>SEO Analytics Overview</h2>
 ...
 <div key={st.label} style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 16, padding: '18px 20px'}}>
 <div style={{ fontSize: 32, fontWeight: 800, color: st.colour, lineHeight: 1 }}>{st.value}</div>
 <div style={{ fontSize: 12, color: '#71717a', marginTop: 8, fontWeight: 400 }}>{st.label}</div>
 </div>
 ))}
 </div>

 <div style={{ background: '#18181b', border: '1px solid #27272a', borderRadius: 16, padding: 24, marginBottom: 20 }}>
 <h3 style={{ margin: '0 0 18px', fontSize: 16, fontWeight: 700, color: '#fafafa'}}>Score Distribution</h3>
 {[
 ...
 <span style={{ fontSize: 13, color: '#a1a1aa'}}>{band.label}</span>
 <span style={{ fontSize: 13, color: band.colour, fontWeight: 700 }}>{count} products - {pct}%</span>
 </div>
 <div style={{ height: 10, background: '#18181b', borderRadius: 5, overflow: 'hidden'}}>
 ...
 <h3 style={{ margin: '0 0 16px', fontSize: 16, fontWeight: 700, color: '#fafafa'}}>Needs Attention</h3>
 ...
 return (
 <div key={p.id} onClick={() => selectProduct(p)}
 style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '12px 16px', background: '#09090b', borderRadius: 10, cursor: 'pointer'}}
