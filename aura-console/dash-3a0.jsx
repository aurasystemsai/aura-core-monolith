		...
		<div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
			<div style={{ flex: 1 }}>
				<div style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 13, color: "#94a3b8", fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px" }}>
					{label}
					{tooltip && (
						<span title={tooltip} style={{ cursor: "help", fontSize: 12, opacity: 0.7 }}>Γä╣∩╕Å</span>
					)}
				</div>
...
const Dashboard = ({ setActiveSection }) => {
				...
				try {
					...
					if (ordersDetailRes.orders && Array.isArray(ordersDetailRes.orders)) {
						...
						setRecentActivity(activities.length > 0 ? activities : [
							{ icon: "≡ƒôª", title: "No recent orders", timestamp: "ΓÇö", type: "Info" }
						]);
					} else {
						// Fallback to generic activity
						setRecentActivity([
							{ icon: "≡ƒôª", title: "Awaiting order data", timestamp: "ΓÇö", type: "Info" }
						]);
					}
				} catch (ordersErr) {
					console.warn("Failed to fetch recent orders:", ordersErr);
					setRecentActivity([
						{ icon: "ΓÜá∩╕Å", title: "Unable to load recent activity", timestamp: "ΓÇö", type: "Error" }
					]);
				}
	...
	const generateRecommendations = (statsData) => {
		...
		if (statsData.seoIssues >= 1) {
			recs.push({ icon: "≡ƒöì", text: "You have " + statsData.seoIssues + " SEO issue" + (statsData.seoIssues > 1 ? "s" : "") + ". Fix them to improve search rankings.", action: "Fix SEO Issues", link: "seo" });
		}
		...
		if (!statsData.conversion || statsData.conversion === "0%" || parseFloat(statsData.conversion) < 2) {
			recs.push({ icon: "≡ƒôê", text: "Low conversion rate. Consider A/B testing your product pages and improving CTAs.", action: "Optimize Conversion", link: "tools" });
		}
		// Low orders
		if (statsData.orders < 10) {
			recs.push({ icon: "≡ƒôº", text: "Boost sales with targeted email campaigns to your customers.", action: "Create Campaign", link: "tools" });
		}
		// Underperforming products
		if (underperformingProducts.length > 0) {
			recs.push({ icon: "ΓÜá∩╕Å", text: underperformingProducts.length + " product" + (underperformingProducts.length > 1 ? "s" : "") + " need attention (low price or missing data).", action: "View Products", link: "products" });
		}
		...
		<div className="aura-dashboard-shell" style={{ padding: "24px", background: "#0f172a", minHeight: "100vh" }}>
						...
						<div style={{ padding:'24px 28px 20px', borderBottom:'1px solid #2f3650', display:'flex', alignItems:'center', justifyContent:'space-between' }}>
							<div style={{ display:'flex', alignItems:'center', gap:14 }}>
								{scanningInProgress ? (
									...
									<span style={{ fontSize:32 }}>Γ£à</span>
								)}
								<div>
									<h2 style={{ color:'#e5e7eb', fontWeight:800, fontSize:20, margin:0 }}>
										{scanningInProgress ? 'Scanning your site...' : `Scan Complete ΓÇö ${crawlResults?.totalIssues || 0} Issues Found`}
									</h2>
									<p style={{ color:'#94a3b8', fontSize:13, margin:'3px 0 0 0' }}>
										{scanningInProgress
											? `Checking pages for SEO issues ΓÇö this takes a minute or two`
											: `${crawlResults?.pagesScanned} pages scanned ΓÇó ${crawlResults?.high} high ΓÇó ${crawlResults?.medium} medium ΓÇó ${crawlResults?.low} low`}
									</p>
								</div>
							</div>
							{!scanningInProgress && (
								...
								<div style={{ display:'flex', flexDirection:'column', gap:6 }}>
									{[...scanLog].reverse().map((entry, i) => (
										<div key={i} style={{ display:'flex', alignItems:'center', gap:10, animation:'scanSlide 0.3s ease', opacity: i === 0 ? 1 : Math.max(0.2, 1 - i * 0.07) }}>
											...
											<span style={{ fontSize:11, color:'#4ade80', flexShrink:0 }}>Γ£ô scanned</span>
										</div>
									))}
								</div>
								{scanRemainingTime > 0 && (
						...
						{!scanningInProgress && crawlResults && (
							<div style={{ flex:1, overflowY:'auto', padding:'0 28px 24px' }}>
								...
								<div style={{ display:'flex', gap:10, padding:'16px 0', flexWrap:'wrap' }}>
									<span style={{ background:'#2d1515', border:'1px solid #e53e3e', color:'#fc8181', padding:'5px 14px', borderRadius:20, fontSize:13, fontWeight:600 }}>≡ƒö┤ {crawlResults.high} High</span>
									<span style={{ background:'#2d2210', border:'1px solid #f59e0b', color:'#fbbf24', padding:'5px 14px', borderRadius:20, fontSize:13, fontWeight:600 }}>≡ƒƒí {crawlResults.medium} Medium</span>
									<span style={{ background:'#1a2315', border:'1px solid #4ade80', color:'#86efac', padding:'5px 14px', borderRadius:20, fontSize:13, fontWeight:600 }}>≡ƒƒó {crawlResults.low} Low</span>
									<span style={{ marginLeft:'auto', color:'#475569', fontSize:13, alignSelf:'center' }}>{crawlResults.pagesScanned} pages scanned ΓÇó {lastScanTime}</span>
								</div>
								{crawlResults.totalIssues === 0 ? (
									<div style={{ textAlign:'center', padding:'48px 0', color:'#7fffd4', fontSize:16 }}>Γ£à No SEO issues found ΓÇö your site looks great!</div>
								) : (
									<div style={{ display:'flex', flexDirection:'column', gap:8 }}>
										{crawlResults.issues.map((issue, i) => (
											<div key={i}
												...
												<span style={{ fontSize:16, marginTop:1 }}>{issue.severity==='high'?'≡ƒö┤':issue.severity==='medium'?'≡ƒƒí':'≡ƒƒó'}</span>
												<div style={{ flex:1, minWidth:0 }}>
													<div style={{ color:'#e5e7eb', fontWeight:600, fontSize:14, display:'flex', alignItems:'center', gap:8 }}>
														{issue.type}
														{issue.fix && <span style={{ fontSize:11, color:'#7fffd4', background:'rgba(127,255,212,0.1)', border:'1px solid rgba(127,255,212,0.3)', borderRadius:4, padding:'1px 7px', fontWeight:500 }}>Click to fix ΓåÆ</span>}
													</div>
			...
			<div style={{ position: "fixed", bottom: 24, right: 24, zIndex: 9999, display: "flex", flexDirection: "column", gap: 10 }}>
				{toasts.map(toast => (
					<div key={toast.id} className="aura-toast" style={{
						...
						<span style={{ fontSize: 18 }}>{toast.type === 'error' ? 'ΓÜá∩╕Å' : 'Γ£à'}</span>
						<span>{toast.message}</span>
						<button onClick={() => setToasts(prev => prev.filter(t => t.id !== toast.id))}
							style={{ marginLeft: "auto", background: "none", border: "none", color: "inherit", cursor: "pointer", fontSize: 18, lineHeight: 1, padding: "0 0 0 8px", opacity: 0.7 }}>├ù</button>
					</div>
				))}
			</div>
					...
					<div style={{ display: "flex", alignItems: "center", gap: 16 }}>
						...
						<div>
							...
							<p style={{ fontSize: 14, color: "#94a3b8", margin: "4px 0 0 0" }}>
								{shop.name || "My Store"} ΓÇó {shop.domain || "ΓÇö"}
							</p>
						</div>
					</div>
			...
			>
				...
				<div style={{ display: "flex", gap: 12, alignItems: "flex-start" }}>
					...
					>
						{copilotLoading ? "ΓÅ│ Thinking..." : "Γ£¿ Ask"}
					</button>
				</div>
				{copilotReply && (
					<div
			...
			{alerts.length > 0 && (
				<div style={{ marginBottom: 24 }}>
					{alerts.map((alert, idx) => (
						...
						>
							<span style={{ fontSize: 20 }}>{alert.type === "danger" ? "≡ƒÜ¿" : "ΓÜá∩╕Å"}</span>
			...
			>
				<StatCard 
					label="Total Revenue" 
					value={stats.revenue || (stats.revenue === "$0" ? "$0" : "ΓÇö")} 
					icon="≡ƒÆ░"
					tooltip="Total sales revenue for the selected period"
					subtitle={(!stats.revenue || stats.revenue === "$0" || stats.revenue === "ΓÇö") ? "No revenue data available" : null}
				/>
				<StatCard 
					label="Orders" 
					value={stats.orders !== null && stats.orders !== undefined ? stats.orders : "ΓÇö"} 
					icon="≡ƒôª"
				...
				<StatCard 
					...
					value={stats.conversion || "ΓÇö"} 
					icon="≡ƒôê"
					tooltip="Percentage of visitors who made a purchase"
				/>
				<StatCard 
					label="Visitors" 
					value={stats.visitors || "ΓÇö"} 
					icon="≡ƒæÑ"
				...
				<StatCard 
					label="Products" 
					value={stats.products !== null ? stats.products : "ΓÇö"} 
					icon="≡ƒ¢ì∩╕Å"
					tooltip="Total number of products in your catalog"
				/>
				<StatCard 
					label="SEO Issues" 
					value={stats.seoIssues !== null ? stats.seoIssues : "ΓÇö"} 
					icon="≡ƒöì"
			...
			{crawlResults && (
				<div style={{ background: "linear-gradient(135deg, #1a1d2e 0%, #232842 100%)", border: "1px solid #2f3650", borderRadius: 16, padding: 24, marginBottom: 32 }}>
					<div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 20 }}>
						<div>
							<h3 style={{ color: "#e5e7eb", fontWeight: 700, fontSize: 18, margin: 0 }}>≡ƒöì SEO Scan Results</h3>
							<p style={{ color: "#94a3b8", fontSize: 13, margin: "4px 0 0 0" }}>
								{crawlResults.pagesScanned} pages scanned ΓÇó Last scan: {lastScanTime}
							</p>
						</div>
						<div style={{ display: "flex", gap: 12, alignItems: "center" }}>
							...
							<span style={{ background: "#2d2210", border: "1px solid #f59e0b", color: "#fbbf24", padding: "4px 12px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>
								≡ƒƒí {crawlResults.medium} Medium
							</span>
							<span style={{ background: "#1a2315", border: "1px solid #4ade80", color: "#86efac", padding: "4px 12px", borderRadius: 20, fontSize: 13, fontWeight: 600 }}>
								≡ƒƒó {crawlResults.low} Low
							</span>
							<button onClick={() => setCrawlResults(null)} style={{ background: "none", border: "none", color: "#94a3b8", cursor: "pointer", fontSize: 20, lineHeight: 1 }}>├ù</button>
						</div>
					</div>
					{crawlResults.totalIssues === 0 ? (
						<div style={{ textAlign: "center", padding: "32px 0", color: "#7fffd4", fontSize: 16 }}>
							Γ£à No SEO issues found ΓÇö your site looks great!
						</div>
					) : (
							...
							{crawlResults.issues.map((issue, i) => (
								...
								>
									<span style={{ fontSize: 16, marginTop: 1 }}>
										{issue.severity === 'high' ? '≡ƒö┤' : issue.severity === 'medium' ? '≡ƒƒí' : '≡ƒƒó'}
									</span>
									<div style={{ flex: 1, minWidth: 0 }}>
										<div style={{ color: "#e5e7eb", fontWeight: 600, fontSize: 14, display: "flex", alignItems: "center", gap: 8 }}>
											{issue.type}
			...
			<div style={{ display: "grid", gridTemplateColumns: "2fr 1fr", gap: 24, marginBottom: 32 }}>
				...
				<div style={{ display: "flex", flexDirection: "column", gap: 24 }}>
					...
					>
						<h3 style={{ fontSize: 18, fontWeight: 700, color: "#e5e7eb", margin: "0 0 20px 0" }}>ΓÜí Recent Activity</h3>
						<div>
					...
					>
						...
						{topProducts.length > 0 ? (
							...
							<div style={{ textAlign: "center", color: "#94a3b8", padding: "20px 0" }}>
								<div style={{ fontSize: 32, marginBottom: 8 }}>≡ƒôª</div>
						...
						>
							...
							<div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
								{underperformingProducts.map((product, idx) => (
									...
									>
										<span style={{ fontSize: 20 }}>ΓÜá∩╕Å</span>
										<div style={{ flex: 1 }}>
											<div style={{ fontSize: 14, fontWeight: 600, color: "#e5e7eb", marginBottom: 4 }}>
												{product.title || "Untitled Product"}
											</div>
			...
			{/* Quick Actions */}
			<div style={{ marginBottom: 32 }}>
				<h3 style={{ fontSize: 20, fontWeight: 700, color: "#e5e7eb", marginBottom: 20 }}>≡ƒÜÇ Quick Actions</h3>
				<div
					style={{
						display: "grid",
				...
				>
					<QuickActionCard
						icon={scanningInProgress ? "ΓÅ│" : "≡ƒöì"}
						title={scanningInProgress ? "Scanning..." : "Run SEO Scan"}
						description={
							scanningInProgress && scanRemainingTime > 0
								? `Time remaining: ${Math.floor(scanRemainingTime / 60)}:${String(scanRemainingTime % 60).padStart(2, '0')} ΓÇó Analyzing ${stats.products || '...'} products` 
								: scanningInProgress 
									? "Analyzing your site for SEO issues..." 
									: lastScanTime 
										? `Scan your store for SEO issues ΓÇó Last: ${lastScanTime}` 
